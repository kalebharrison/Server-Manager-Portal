...
/** Light-only chrome so Gmail dark mode does not invert the header into unreadable white-on-white. */
...
<div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f6f9; padding: 30px; color: #333333; line-height: 1.6;">
  <div style="max-width: 600px; margin: auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; border-top: 6px solid #e5a00d;">
    <div style="background-color: #ffffff; padding: 24px 32px 0; text-align: center;">
      <p style="margin: 0; color: #282A2D; font-size: 12px; font-weight: 700; letter-spacing: 1.4px; text-transform: uppercase;">${heading}</p>
    </div>
    <div style="padding: 16px 40px 32px; color: #333333;">
      <h2 style="color: #282A2D; font-size: 20px; margin: 0 0 16px; font-weight: 600;">${title}</h2>
      ${bodyHtml}
    </div>
...
export const buildSmtpTestHtml = () => buildPortalEmailHtml({
    ...
    bodyHtml: `
      <p>This confirms Server Portal can send mail with the saved SMTP settings.</p>
      <p>If delivery is set to <strong>admins only</strong>, members will not receive request, issue, or expiry mail until you switch it back to all members.</p>
    `,
