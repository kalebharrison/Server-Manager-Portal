...
import { resolveOwnerContactHref, resolveOwnerInbox } from './email-recipients.js';
...
export const buildSmtpTestHtml = (config = {}) => {
    ...
    return buildPortalEmailHtml({
        ...
        bodyHtml: `
          ...
          <p style="margin:0 0 12px;">Reply to this message to confirm inbound Parse. It should land in the portal — not bounce.</p>
...
export const buildExpiryWarningHtml = ({ user, config, days, escapeHtmlAttr }) => {
    ...
    const name = resolveEmailServerName(config);
    const contactHref = escapeHtmlAttr(resolveOwnerContactHref(config));
    return buildPortalEmailHtml({
        heading: name,
        ...
        bodyHtml: `
          ...
          ${contactHref ? buildEmailButton(contactHref, 'Request Extension') : ''}
        `,
    });
};
...
export const buildAccessExpiredHtml = ({ user, config, escapeHtmlAttr }) => {
    const locale = resolveLocale(user);
    const name = resolveEmailServerName(config);
    const contact = escapeHtmlAttr(resolveOwnerInbox(config));
    return buildPortalEmailHtml({
        heading: name,
