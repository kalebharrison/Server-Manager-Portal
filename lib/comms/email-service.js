...
import { emailSubject, toEmailTitleCase } from './email-identity.js';
...
export const createEmailService = ({
    ...
    const checkAndSendNotifications = async (config) => {
        ...
        for (const user of users) {
            ...
            const subject = emailSubject(config, toEmailTitleCase(`Your shared access expires in ${days} day${days === 1 ? '' : 's'}`));
    ...
    const sendExpiryEmail = async (config, user) => {
        ...
        const subject = emailSubject(config, toEmailTitleCase('Your shared access has expired'));
    ...
    const sendAdjustmentEmail = async (config, user) => {
        ...
        const subject = emailSubject(config, toEmailTitleCase('Your access has been updated'));
    ...
    const sendMemberNotice = async (config, user, { type, uniqueKey, subject, html, replyTo } = {}) => {
        const deliveryEmail = getDeliveryEmail(user);
        if (!deliveryEmail) return false;
        if (uniqueKey) {
        ...
        try {
            const sent = await sendEmail(config, deliveryEmail, subject, html, null, { replyTo });
