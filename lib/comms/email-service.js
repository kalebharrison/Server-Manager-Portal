...
import { checkHasLogo, createEmailSendHelpers } from './email-send.js';
...
export const createEmailService = ({
    ...
    const checkAndSendNotifications = async (config) => {
        ...
        const hasLogo = await checkHasLogo();
        ...
        for (const user of users) {
            ...
            const html = buildExpiryWarningHtml({ user, config, days, hasLogo, escapeHtmlAttr });
    ...
    const sendExpiryEmail = async (config, user, hasLogo) => {
        ...
        const html = buildAccessExpiredHtml({ user, config, hasLogo, escapeHtmlAttr });
    ...
    const sendAdjustmentEmail = async (config, user, hasLogo) => {
        ...
        const html = buildAccessAdjustmentHtml({ user, hasLogo, escapeHtmlAttr, getDaysUntilExpiry });
