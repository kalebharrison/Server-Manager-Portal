import { inboundWebhookAuthorized } from './email-inbound-auth.js';
...
export const registerMailjetInboundRoutes = ({
    ...
    const handleInbound = async (req, res) => {
        try {
            ...
            const webhookToken = String(config.inboundWebhookToken || '').trim();
            if (!inboundRepliesAreEnabled(config) || !String(config.mailjetInboundSecret || '').trim() || !webhookToken) {
                log('[inbound-email] rejected: inbound replies are not enabled');
                return res.status(503).json({ error: 'Inbound email is not enabled' });
            }
            if (!inboundWebhookAuthorized(req, webhookToken)) {
                log('[inbound-email] rejected: webhook token mismatch');
