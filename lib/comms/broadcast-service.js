...
import { wrapBroadcastEmailHtml } from './email-templates.js';
...
const selectBroadcastRecipients = (users, recipientFilter, selectedUserIds) => {
    ...
    return users.filter((user) => {
        if (!getDeliveryEmail(user)) return false;
...
export const createBroadcastService = ({
    ...
    const startBroadcast = async ({ subject, body, recipientFilter, selectedUserIds }) => {
        ...
        (async () => {
            ...
            for (const user of targetUsers) {
                try {
                    const html = wrapBroadcastEmailHtml({
                        subject,
                        bodyHtml: safeBody,
                        config,
                    });
                    const email = getDeliveryEmail(user);
                    await sendEmail(config, email, subject, html, bulkTransporter);
                    await new Promise((resolve) => setTimeout(resolve, 2000));
                } catch (e) {
                    log(`Broadcast failed to ${getDeliveryEmail(user) || user.email}: ${e.message}`);
                }
            }
    ...
    const sendTestBroadcast = async ({ subject, body, adminEmail }) => {
        ...
        await sendEmail(config, adminEmail, subject, wrapBroadcastEmailHtml({
            subject,
            bodyHtml: sanitizeBroadcastHtml(body),
            config,
        }));
    };
