...
export const inboundRepliesAreEnabled = (config = {}) => config.inboundRepliesEnabled !== false;

export const normalizeInboundReplyDomain = (value = '') => {
    const host = String(value || '')
        ...
        .split(':')[0]
        .replace(/\.$/, '');
    if (!host || !/^[a-z0-9][a-z0-9.-]*\.[a-z]{2,}$/i.test(host)) return '';
    return host;
};

export const suggestInboundReplyDomain = (smtpFrom = '') => {
    const from = extractEmailAddress(smtpFrom);
    const host = (from.split('@')[1] || '').toLowerCase();
    if (!host) return '';
    return host.startsWith('reply.') ? host : `reply.${host}`;
};

export const resolveInboundReplyDomain = (config = {}) => {
    const explicit = normalizeInboundReplyDomain(config.inboundReplyDomain);
    if (explicit) return explicit;
    return suggestInboundReplyDomain(config.smtpFrom || '');
};
...
export const buildInboundReplyTo = (config = {}, { kind, id } = {}) => {
    if (!inboundRepliesAreEnabled(config)) return '';
