...
import { listDeliverableMembers } from './email-recipients.js';
...
export const registerCommunicationRoutes = ({
    ...
    app.post('/api/users/broadcast/test', requireAdmin, adminSensitiveRateLimit, async (req, res) => {
        ...
        try {
            const adminEmail = getDeliveryEmail(req.user) || req.user.email;
            await sendTestBroadcast({ subject, body, adminEmail });
    ...
    app.post('/api/newsletter/test', requireAdmin, adminSensitiveRateLimit, async (req, res) => {
        try {
            ...
            await sendTestNewsletter(config, { to: getDeliveryEmail(req.user) || req.user.email });
    ...
    app.post('/api/newsletter/send-now', requireAdmin, adminSensitiveRateLimit, async (req, res) => {
        try {
            ...
            const validUsers = listDeliverableMembers(users);
    ...
    app.post('/api/announcements/push', requireAdmin, adminSensitiveRateLimit, async (req, res) => {
        ...
        try {
            ...
            if (shouldSendEmail && text) {
                ...
                const users = await loadFile(USERS_PATH, []);
                const activeUsers = listDeliverableMembers(users, (user) => user.plexAccessStatus === 'active');
                if (activeUsers.length > 0) {
                    ...
                    (async () => {
                        ...
                        for (let i = 0; i < activeUsers.length; i++) {
                            const { email } = activeUsers[i];
                            const html = buildAnnouncementEmailHtml({
                                text: escapeHtmlAttr(String(text || '')),
                                config,
                            });
                            try {
                                await sendEmail(config, email, emailSubject(config, 'Server Announcement'), html);
                                sentCount++;
                            } catch (emailErr) {
                                log(`Failed to send announcement email to ${email}`);
                            }
