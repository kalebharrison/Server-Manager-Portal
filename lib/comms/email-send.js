...
export const checkHasLogo = async () => {
...
export const buildInlineLogoAttachment = async () => {
    if (!(await checkHasLogo())) return null;
    return {
        filename: 'logo.png',
        path: path.join(process.cwd(), 'static', 'logo.png'),
        cid: 'logo',
    };
};
...
export const createEmailSendHelpers = ({
    ...
    const sendEmail = async (config, to, subject, html, customTransporter = null, options = {}) => {
        ...
        const logo = shouldAttachInlineLogo(html) ? await buildInlineLogoAttachment() : null;
