...
export const createDefaultStatusConfig = (config = {}) => {
    ...
    addService('lidarr', 'Lidarr', config.lidarrUrl, 'downloads', 'Music automation');
...
export const reconcileBuiltInStatusConfig = (statusConfig = {}, appConfig = {}) => {
    ...
    const lidarrUrl = String(appConfig.lidarrUrl || '').trim();
    ...
    next.services = next.services.filter((service) => service?.id !== 'plex' || plexConfigured).map((service) => {
        ...
        if (service?.id === 'lidarr' && lidarrUrl) {
            return { ...service, url: lidarrUrl };
        }
        return service;
    });
    ...
    if (lidarrUrl && !next.services.some((service) => service?.id === 'lidarr')) {
        next.services.push({ id: 'lidarr', name: 'Lidarr', url: lidarrUrl, type: 'web', groupId: 'downloads', description: 'Music automation' });
    }
    return next;
};

export const createPublicStatusPayload = (statusConfig = {}, healthData = {}) => {
    const requestServiceIds = new Set(['seerr', 'jellyseerr', 'overseerr']);
    const publicServiceName = (service = {}) => {
        ...
        if (service.id === 'lidarr' && (!name || name.toLowerCase() === 'lidarr')) return 'Music Automation';
        if (requestServiceIds.has(service.id) && (!name || ['seerr', 'jellyseerr', 'overseerr'].includes(name.toLowerCase()))) return 'Request Service';
        return name || 'Service';
    };
    const publicServiceDescription = (service = {}) => {
        ...
        if (service.id === 'lidarr' && (!description || description.toLowerCase() === 'music automation')) return 'Music release automation';
