...
export const createPublicStatusPayload = (statusConfig = {}, healthData = {}) => {
    const publicServiceName = (service = {}) => {
        if (service.id === 'sonarr') return 'TV Automation';
        if (service.id === 'radarr') return 'Movie Automation';
        return service.name;
    };
    const publicServiceDescription = (service = {}) => {
        if (service.id === 'sonarr') return 'TV release automation';
        if (service.id === 'radarr') return 'Movie release automation';
