...
const isShowCompleteFromStatistics = (series = {}) => {
    ...
    // Prefer per-season monitored counts so specials (season 0) and unmonitored
    // extras in totalEpisodeCount do not block "complete".
    ...
    for (const season of seasons) {
        ...
        if (sExpected > 0 && sFiles < sExpected) return false;
        if (Number.isFinite(sPercent) && sPercent < 100) return false;
    }
    if (sawMonitoredMain) return true;

    // Fallback when season rows are missing: monitored series % only.
    ...
    if (monitored > 0 && episodeFileCount >= monitored) return true;
    return false;
};
...
export const applySonarrLibraryStatusToTvDetails = (details, sonarrStatus) => {
    ...
    for (const season of sonarrStatus.seasons || []) {
        ...
        const nextStatus = season.complete ? MEDIA_STATUS_AVAILABLE : MEDIA_STATUS_PARTIAL;
        const existing = bySeason.get(seasonNumber);
        if (existing) {
            if (season.complete || Number(existing.status) !== MEDIA_STATUS_AVAILABLE) {
    ...
    if (sonarrStatus.showComplete && !sonarrStatus.hasActiveDownloads) {
        mediaInfo.status = MEDIA_STATUS_AVAILABLE;
    } else if (mainSeasonsHaveFiles(sonarrStatus.seasons)) {
        mediaInfo.status = MEDIA_STATUS_PARTIAL;
    }
