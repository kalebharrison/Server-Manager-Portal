...
export const registerMediaStackRoutes = ({
    ...
    const loadQueue = async (isAdmin = false) => {
        const config = await loadFile(configPath, {});
        ...
        return withCache(cacheKey, Math.min(cacheRefreshMs(config), 30_000), async () => {
    ...
    app.get('/api/media-stack/queue', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            res.json(await loadQueue(isAdmin));
        } catch {
    ...
    const startCacheWarmer = () => {
        ...
        const warm = (config) => Promise.all([
            ...
            // Keep Discover "On the way" and Status queues warm for instant page loads.
            loadQueue(true),
            loadQueue(false),
        ]);
