import { isAlwaysBlockedIp, normalizeExternalBaseUrl } from '../http/network-policy.js';
...
const sanitizeRequestAppInternalUrl = (raw) => {
    const value = String(raw || '').trim();
    if (!value) return '';
    try {
        const normalized = normalizeExternalBaseUrl(value, { allowPrivate: true, allowHttp: true });
        const host = new URL(normalized).hostname;
        if (isAlwaysBlockedIp(host)) return '';
        return normalized;
    } catch {
        console.error('WARN: REQUEST_APP_INTERNAL_URL is invalid or points at a blocked address; ignoring.');
        return '';
    }
};
...
export const loadPortalEnv = () => {
    ...
    const REQUEST_APP_INTERNAL_URL = sanitizeRequestAppInternalUrl(process.env.REQUEST_APP_INTERNAL_URL || '');
