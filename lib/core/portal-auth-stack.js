...
import { createSetupAccess, getSocketPeerIp } from '../http/setup-access.js';
import { loadFile as loadJsonFile, loadFileShared as loadJsonFileShared, saveFile as saveJsonFile } from '../core/json-file-store.js';
...
export const createPortalAuthFoundation = ({
    ...
    // Auth/setup quotas key on the TCP peer so forged X-Forwarded-For cannot reset them.
    const peerIp = (req) => getSocketPeerIp(req);
    const authRateLimit = createRateLimiter({ windowMs: 15 * 60 * 1000, maxRequests: 10, getClientIp: peerIp });
    const authCallbackRateLimit = createRateLimiter({ windowMs: 15 * 60 * 1000, maxRequests: 40, getClientIp: peerIp });
    const jellyfinQuickConnectPollRateLimit = createRateLimiter({ windowMs: 5 * 60 * 1000, maxRequests: 140, getClientIp: peerIp });
    const publicReadRateLimit = createRateLimiter({ windowMs: 60 * 1000, maxRequests: 120, getClientIp: peerIp });
    const memberApiRateLimit = createRateLimiter({ windowMs: 60 * 1000, maxRequests: 180, getClientIp: peerIp });
    const setupRateLimit = createRateLimiter({ windowMs: 15 * 60 * 1000, maxRequests: 30, getClientIp: peerIp });
    const adminSensitiveRateLimit = createRateLimiter({ windowMs: 15 * 60 * 1000, maxRequests: 20, getClientIp: peerIp });
    const integrityTriggerRateLimit = createRateLimiter({ windowMs: 60_000, maxRequests: 20, getClientIp: peerIp });
    ...
    return {
        ...
        sanitizeIntegrationUrlAsync,
        loadFile,
        loadFileShared: loadJsonFileShared,
        saveFile,
        secureConfigAtRest,
        appendAuditLog,
        rememberDeletedUser,
