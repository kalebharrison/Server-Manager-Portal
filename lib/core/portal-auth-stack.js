...
export const createPortalAuthFoundation = ({
    ...
    const memberApiRateLimit = createRateLimiter({ windowMs: 60 * 1000, maxRequests: 180 });
    const setupRateLimit = createRateLimiter({ windowMs: 15 * 60 * 1000, maxRequests: 30, getClientIp: peerIp });
    const adminSensitiveRateLimit = createRateLimiter({ windowMs: 15 * 60 * 1000, maxRequests: 20 });
