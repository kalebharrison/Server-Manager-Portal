...
import { ensureIntegrityWebhookAuth, syncIntegrityHooksToArr } from '../upgrader/arr-integrity-hook-sync.js';
import { recordIntegrityHookSync } from '../upgrader/qc-arr-optimize.js';
...
export const createPortalRuntime = ({
    ...
    const startPortalService = async () => {
        ...
        const ensuredHooks = ensureIntegrityWebhookAuth(config);
        if (ensuredHooks.changed) {
            config = ensuredHooks.config;
            configSecretsChanged = true;
            log('[integrity] Generated Arr hook credentials — syncing Connect webhooks via Arr API');
        }
        if (configSecretsChanged) {
            await saveFile(CONFIG_PATH, config);
        }
        ...
        const syncPortalRequestStatuses = async ({ forceRefresh = false } = {}) => {
            ...
            try {
                ...
                await service.syncRequestStatuses({ forceRefresh, log });
            } catch (error) {
        ...
        // Expose for Arr import webhooks (Processing → Available without waiting for the 5m poll).
        app.locals.syncPortalRequestStatuses = syncPortalRequestStatuses;
        ...
        void syncIntegrityHooksToArr(backupConfig, {
            fetchArr: fetchArrInstance,
            resolveUrl: services.resolveIntegrationUrlForFetch,
            log,
            test: false,
        }).then(async (result) => {
            if (result.error) log(`[integrity] Arr hook sync skipped: ${result.error}`);
            else if (!result.ok) log('[integrity] Arr hook sync finished with failures');
            else await recordIntegrityHookSync(backupConfig, result, {
                loadPrefs: upgrader.loadPrefs,
                savePrefs: upgrader.savePrefs,
            });
        }).catch((error) => log(`[integrity] Arr hook sync failed: ${error.message}`));
