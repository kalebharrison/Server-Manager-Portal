...
export const registerMediaIssueRoutes = ({
    ...
    const plexRatingKeyFromUrl = (value) => {
        const match = String(value || '').match(/[?&]key=([^&]+)/);
        if (!match) return null;
        try {
            return decodeURIComponent(match[1]).match(/\/library\/metadata\/(\d+)/)?.[1] || null;
        } catch {
            return null;
        }
    };
        ...
        return {
            ...
            issues: await Promise.all(nodes.map(async (report) => {
                ...
                return {
                    ...
                    issueType: 4,
                    title: metadata?.grandparentTitle || metadata?.parentTitle || metadata?.title || 'Plex reported issue',
                    mediaType,
                    ratingKey,
