...
export const registerMediaIssueRoutes = ({
    ...
    app.get('/api/media-issues', requireAuth, requireMember, async (req, res) => {
        ...
        const visibleLocal = localIssues.map((issue) => ({
            ...issue,
            canComment: isAdmin || (issue.source === 'portal' && issue.reporterId === reporterId(req.user)),
            commentCount: issue.source === 'portal' ? issue.comments?.length || 0 : issue.commentCount || 0,
