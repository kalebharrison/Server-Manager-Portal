...
export const createPlexDashboardService = ({
    ...
    const getRecentData = async (config, uri, limit = 100) => {
        ...
        if (matches) {
            // Await stale rebuild so Discover Movies/Series don't keep showing yesterday's adds.
            if (Date.now() - Number(current.generatedAt || 0) >= cacheRefreshMs(config)) {
                const refreshed = await refresh({ force: true });
                return sliceRecentData(refreshed?.data || current.data || emptyRecentData(), limit);
            }
