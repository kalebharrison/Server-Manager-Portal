...
export const registerPlexRoutes = ({
    ...
    const memberLimit = memberApiRateLimit || ((req, res, next) => next());

    app.get('/api/plex/image', requireAuth, requireMember, memberLimit, async (req, res) => {
    ...
    app.get('/api/plex/library', requireAuth, requireMember, memberLimit, async (req, res) => {
        try {
            const { config, uri } = await loadPlexContext();
            res.json(await plexDashboardService.getRecentData(config, uri, readLimit(req)));
        } catch (error) {
            res.status(503).json({ error: 'Failed to fetch library data' });
        }
    });

    app.get('/api/plex/search', requireAuth, requireMember, memberLimit, async (req, res) => {
        const query = String(req.query.query || '').trim();
        if (query.length < 2) return res.json({ results: [] });
        const perSection = Math.max(5, Math.min(parseInt(String(req.query.limit || '20'), 10) || 20, 30));
        try {
            const { config, uri } = await loadPlexContext();
            ...
            const payloads = await Promise.all(sections.map(async (section) => {
                const params = new URLSearchParams({
                    ...
                    'X-Plex-Container-Size': String(perSection),
            ...
            res.status(503).json({ error: 'Failed to search Plex library' });
        }
    });

    app.get('/api/plex/sessions', requireAuth, requireMember, memberLimit, async (req, res) => {
        try {
            const { config, uri } = await loadPlexContext();
    ...
    app.get('/api/plex/dashboard', requireAuth, requireMember, memberLimit, async (req, res) => {
        try {
            const { config, uri } = await loadPlexContext();
