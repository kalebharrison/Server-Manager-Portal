...
export const buildCurrentUserPayload = async ({
    ...
    const navOrder = (config.navOrder || ['home', 'discover', 'users', 'status', 'analytics', 'mediastack', 'request', 'scanner', 'upgrader', 'settings', 'logout'])
        .filter((key) => key !== 'maintenance');
    if (!navOrder.includes('scanner')) {
        const settingsIndex = navOrder.indexOf('settings');
        navOrder.splice(settingsIndex >= 0 ? settingsIndex : navOrder.length, 0, 'scanner');
    }
    if (!navOrder.includes('upgrader')) {
        const settingsIndex = navOrder.indexOf('settings');
        navOrder.splice(settingsIndex >= 0 ? settingsIndex : navOrder.length, 0, 'upgrader');
    }
    try {
    ...
    const portalRequestEngine = isPortalRequestEngineEnabled(config);
    const seerrRequestConfigured = !!(requestAppType && requestAppType !== 'none' && requestUrl && requestUrl !== 'https://yourdomain.com');
    const navFeatures = {
        // Feature gate (chat widget, etc.). Portal Discover covers requests — no separate nav item.
        request: portalRequestEngine || seerrRequestConfigured,
        requestNav: !portalRequestEngine && seerrRequestConfigured,
    ...
    return {
        ...
        body: {
            ...
            navHiddenKeys: Array.isArray(config.navHiddenKeys)
                ? config.navHiddenKeys.filter((key) => typeof key === 'string' && key && key !== 'home' && key !== 'settings' && key !== 'logout' && key !== 'preferences')
                : [],
            navFeatures,
