...
export const buildCurrentUserPayload = async ({
    ...
    const requestUrl = config.requestUrl || 'https://yourdomain.com';
    const navOrder = (config.navOrder || ['home', 'discover', 'request', 'users', 'status', 'analytics', 'mediastack', 'upgrader', 'settings', 'logout'])
        .filter((key) => key !== 'maintenance' && key !== 'scanner');
    // Keep Request Content immediately under Discover regardless of saved order.
    {
        const requestIdx = navOrder.indexOf('request');
        if (requestIdx >= 0) navOrder.splice(requestIdx, 1);
        const discoverIdx = navOrder.indexOf('discover');
        if (discoverIdx >= 0) navOrder.splice(discoverIdx + 1, 0, 'request');
        else navOrder.splice(1, 0, 'request');
    }
    ...
    const navFeatures = {
        // Discover (library / in-flight) and Request (requestable) are separate surfaces.
        request: true,
        requestNav: true,
