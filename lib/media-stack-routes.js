        ...
        try {
            ...
            const data = await withCache(cacheKey, 60000, async () => {
                ...
                const [sonarrStatus, sonarrQueueRaw, sonarrHistory, sonarrDisk, sonarrCalendarRaw, radarrStatus, radarrQueueRaw, radarrHistory, radarrDisk, radarrCalendarRaw] = await Promise.all([
                    ...
                    fetchArr(config.sonarrUrl, config.sonarrApiKey, '/api/v3/queue?includeSeries=true&includeEpisode=true'),
                    ...
                    fetchArr(config.radarrUrl, config.radarrApiKey, '/api/v3/queue?includeMovie=true'),
                ...
                // Some Arr versions ignore the queue include flags. Resolve only the few queue
                // records missing their media object so Discover can show a real title and poster.
                const hydrateQueue = async (queue, service) => {
                    ...
                    const hydrated = await Promise.all(records.map(async (record) => {
                        if (record?.[mediaKey]?.title) return record;
                        const media = await readMedia(record?.[idKey]);
                        return media?.title ? { ...record, [mediaKey]: media } : record;
                    }));
