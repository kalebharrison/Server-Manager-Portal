...
export const createArrInstance = ({
    ...
    // Defaults used when portal requests omit advanced routing.
...
export const maskArrInstancesForApi = (instances = [], secretMask = '••••••••') => (
    (Array.isArray(instances) ? instances : []).map((entry) => ({
        ...
        activeProfileId: normalizeOptionalId(entry.activeProfileId),
        activeDirectory: String(entry.activeDirectory || '').trim(),
        activeLanguageProfileId: normalizeOptionalId(entry.activeLanguageProfileId),
        activeAnimeProfileId: normalizeOptionalId(entry.activeAnimeProfileId),
        activeAnimeDirectory: String(entry.activeAnimeDirectory || '').trim(),
        tags: normalizeTagIdList(entry.tags),
        animeTags: normalizeTagIdList(entry.animeTags),
    }))
);
...
export const fetchArrInstance = async (instance, endpoint, {
    ...
    try {
        // resolveIntegrationUrlForFetch is async — awaiting keeps docker hostnames working.
        const safeBaseUrl = String(await Promise.resolve(resolveUrl(instance.url)) || '').trim();
        if (!safeBaseUrl) return { ok: false, status: 0, data: null };
...
/** Merge tag ids onto a Radarr movie / Sonarr series (PUT full entity). */
export const updateArrEntityTags = async (instance, entity, tagIds, {
    resolveUrl = (url) => url,
    fetchImpl = fetch,
} = {}) => {
    if (!entity?.id) return { ok: false, reason: 'Missing ARR entity id' };
    const endpoint = instance?.type === 'radarr'
        ? `/api/v3/movie/${entity.id}`
        : `/api/v3/series/${entity.id}`;
    const tags = Array.isArray(tagIds)
        ? [...new Set(tagIds.map((id) => Number(id)).filter((n) => Number.isFinite(n)))]
        : [];
    const result = await fetchArrInstance(instance, endpoint, {
        resolveUrl,
        fetchImpl,
    ...
    if (!result?.ok) {
        return { ok: false, reason: `Tag update failed (${result?.status || 'unknown'})` };
    }
    return { ok: true, entity: result.data || { ...entity, tags } };
};

/** Monitored non-special season numbers from a Sonarr series / season list. */
export const monitoredSonarrSeasonNumbers = (seasons = []) => (
    [...new Set(
...
/**
 * Prefer season-level searches so Sonarr queries packs (Sxx) for standard series
 * instead of walking MissingEpisodeSearch / per-episode grabs.
 */
export const triggerSonarrSeasonSearches = async (
    instance,
    seriesId,
    seasons = [],
    { resolveUrl = (url) => url, fetchImpl = fetch, timeoutMs = 30000 } = {},
) => {
    const id = Number(seriesId);
    if (!Number.isFinite(id) || id <= 0) return { ok: false, reason: 'Missing Sonarr series id' };

    const seasonNumbers = monitoredSonarrSeasonNumbers(seasons);
    if (!seasonNumbers.length) {
        const result = await fetchArrInstance(instance, '/api/v3/command', {
            resolveUrl,
            fetchImpl,
            method: 'POST',
            body: { name: 'SeriesSearch', seriesId: id },
            timeoutMs,
        });
        if (!result?.ok) return { ok: false, reason: `Search command failed (${result?.status || 'unknown'})` };
        return { ok: true, commandId: result.data?.id || null, seasonNumbers: [] };
    }
    ...
    for (const seasonNumber of seasonNumbers) {
        const result = await fetchArrInstance(instance, '/api/v3/command', {
            ...
            fetchImpl,
            method: 'POST',
            body: { name: 'SeasonSearch', seriesId: id, seasonNumber },
            timeoutMs,
        });
        if (!result?.ok) continue;
        okCount += 1;
        if (result.data?.id != null) commandIds.push(result.data.id);
    }
    if (!okCount) {
        return { ok: false, reason: 'Season search commands failed' };
    }
    return { ok: true, commandIds, seasonNumbers };
};
...
export const triggerArrEntitySearch = async (instance, entity, type, { resolveUrl = (url) => url, fetchImpl = fetch, timeoutMs = 30000 } = {}) => {
    ...
    if (arrType === 'radarr') {
        const result = await fetchArrInstance(instance, '/api/v3/command', {
            ...
            fetchImpl,
            method: 'POST',
            body: { name: 'MoviesSearch', movieIds: [entity.id] },
            timeoutMs,
        });
        if (!result?.ok) return { ok: false, reason: `Search command failed (${result?.status || 'unknown'})` };
    ...
    return triggerSonarrSeasonSearches(instance, entity.id, entity.seasons, {
        resolveUrl,
        fetchImpl,
        timeoutMs,
    });
...
/**
 * Build season monitored flags for Sonarr add/update.
 ...
 * Uses a minimal payload — never POST the raw lookup blob (breaks Sonarr v4 / truncates on abort).
 * @returns {{ ok: boolean, created: boolean, entity?: object, reason?: string }}
 */
export const addSonarrSeries = async (instance, {
    ...
    const claimExistingSeries = (entity) => {
        ...
        void (async () => {
            try {
                ...
                if (search) {
                    await triggerArrEntitySearch(instance, { ...existing, seasons: updatedSeasons }, 'sonarr', {
                        ...fetchOpts,
                        timeoutMs: 10000,
                    });
                }
    ...
    const body = {
        ...
        addOptions: {
            ...
            // Portal fires SeasonSearch after add so Sonarr prefers packs over episode walks.
    ...
    if (search !== false && entity?.id) {
        void triggerSonarrSeasonSearches(
            instance,
            entity.id,
            seasonList.length ? seasonList : entity.seasons,
            { ...fetchOpts, timeoutMs: 10000 },
        );
    }
    return { ok: true, created: true, entity };
};
