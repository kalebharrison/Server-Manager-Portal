import { normalizeArrConfig, sanitizeArrInstances } from '../media-stack/arr-instances.js';
import { normalizeCacheRefreshMinutes } from '../cache/cache-refresh.js';
import { buildBrandingFields } from './config-settings-branding-fields.js';
import {
    createResolveConfigIntegrationUrl,
    createResolveSecret,
    resolveSafeIntegrationUrls,
} from './config-settings-resolvers.js';
import { buildDiscordConfigFields } from '../discord/discord-config.js';
import { buildPublicContactUrlFields } from './config-settings-url-fields.js';
import { normalizeInboundReplyDomain } from '../comms/email-inbound.js';
import { parseHuntIndexerDeny } from '../upgrader/upgrader-hunt-indexers.js';
import { normalizeIntegritySchedulePolicy } from '../upgrader/qc-integrity-schedule-policy.js';
import { normalizeIntegrityAutoReplaceByCategory } from '../upgrader/qc-integrity-severity.js';

/** Keys left behind by retired external integrations. */
export const RETIRED_REQUEST_APP_KEYS = [
    'requestAppType',
    'requestAppUrl',
    'requestAppApiKey',
    'requestAppFetchUrl',
    'requestAppMembershipSync',
    'ombiUrl',
    'ombiApiKey',
    'contactWhatsApp',
];

export const buildConfigFromPostBody = async ({
    body,
    existingConfig,
    secretMask,
    normalizePlexToken,
    sanitizeIntegrationUrl,
    normalizeSectionLayout,
}) => {
    const {
        token, mediaServerType, serverIdentifier, checkIntervalMinutes,
        smtpEnabled, smtpAdminOnly, smtpHost, smtpPort, smtpUser, smtpPass, smtpFrom, smtpSecure, emailDaysBefore,
        inboundRepliesEnabled, inboundReplyDomain,
        newsletterFrequency, newsletterDay, publicDomain, requestUrl, contactUrl, contactEmail,
        sonarrApiKey, radarrApiKey, lidarrApiKey, arrInstances, tautulliApiKey, jellystatApiKey,
        jellyfinApiKey,
        inactiveCleanupEnabled, inactiveCleanupDays,
        primaryColor, customLogoUrl, brandingTheme, backgroundImageUrl, useScrollRevealAnimations, useCinematicLoading, useBrandedSkeleton, useTrendingSlideshow, trendingSlideshowInterval, tmdbApiKey, tvdbApiKey, tvdbPin, cacheRefreshMinutes, referralEnabled, referralTrialDays, referralRewardDays, announcement, navOrder, navHiddenKeys, hideStreamUsers, defaultLibraryIds, use24HourClock, showPosterQualityBadges,
        publicStatusEnabled, showLoginServerStats, autoBackupEnabled, autoBackupIntervalDays, autoBackupRetentionCount, dashboardLayout,
        useTrendingSlideshowOnLogin,
        upgraderEnabled, upgraderAutomationEnabled, upgraderProfileMap, upgraderDefaultPreset,
        upgraderMinSizeGB, upgraderMaxActionsPerHour, upgraderMaxDownloadsPerLibrary, upgraderMinScoreDelta,
        upgraderHuntIntensity,
        upgraderPreferences, upgraderPreferenceWeights,
        upgraderDefaultSort, upgraderDrawerPosition,
        upgraderHuntMissingEpisodes, upgraderHuntAvailableMovies, upgraderHuntIndexerDeny,
        qcCleanupAutomationEnabled, qcCleanupAggression, qcMetaDlMinutes, qcStalledHours, qcSlowDownloadFloorKbps, qcSlowDownloadMinAgeHours, qcCompletedNotImportingMinutes, qcOrphanGraceMinutes, qcMaxStrikes,
        qcResearchThrottleHours, qcSnoozeDefaultHours, qcDiscordDigestEnabled,
        qcQbitUrl, qcQbitUsername, qcQbitPassword, qcSabUrl, qcSabApiKey, qcBlockedExtensions,
        qcIntegrityEnabled, qcIntegrityAutomationEnabled, qcIntegrityAutoReplaceByCategory, qcIntegrityRequireAudio,
        qcIntegrityIncludeMusic, qcIntegrityXxhashEnabled,
        qcTrimEnabled, qcTrimDryRun, qcTrimLanguages, qcTrimKeepNativeAudio,
        qcTrimStripCommentary, qcTrimStripLowerChannels, qcTrimDeleteMetadataTitle,
        qcIntegrityPathMaps, qcIntegrityMediaRoots, qcIntegrityMaxPerCycle, qcIntegrityConcurrency,
        qcIntegrityPlayabilityConcurrency, qcIntegrityTrimConcurrency,
        qcIntegrityBreakerMaxFindings, qcIntegrityBreakerMaxPercent,
        qcIntegrityPauseWhenSessions, qcIntegrityNightlyHour, qcIntegrityDiscordDigestEnabled,
        qcIntegrityDecodeWindowSec, qcIntegrityDecodeTimeoutMs, qcIntegrityDecodeRetries, qcIntegritySoftDecodeTimeouts,
        qcIntegrityPlexRefreshAfterImport,
        qcIntegritySchedulePolicy,
        qcIntegrityWebhookUsername, qcIntegrityWebhookPassword,
    } = body;

    const normalizedMediaServerType = ['plex', 'jellyfin'].includes(String(mediaServerType || '').toLowerCase())
        ? String(mediaServerType || '').toLowerCase()
        : (existingConfig.mediaServerType || 'plex');
    const normalizedToken = normalizePlexToken(token);
    const normalizedServerIdentifier = String(serverIdentifier).trim();
    const interval = parseInt(checkIntervalMinutes, 10);

    const resolveConfigIntegrationUrl = createResolveConfigIntegrationUrl(sanitizeIntegrationUrl);
    const resolveSecret = createResolveSecret(secretMask);

    let safeSonarrUrl;
    let safeRadarrUrl;
    let safeLidarrUrl;
    let safeTautulliUrl;
    let safeJellystatUrl;
    let safeJellyfinUrl;
    let safePlexServerUrl;
    let safeQbitUrl;
    let safeSabUrl;
    try {
        ({
            safeSonarrUrl,
            safeRadarrUrl,
            safeLidarrUrl,
            safeTautulliUrl,
            safeJellystatUrl,
            safeJellyfinUrl,
            safePlexServerUrl,
        } = await resolveSafeIntegrationUrls({ body, existingConfig, resolveConfigIntegrationUrl }));
        [safeQbitUrl, safeSabUrl] = await Promise.all([
            resolveConfigIntegrationUrl(qcQbitUrl, existingConfig.qcQbitUrl || ''),
            resolveConfigIntegrationUrl(qcSabUrl, existingConfig.qcSabUrl || ''),
        ]);
    } catch (e) {
        throw new Error(`Invalid integration URL: ${e.message}`);
    }

    let safeArrInstances;
    try {
        safeArrInstances = await sanitizeArrInstances(arrInstances, existingConfig, {
            resolveSecret,
            resolveUrl: resolveConfigIntegrationUrl,
        });
    } catch (e) {
        throw new Error(`Invalid automation instance: ${e.message}`);
    }

    const publicContactFields = buildPublicContactUrlFields({
        publicDomain,
        requestUrl,
        contactUrl,
        contactEmail,
        existingConfig,
    });
    const discordFields = buildDiscordConfigFields({
        body,
        existingConfig,
        resolveSecret,
    });
    const brandingFields = buildBrandingFields({
        primaryColor,
        customLogoUrl,
        brandingTheme,
        backgroundImageUrl,
        useScrollRevealAnimations,
        useCinematicLoading,
        useBrandedSkeleton,
        useTrendingSlideshow,
        trendingSlideshowInterval,
        useTrendingSlideshowOnLogin,
        existingConfig,
    });
    const resolvedIntegrityWebhookUsername = (() => {
        if (qcIntegrityWebhookUsername !== undefined) {
            return String(qcIntegrityWebhookUsername || '').trim();
        }
        return String(existingConfig.qcIntegrityWebhookUsername || '').trim();
    })();
    const resolvedIntegrityWebhookPassword = resolveSecret(
        qcIntegrityWebhookPassword,
        existingConfig.qcIntegrityWebhookPassword || '',
    );

    const config = normalizeArrConfig({
        ...existingConfig,
        mediaServerType: normalizedMediaServerType,
        plexToken: normalizedMediaServerType === 'jellyfin' ? resolveSecret(token, existingConfig.plexToken) : resolveSecret(normalizedToken, existingConfig.plexToken),
        serverIdentifier: normalizedMediaServerType === 'jellyfin' ? (normalizedServerIdentifier || existingConfig.serverIdentifier || '') : normalizedServerIdentifier,
        plexServerUrl: safePlexServerUrl || '',
        jellyfinUrl: safeJellyfinUrl,
        jellyfinApiKey: resolveSecret(jellyfinApiKey, existingConfig.jellyfinApiKey),
        checkIntervalMinutes: (interval > 0 ? interval : 60),
        smtpEnabled: smtpEnabled !== undefined ? !!smtpEnabled : existingConfig.smtpEnabled !== false,
        smtpAdminOnly: smtpAdminOnly !== undefined ? !!smtpAdminOnly : !!existingConfig.smtpAdminOnly,
        smtpHost: smtpHost || '',
        smtpPort: parseInt(smtpPort, 10) || 587,
        smtpUser: resolveSecret(smtpUser, existingConfig.smtpUser),
        smtpPass: resolveSecret(smtpPass, existingConfig.smtpPass),
        smtpFrom: smtpFrom || '',
        smtpSecure: !!smtpSecure,
        emailDaysBefore: parseInt(emailDaysBefore, 10) || 7,
        inboundRepliesEnabled: inboundRepliesEnabled !== undefined
            ? !!inboundRepliesEnabled
            : existingConfig.inboundRepliesEnabled !== false,
        inboundReplyDomain: inboundReplyDomain !== undefined
            ? normalizeInboundReplyDomain(inboundReplyDomain)
            : (existingConfig.inboundReplyDomain || ''),
        newsletterFrequency: newsletterFrequency || 'disabled',
        newsletterDay: parseInt(newsletterDay, 10) || 0,
        inactiveCleanupEnabled: !!inactiveCleanupEnabled,
        inactiveCleanupDays: parseInt(inactiveCleanupDays, 10) || 90,
        ...publicContactFields,
        ...discordFields,
        sonarrUrl: safeSonarrUrl,
        sonarrApiKey: resolveSecret(sonarrApiKey, existingConfig.sonarrApiKey),
        radarrUrl: safeRadarrUrl,
        radarrApiKey: resolveSecret(radarrApiKey, existingConfig.radarrApiKey),
        lidarrUrl: safeLidarrUrl,
        lidarrApiKey: resolveSecret(lidarrApiKey, existingConfig.lidarrApiKey),
        arrInstances: safeArrInstances,
        tautulliUrl: safeTautulliUrl,
        tautulliApiKey: resolveSecret(tautulliApiKey, existingConfig.tautulliApiKey),
        jellystatUrl: safeJellystatUrl,
        jellystatApiKey: resolveSecret(jellystatApiKey, existingConfig.jellystatApiKey),
        ...brandingFields,
        tmdbApiKey: resolveSecret(tmdbApiKey, existingConfig.tmdbApiKey),
        tvdbApiKey: resolveSecret(tvdbApiKey, existingConfig.tvdbApiKey),
        tvdbPin: resolveSecret(tvdbPin, existingConfig.tvdbPin),
        cacheRefreshMinutes: normalizeCacheRefreshMinutes(cacheRefreshMinutes ?? existingConfig.cacheRefreshMinutes),
        referralEnabled: !!referralEnabled,
        referralTrialDays: parseInt(referralTrialDays, 10) || 3,
        referralRewardDays: parseInt(referralRewardDays, 10) || 7,
        announcement: String(announcement || '')
            .replace(/<\s*script[\s\S]*?>[\s\S]*?<\s*\/\s*script\s*>/gi, '')
            .replace(/on\w+\s*=/gi, '')
            .slice(0, 2000),
        hideStreamUsers: hideStreamUsers === 'hidden' ? 'hidden' : 'anonymous',
        navOrder: (Array.isArray(navOrder) ? navOrder : existingConfig.navOrder || ['home', 'discover', 'request', 'users', 'status', 'analytics', 'mediastack', 'settings', 'logout'])
            .filter((key) => key !== 'maintenance' && key !== 'scanner'),
        navHiddenKeys: (() => {
            const ALWAYS = new Set(['home', 'settings', 'logout', 'preferences']);
            const incoming = Array.isArray(navHiddenKeys) ? navHiddenKeys : existingConfig.navHiddenKeys;
            if (!Array.isArray(incoming)) return [];
            const seen = new Set();
            const result = [];
            for (const raw of incoming) {
                const key = String(raw || '').trim();
                if (!key || ALWAYS.has(key) || seen.has(key)) continue;
                seen.add(key);
                result.push(key);
            }
            return result;
        })(),
        defaultLibraryIds: Array.isArray(defaultLibraryIds) ? defaultLibraryIds : null,
        use24HourClock: !!use24HourClock,
        // Open self-serve temp access is retired; invite duration still controls timed membership.
        allowTemporaryAccess: false,
        publicStatusEnabled: publicStatusEnabled === true,
        publicStatusExplicitlyConfigured: true,
        showLoginServerStats: showLoginServerStats === true,
        showPosterQualityBadges: showPosterQualityBadges !== false,
        autoBackupEnabled: !!autoBackupEnabled,
        autoBackupIntervalDays: Math.max(1, parseInt(autoBackupIntervalDays, 10) || 2),
        autoBackupRetentionCount: Math.max(1, parseInt(autoBackupRetentionCount, 10) || 10),
        upgraderEnabled: upgraderEnabled !== undefined ? !!upgraderEnabled : !!existingConfig.upgraderEnabled,
        upgraderAutomationEnabled: upgraderAutomationEnabled !== undefined ? !!upgraderAutomationEnabled : !!existingConfig.upgraderAutomationEnabled,
        upgraderHuntMissingEpisodes: upgraderHuntMissingEpisodes !== undefined
            ? !!upgraderHuntMissingEpisodes
            : existingConfig.upgraderHuntMissingEpisodes !== false,
        upgraderHuntAvailableMovies: upgraderHuntAvailableMovies !== undefined
            ? !!upgraderHuntAvailableMovies
            : existingConfig.upgraderHuntAvailableMovies !== false,
        upgraderHuntIndexerDeny: parseHuntIndexerDeny(
            upgraderHuntIndexerDeny !== undefined
                ? upgraderHuntIndexerDeny
                : existingConfig.upgraderHuntIndexerDeny,
        ),
        upgraderProfileMap: upgraderProfileMap && typeof upgraderProfileMap === 'object' ? upgraderProfileMap : (existingConfig.upgraderProfileMap || {}),
        upgraderDefaultPreset: String(upgraderDefaultPreset || existingConfig.upgraderDefaultPreset || 'score_gap'),
        upgraderMinSizeGB: Math.max(0, Number(upgraderMinSizeGB ?? existingConfig.upgraderMinSizeGB ?? 5) || 5),
        upgraderMaxActionsPerHour: Math.max(1, Number(upgraderMaxActionsPerHour ?? existingConfig.upgraderMaxActionsPerHour ?? 25) || 25),
        upgraderMaxDownloadsPerLibrary: Math.max(1, Number(upgraderMaxDownloadsPerLibrary ?? existingConfig.upgraderMaxDownloadsPerLibrary ?? 5) || 5),
        upgraderMinScoreDelta: Math.max(0, Number(upgraderMinScoreDelta ?? existingConfig.upgraderMinScoreDelta ?? 10) || 0),
        upgraderHuntIntensity: ['relaxed', 'balanced', 'aggressive', 'custom'].includes(String(upgraderHuntIntensity || existingConfig.upgraderHuntIntensity || ''))
            ? String(upgraderHuntIntensity || existingConfig.upgraderHuntIntensity)
            : 'balanced',
        upgraderPreferences: {
            preferDolbyVisionHdr: (upgraderPreferences || existingConfig.upgraderPreferences)?.preferDolbyVisionHdr !== false,
            preferAtmos: (upgraderPreferences || existingConfig.upgraderPreferences)?.preferAtmos !== false,
            preferRemux: (upgraderPreferences || existingConfig.upgraderPreferences)?.preferRemux !== false,
            preferSeasonPacks: (upgraderPreferences || existingConfig.upgraderPreferences)?.preferSeasonPacks !== false,
        },
        upgraderPreferenceWeights: {
            dolbyVisionHdr: Number((upgraderPreferenceWeights || existingConfig.upgraderPreferenceWeights)?.dolbyVisionHdr ?? 50) || 0,
            atmos: Number((upgraderPreferenceWeights || existingConfig.upgraderPreferenceWeights)?.atmos ?? 25) || 0,
            remux: Number((upgraderPreferenceWeights || existingConfig.upgraderPreferenceWeights)?.remux ?? 40) || 0,
            seasonPack: Number((upgraderPreferenceWeights || existingConfig.upgraderPreferenceWeights)?.seasonPack ?? 30) || 0,
        },
        upgraderDefaultSort: ['title', 'sizeGB', 'watchCount', 'addedAt', 'score', 'customFormatScore'].includes(String(upgraderDefaultSort || existingConfig.upgraderDefaultSort || 'sizeGB'))
            ? String(upgraderDefaultSort || existingConfig.upgraderDefaultSort || 'sizeGB') : 'sizeGB',
        upgraderDrawerPosition: ['sidebar', 'modal'].includes(String(upgraderDrawerPosition || existingConfig.upgraderDrawerPosition || 'sidebar'))
            ? String(upgraderDrawerPosition || existingConfig.upgraderDrawerPosition || 'sidebar') : 'sidebar',
        qcCleanupAutomationEnabled: qcCleanupAutomationEnabled !== undefined
            ? !!qcCleanupAutomationEnabled
            : !!existingConfig.qcCleanupAutomationEnabled,
        qcCleanupAggression: ['relaxed', 'balanced', 'aggressive', 'custom'].includes(String(qcCleanupAggression || existingConfig.qcCleanupAggression || ''))
            ? String(qcCleanupAggression || existingConfig.qcCleanupAggression)
            : 'balanced',
        qcIntegrityEnabled: qcIntegrityEnabled !== undefined
            ? !!qcIntegrityEnabled
            : !!existingConfig.qcIntegrityEnabled,
        qcIntegrityAutomationEnabled: qcIntegrityAutomationEnabled !== undefined
            ? !!qcIntegrityAutomationEnabled
            : !!existingConfig.qcIntegrityAutomationEnabled,
        qcIntegrityAutoReplaceByCategory: normalizeIntegrityAutoReplaceByCategory(
            qcIntegrityAutoReplaceByCategory !== undefined
                ? qcIntegrityAutoReplaceByCategory
                : existingConfig.qcIntegrityAutoReplaceByCategory,
        ),
        qcIntegrityRequireAudio: qcIntegrityRequireAudio !== undefined
            ? !!qcIntegrityRequireAudio
            : existingConfig.qcIntegrityRequireAudio !== false,
        qcIntegrityIncludeMusic: qcIntegrityIncludeMusic !== undefined
            ? !!qcIntegrityIncludeMusic
            : existingConfig.qcIntegrityIncludeMusic !== false,
        qcIntegrityXxhashEnabled: qcIntegrityXxhashEnabled !== undefined
            ? !!qcIntegrityXxhashEnabled
            : !!existingConfig.qcIntegrityXxhashEnabled,
        qcTrimEnabled: qcTrimEnabled !== undefined
            ? !!qcTrimEnabled
            : !!existingConfig.qcTrimEnabled,
        qcTrimDryRun: qcTrimDryRun !== undefined
            ? !!qcTrimDryRun
            : existingConfig.qcTrimDryRun !== false,
        qcTrimLanguages: String(
            qcTrimLanguages !== undefined ? qcTrimLanguages : (existingConfig.qcTrimLanguages || 'eng'),
        ).trim() || 'eng',
        qcTrimKeepNativeAudio: qcTrimKeepNativeAudio !== undefined
            ? !!qcTrimKeepNativeAudio
            : existingConfig.qcTrimKeepNativeAudio !== false,
        qcTrimStripCommentary: qcTrimStripCommentary !== undefined
            ? !!qcTrimStripCommentary
            : existingConfig.qcTrimStripCommentary !== false,
        qcTrimStripLowerChannels: qcTrimStripLowerChannels !== undefined
            ? !!qcTrimStripLowerChannels
            : existingConfig.qcTrimStripLowerChannels !== false,
        qcTrimDeleteMetadataTitle: qcTrimDeleteMetadataTitle !== undefined
            ? !!qcTrimDeleteMetadataTitle
            : existingConfig.qcTrimDeleteMetadataTitle !== false,
        qcIntegrityPathMaps: Array.isArray(qcIntegrityPathMaps)
            ? qcIntegrityPathMaps
                .map((entry) => ({
                    from: String(entry?.from || entry?.arrPrefix || '').trim(),
                    to: String(entry?.to || entry?.containerPrefix || '').trim(),
                }))
                .filter((entry) => entry.from && entry.to)
            : (Array.isArray(existingConfig.qcIntegrityPathMaps) ? existingConfig.qcIntegrityPathMaps : []),
        qcIntegrityMediaRoots: (() => {
            const incoming = qcIntegrityMediaRoots !== undefined
                ? qcIntegrityMediaRoots
                : existingConfig.qcIntegrityMediaRoots;
            if (Array.isArray(incoming)) {
                return incoming.map((entry) => String(entry || '').trim()).filter(Boolean);
            }
            return String(incoming || '')
                .split(/[\n,]+/)
                .map((entry) => entry.trim())
                .filter(Boolean);
        })(),
        qcIntegrityMaxPerCycle: (() => {
            const raw = Number(qcIntegrityMaxPerCycle ?? existingConfig.qcIntegrityMaxPerCycle ?? 200);
            if (!Number.isFinite(raw) || raw <= 0 || raw === 25) return 200;
            return Math.max(1, raw);
        })(),
        qcIntegrityConcurrency: Math.max(
            1,
            Number(qcIntegrityConcurrency ?? existingConfig.qcIntegrityConcurrency ?? 4) || 4,
        ),
        qcIntegrityPlayabilityConcurrency: Math.max(
            1,
            Number(qcIntegrityPlayabilityConcurrency ?? existingConfig.qcIntegrityPlayabilityConcurrency ?? 1) || 1,
        ),
        qcIntegrityTrimConcurrency: Math.max(
            1,
            Math.min(2, Number(qcIntegrityTrimConcurrency ?? existingConfig.qcIntegrityTrimConcurrency ?? 1) || 1),
        ),
        qcIntegrityBreakerMaxFindings: Math.max(
            1,
            Number(qcIntegrityBreakerMaxFindings ?? existingConfig.qcIntegrityBreakerMaxFindings ?? 50) || 50,
        ),
        qcIntegrityBreakerMaxPercent: Math.max(
            0.1,
            Number(qcIntegrityBreakerMaxPercent ?? existingConfig.qcIntegrityBreakerMaxPercent ?? 10) || 10,
        ),
        qcIntegrityPauseWhenSessions: Math.max(
            0,
            Number(qcIntegrityPauseWhenSessions ?? existingConfig.qcIntegrityPauseWhenSessions ?? 0) || 0,
        ),
        qcIntegrityNightlyHour: Math.max(
            0,
            Math.min(23, Number(qcIntegrityNightlyHour ?? existingConfig.qcIntegrityNightlyHour ?? 2) || 2),
        ),
        qcIntegritySchedulePolicy: normalizeIntegritySchedulePolicy(
            qcIntegritySchedulePolicy !== undefined
                ? qcIntegritySchedulePolicy
                : existingConfig.qcIntegritySchedulePolicy,
        ),
        qcIntegrityDiscordDigestEnabled: qcIntegrityDiscordDigestEnabled !== undefined
            ? !!qcIntegrityDiscordDigestEnabled
            : !!existingConfig.qcIntegrityDiscordDigestEnabled,
        qcIntegrityDecodeWindowSec: Math.max(
            1,
            Number(qcIntegrityDecodeWindowSec ?? existingConfig.qcIntegrityDecodeWindowSec ?? 10) || 10,
        ),
        qcIntegrityDecodeTimeoutMs: Math.max(
            1000,
            Number(qcIntegrityDecodeTimeoutMs ?? existingConfig.qcIntegrityDecodeTimeoutMs ?? 90000) || 90000,
        ),
        qcIntegrityDecodeRetries: Math.max(
            0,
            Number(qcIntegrityDecodeRetries ?? existingConfig.qcIntegrityDecodeRetries ?? 2) || 0,
        ),
        qcIntegritySoftDecodeTimeouts: qcIntegritySoftDecodeTimeouts !== undefined
            ? !!qcIntegritySoftDecodeTimeouts
            : existingConfig.qcIntegritySoftDecodeTimeouts !== false,
        qcIntegrityPlexRefreshAfterImport: qcIntegrityPlexRefreshAfterImport !== undefined
            ? !!qcIntegrityPlexRefreshAfterImport
            : existingConfig.qcIntegrityPlexRefreshAfterImport !== false,
        qcIntegrityWebhookUsername: resolvedIntegrityWebhookUsername,
        qcIntegrityWebhookPassword: resolvedIntegrityWebhookPassword,
        qcMetaDlMinutes: Math.max(1, Number(qcMetaDlMinutes ?? existingConfig.qcMetaDlMinutes ?? 20) || 20),
        qcStalledHours: Math.max(1, Number(qcStalledHours ?? existingConfig.qcStalledHours ?? 2) || 2),
        qcSlowDownloadFloorKbps: Math.max(
            0,
            Number(qcSlowDownloadFloorKbps ?? existingConfig.qcSlowDownloadFloorKbps ?? 100) || 0,
        ),
        qcSlowDownloadMinAgeHours: Math.max(
            0,
            Number(qcSlowDownloadMinAgeHours ?? existingConfig.qcSlowDownloadMinAgeHours ?? 6) || 0,
        ),
        qcCompletedNotImportingMinutes: Math.max(
            1,
            Number(qcCompletedNotImportingMinutes ?? existingConfig.qcCompletedNotImportingMinutes ?? 90) || 90,
        ),
        qcOrphanGraceMinutes: Math.max(0, Number(qcOrphanGraceMinutes ?? existingConfig.qcOrphanGraceMinutes ?? 15) || 0),
        qcMaxStrikes: Math.max(1, Number(qcMaxStrikes ?? existingConfig.qcMaxStrikes ?? 3) || 3),
        qcResearchThrottleHours: Math.max(
            1,
            Number(qcResearchThrottleHours ?? existingConfig.qcResearchThrottleHours ?? 24) || 24,
        ),
        qcSnoozeDefaultHours: Math.max(1, Number(qcSnoozeDefaultHours ?? existingConfig.qcSnoozeDefaultHours ?? 24) || 24),
        qcDiscordDigestEnabled: qcDiscordDigestEnabled !== undefined
            ? !!qcDiscordDigestEnabled
            : !!existingConfig.qcDiscordDigestEnabled,
        qcQbitUrl: safeQbitUrl || '',
        qcQbitUsername: String(qcQbitUsername ?? existingConfig.qcQbitUsername ?? '').trim(),
        qcQbitPassword: resolveSecret(qcQbitPassword, existingConfig.qcQbitPassword),
        qcSabUrl: safeSabUrl || '',
        qcSabApiKey: resolveSecret(qcSabApiKey, existingConfig.qcSabApiKey),
        qcBlockedExtensions: Array.isArray(qcBlockedExtensions)
            ? [...new Set(qcBlockedExtensions.map((entry) => String(entry || '').trim().replace(/^\*\./, '').replace(/^\./, '').toLowerCase()).filter(Boolean))].sort()
            : (Array.isArray(existingConfig.qcBlockedExtensions) ? existingConfig.qcBlockedExtensions : []),
        dashboardLayout: ('dashboardLayout' in body)
            ? normalizeSectionLayout(body.dashboardLayout)
            : normalizeSectionLayout(existingConfig.dashboardLayout),
    });

    // Drop retired Cleaner / Library Maintenance flag if present on older configs.
    delete config.maintenanceExperimentalEnabled;
    delete config.scannerEnabled;
    delete config.scannerHomeWidgetEnabled;
    delete config.scannerWebhooksVisible;
    delete config.scannerManualPathVisible;
    delete config.scanner;
    // Retired request-app integration keys must not survive settings writes.
    for (const key of RETIRED_REQUEST_APP_KEYS) delete config[key];

    return {
        config,
        normalizedMediaServerType,
        normalizedToken,
        normalizedServerIdentifier,
    };
};
