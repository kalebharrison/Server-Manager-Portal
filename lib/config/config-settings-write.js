...
import { normalizeDiscoverySource, normalizeRequestEngine } from '../discovery-settings.js';
import {
...
export const buildConfigFromPostBody = async ({
    ...
    const config = normalizeArrConfig({
        ...
        requestEngine: normalizeRequestEngine(requestEngine ?? existingConfig.requestEngine),
        discoverySource: normalizeDiscoverySource(discoverySource ?? existingConfig.discoverySource),
        ombiUrl: safeOmbiUrl,
        ...
        showPosterQualityBadges: showPosterQualityBadges === true,
    ...
    // Sticky override must not survive settings writes / restores (SSRF footgun).
    delete config.requestAppFetchUrl;
