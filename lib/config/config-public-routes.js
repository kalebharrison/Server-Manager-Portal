...
import { buildPublicDiscordConfig } from '../discord/discord-config.js';
...
const EMPTY_RELEASE_NOTES = {
    ...
    title: "What's new",
    sections: [],
    changelogUrl: 'https://github.com/kalebharrison/Server-Manager-Portal/blob/beta/docs/CHANGELOG.md',
};
...
export const registerPublicConfigRoutes = ({
    ...
    const buildPublicConfig = async () => {
        ...
        return {
            ...
            // Open temp-access registration is retired; keep the field false for older clients.
            allowTemporaryAccess: false,
            ...
            showPosterQualityBadges: config.showPosterQualityBadges === true,
            requestEngine: config.requestEngine === 'portal' ? 'portal' : 'seerr',
            discoverySource: config.discoverySource === 'tmdb' ? 'tmdb' : 'seerr',
            ...
            ...buildPublicDiscordConfig(config),
        };
    };

    let publicConfigCache = { data: null, expiresAt: 0, inFlight: null };

    app.get('/api/release-notes', publicReadRateLimit, async (_req, res) => {
        try {
    ...
    app.get('/api/config/public', publicReadRateLimit, async (_req, res) => {
        try {
            // Ignore ?refresh/?force — this endpoint is anonymous and must stay cache-bound.
            if (publicConfigCache.data && publicConfigCache.expiresAt > Date.now()) {
                res.setHeader('Cache-Control', 'private, max-age=5');
                return res.json(publicConfigCache.data);
            }

            if (publicConfigCache.inFlight) {
                res.setHeader('Cache-Control', 'private, max-age=5');
