...
export const runIntegrationTest = async ({
    ...
    const urlOpts = { secretMask, sanitizeIntegrationUrl, resolveIntegrationUrlForFetch };
    ...
    if (type === 'jellyfin') {
        const url = resolveIntegrationUrlForFetch(resolveTestCredential(jellyfinUrl, stored.jellyfinUrl, secretMask));
    ...
    if (type === 'sonarr') {
        ...
        const url = resolveIntegrationUrlForTest(sonarrUrl, instance?.url || stored.sonarrUrl, urlOpts);
        const apiKey = resolveTestCredential(sonarrApiKey, instance?.apiKey || stored.sonarrApiKey, secretMask);
        if (!url || !apiKey) return { status: 400, error: 'Sonarr URL and API key are required.' };
        ...
        return { ok: true, message: `Sonarr v${data.version || '?'} connected`, details: { version: data.version, appName: data.appName } };
    }

    if (type === 'radarr') {
        const instance = getArrInstance(stored, instanceId);
        const url = resolveIntegrationUrlForTest(radarrUrl, instance?.url || stored.radarrUrl, urlOpts);
        const apiKey = resolveTestCredential(radarrApiKey, instance?.apiKey || stored.radarrApiKey, secretMask);
        if (!url || !apiKey) return { status: 400, error: 'Radarr URL and API key are required.' };
        const statusRes = await fetchWithTimeout(`${url}/api/v3/system/status`, {
            headers: { 'X-Api-Key': apiKey, Accept: 'application/json' },
        }, 12000);
        if (!statusRes.ok) throw new Error(`Radarr returned HTTP ${statusRes.status}`);
        const data = await statusRes.json();
        return { ok: true, message: `Radarr v${data.version || '?'} connected`, details: { version: data.version, appName: data.appName } };
    }

    if (type === 'lidarr') {
        const instance = getArrInstance(stored, instanceId);
        const url = resolveIntegrationUrlForTest(lidarrUrl, instance?.url || stored.lidarrUrl, urlOpts);
        const apiKey = resolveTestCredential(lidarrApiKey, instance?.apiKey || stored.lidarrApiKey, secretMask);
        if (!url || !apiKey) return { status: 400, error: 'Lidarr URL and API key are required.' };
        ...
        const data = await statusRes.json();
        return { ok: true, message: `Lidarr v${data.version || '?'} connected`, details: { version: data.version, appName: data.appName } };
    }
    ...
    if (type === 'tautulli') {
        const url = resolveIntegrationUrlForTest(tautulliUrl, stored.tautulliUrl, urlOpts);
    ...
    if (type === 'jellystat') {
        const url = resolveIntegrationUrlForFetch(resolveTestCredential(jellystatUrl, stored.jellystatUrl, secretMask));
    ...
    if (type === 'requestApp') {
        ...
        const baseUrl = secondaryOmbi
            ? resolveIntegrationUrlForTest(ombiUrl, stored.ombiUrl, urlOpts)
            : resolveIntegrationUrlForTest(requestAppUrl, stored.requestAppUrl, urlOpts);
