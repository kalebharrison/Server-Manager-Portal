...
import { allowSameOriginFrame } from '../http/http-security.js';
import { emailSubject } from '../comms/email-identity.js';
import { htmlForPreview } from '../comms/email-logo.js';
import {
    buildEmailPreviewIndexHtml,
    buildMockEmailCatalog,
    sendMockEmailCatalog,
} from '../comms/email-mock-catalog.js';
import { createInboundWebhookToken } from '../comms/email-inbound-auth.js';
import { buildInboundReplyTo } from '../comms/email-inbound.js';
...
export const registerConfigAdminUtilityRoutes = ({
    ...
    app.get('/api/config/inbound-webhook-token', requireAdmin, adminSensitiveRateLimit, async (_req, res) => {
        const storedConfig = await loadFile(CONFIG_PATH, {});
        const token = String(storedConfig.inboundWebhookToken || '').trim();
        if (!token) return res.status(404).json({ error: 'Inbound webhook token is not configured.' });
        res.json({ token });
    });

    app.post('/api/config/inbound-webhook-token/rotate', requireAdmin, adminSensitiveRateLimit, async (_req, res) => {
        const storedConfig = await loadFile(CONFIG_PATH, {});
        const token = createInboundWebhookToken();
        await saveFile(CONFIG_PATH, { ...storedConfig, inboundWebhookToken: token });
        log('[email] Rotated inbound webhook token');
        res.json({ token });
    });
    ...
    app.post('/api/config/test-email', requireAdmin, adminSensitiveRateLimit, async (req, res) => {
        const {
            smtpHost, smtpPort, smtpUser, smtpPass, smtpFrom, smtpSecure, testRecipient,
        ...
        const config = {
            ...
            mailjetInboundSecret: storedConfig.mailjetInboundSecret,
            inboundRepliesEnabled: inboundRepliesEnabled !== undefined
                ? !!inboundRepliesEnabled
                : storedConfig.inboundRepliesEnabled !== false,
            inboundReplyDomain: inboundReplyDomain !== undefined
                ? inboundReplyDomain
                : storedConfig.inboundReplyDomain,
            inboundReplyMailbox: storedConfig.inboundReplyMailbox,
            publicDomain: storedConfig.publicDomain,
            customLogoUrl: storedConfig.customLogoUrl,
        };
        ...
        try {
            ...
            await sendEmail(
                ...
                null,
                {
                    ...
                    replyTo: buildInboundReplyTo(config, { kind: 'test', id: 'smtp' }),
                },
            );
    ...
    app.get('/api/config/email-previews', requireAdmin, async (req, res) => {
        const storedConfig = await loadFile(CONFIG_PATH, {});
        const catalog = buildMockEmailCatalog({
            config: storedConfig,
            to: storedConfig.contactEmail || storedConfig.smtpFrom || 'preview@example.com',
        });
        const logoHref = String(storedConfig.customLogoUrl || '/static/logo.png').split('?')[0] || '/static/logo.png';
        const requestedId = String(req.query.id || '').trim();
        if (requestedId) {
            const entry = catalog.find((item) => item.id === requestedId);
            if (!entry) return res.status(404).type('html').send('Unknown email preview.');
            allowSameOriginFrame(res);
            return res.type('html').send(htmlForPreview(entry.html, logoHref));
        }
        res.type('html').send(buildEmailPreviewIndexHtml(catalog, {
            logoHref,
            hrefForId: (id) => `?id=${encodeURIComponent(id)}`,
        }));
    });
};
