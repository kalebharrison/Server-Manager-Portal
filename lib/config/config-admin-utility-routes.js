...
export const registerConfigAdminUtilityRoutes = ({
    ...
    app.get('/api/config/portal-agent-key', requireAdmin, adminSensitiveRateLimit, async (_req, res) => {
        try {
            const config = await loadFile(CONFIG_PATH, {});
            const ensured = await ensurePortalAgentApiKey(config, {
                save: typeof saveFile === 'function' ? (next) => saveFile(CONFIG_PATH, next) : null,
            });
            res.json({
                key: ensured.key,
                created: ensured.created,
                hint: 'Copy into gitignored .local/portal-agent.env as PORTAL_AGENT_API_KEY for Cursor/local agent calls.',
            });
        } catch (error) {
            log?.(`Failed to read portal agent key: ${error.message}`);
            res.status(500).json({ error: 'Failed to read portal agent key.' });
        }
    });

    app.post('/api/config/portal-agent-key/regenerate', requireAdmin, adminSensitiveRateLimit, async (_req, res) => {
        try {
            const config = await loadFile(CONFIG_PATH, {});
            const key = generatePortalAgentApiKey();
            const next = { ...config, portalAgentApiKey: key };
            if (typeof saveFile !== 'function') {
                return res.status(500).json({ error: 'Config save is unavailable.' });
            }
            await saveFile(CONFIG_PATH, next);
            res.json({
                key,
                regenerated: true,
                hint: 'Update .local/portal-agent.env with the new PORTAL_AGENT_API_KEY.',
            });
        } catch (error) {
            log?.(`Failed to regenerate portal agent key: ${error.message}`);
            res.status(500).json({ error: 'Failed to regenerate portal agent key.' });
        }
    });
