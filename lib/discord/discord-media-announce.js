...
import { canPostDiscordMemberEvent } from './discord-notify.js';

const DEFAULT_DEBOUNCE_MINUTES = 60;
const MAX_DEBOUNCE_MINUTES = 180;
...
/** Discord cannot fetch Arr MediaCover, LAN hosts, or non-HTTPS art. */
export const isDiscordSafeImageUrl = (value) => {
    ...
    try {
        ...
        if (!host || host === 'localhost' || host.endsWith('.local') || host.endsWith('.internal')
            || host.endsWith('.lan')) return false;
...
const safeThumb = (...values) => values.find((value) => isDiscordSafeImageUrl(value)) || null;

const copyIndexMeta = (item = {}) => ({
    year: item.year || null,
    overview: item.overview || '',
    thumbUrl: safeThumb(item.thumbUrl, item.posterUrl),
...
const sameArrIdentity = (group = {}, item = {}) => {
    if (group.arrType && item.arrType
        && String(item.arrType) !== String(group.arrType)) return false;
    if (group.arrInstanceId != null && item.arrInstanceId != null
        && String(group.arrInstanceId) !== String(item.arrInstanceId)) return false;
    return true;
};

const ratingKeyOwnsFileKey = (ratingKey, fileKey) => {
...
export const enrichAnnounceGroupFromIndex = (group = {}, index = {}) => {
    ...
    const hasSafeThumb = isDiscordSafeImageUrl(group.thumbUrl);
    if (hasSafeThumb && group.overview && group.year && group.tmdbId && group.imdbId
        && (!isShow || group.tvdbId)) return group;
    const items = Array.isArray(index.items) ? index.items : [];
    const fileKey = String(group.items?.[0]?.key || '');
    let match = items.find((item) => (
        item
        && sameArrIdentity(group, item)
        && group.entityId != null
        && item.entityId != null
        && String(item.entityId) === String(group.entityId)
    ));
    if (!match && fileKey) {
        match = items.find((item) => item?.ratingKey && ratingKeyOwnsFileKey(item.ratingKey, fileKey));
    }
    if (!match && group.title) {
        const titled = items.filter((item) => (
            item?.title === group.title && sameArrIdentity(group, item)
        ));
        if (titled.length === 1) match = titled[0];
    }
    if (!match) {
        return {
            ...group,
            thumbUrl: safeThumb(group.thumbUrl),
        };
    }
    return {
        ...group,
        ...
        thumbUrl: safeThumb(group.thumbUrl, match.thumbUrl, match.posterUrl),
...
const posterUrl = (value) => {
    const raw = String(value || '').trim();
    if (!isDiscordSafeImageUrl(raw)) return '';
    return raw
...
export const createDiscordMediaAnnounce = ({
    ...
    /** Optional: return true to defer a due TV group (Arr still downloading / more imports pending). */
    isSeriesBusy = null,
    log = () => {},
} = {}) => {
    const debounceMs = (config) => {
        const minutes = Math.max(
            ...
            Math.min(
                ...
                Number(config?.discordMediaAnnounceDebounceMinutes) || DEFAULT_DEBOUNCE_MINUTES,
            ),
        );
        return minutes * 60 * 1000;
    };

    const isTvGroup = (group = {}) => (
        String(group.arrType || '').toLowerCase() === 'sonarr'
        || String(group.mediaType || '').toLowerCase() === 'show'
    );
    ...
    const enqueue = async (config, item = {}) => {
        ...
        if (existingIdx >= 0) {
            ...
            pending.push({
                ...
                arrInstanceId: nextItem.arrInstanceId,
    ...
    const flushDue = async (config) => {
        ...
        for (const group of due) {
            if (isTvGroup(group) && typeof isSeriesBusy === 'function') {
                let busy = false;
                try {
                    busy = !!(await isSeriesBusy(config, group));
                } catch (error) {
                    log(`[discord] media announce busy-check failed: ${error.message}`);
                }
                if (busy) {
                    deferred += 1;
                    keep.push({
            ...
            try {
                ...
                if (ok) {
                    ...
                    log(`[discord] media announce webhook declined ${group.groupKey || 'group'}`);
                    keep.push(group);
                }
            } catch (error) {
    ...
    const postTestAnnounces = async (config, { index } = {}) => {
        ...
        if (!canPostDiscordMemberEvent(config)) {
            const error = new Error('Save a member channel ID or webhook URL first.');
            error.statusCode = 400;
            throw error;
        }
        const notifier = getDiscordNotifier?.();
        ...
        for (const group of groups) {
            ...
            if (!ok) {
                const error = new Error('Could not post the test announce to Discord.');
