...
export const enrichAnnounceGroupFromIndex = (group = {}, index = {}) => {
    if (group.thumbUrl && group.overview && group.year && group.tmdbId) return group;
    const items = Array.isArray(index.items) ? index.items : [];
...
const posterUrl = (value) => {
    ...
    if (/^https:\/\//i.test(raw)) {
        return raw
            .replace(/\/t\/p\/w\d+\//, '/t/p/w500/')
            .replace(/\/t\/p\/original\//, '/t/p/w500/');
    }
    return '';
};
...
export const buildMediaAnnounceEmbed = (group = {}, { isTest = false, links = [] } = {}) => {
    ...
    const authorName = isUpgrade ? `Upgraded ${kindLabel} available` : `${kindLabel[0].toUpperCase()}${kindLabel.slice(1)} available`;
    ...
    const linkBits = validLinks.map((link) => `[${link.label}](${link.url})`);
    if (linkBits.length) {
        fields.push({ name: 'Links', value: linkBits.join('  ·  '), inline: false });
    }
