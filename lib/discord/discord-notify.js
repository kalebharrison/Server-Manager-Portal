import { discordEmbed, postDiscordWebhook, resolveDiscordWebhookChannelId } from './discord-webhook.js';
...
export const createDiscordNotifier = ({
    ...
    const postAdminEvent = async (config, { title, description, fields = [], color } = {}) => {
        ...
        const message = { embeds: [discordEmbed({ title, description, fields, color })] };
        let channelId = String(config.discordAdminChannelId || '').trim();
        if (!channelId && adminUrl) {
            channelId = await resolveDiscordWebhookChannelId(adminUrl, { fetchImpl });
        }
        if (channelId) {
            const posted = await sendChannelMessage(config, channelId, message);
            if (posted) return true;
        ...
        return postDiscordWebhook(adminUrl, message, { fetchImpl, log });
    };
