...
export const createDiscordBotRuntime = ({
    ...
    const registerCommands = async (token, guildId, clientId) => {
        ...
        // Guild copy is what the server slash picker shows. Global copy is DM-only
        // so /help still works in bot DMs without duplicating in-server commands.
        await rest.put(Routes.applicationGuildCommands(clientId, guildId), {
            body: buildDiscordSlashCommands({ contexts: [InteractionContextType.Guild] }),
        });
        await rest.put(Routes.applicationCommands(clientId), {
            body: buildDiscordSlashCommands({ contexts: [InteractionContextType.BotDM] }),
        });
    };
