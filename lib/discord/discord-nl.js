import { normalizeThemeQuery } from './normalizeThemeQuery.js';
...
/** Strip articles and trailing media-type words that break TMDB multi-search (e.g. "Brad Pitt movie" → "Brad Pitt"). */
...
const PHRASES = [
    ...
    {
        re: /\b(?:what(?:'s|s| are| is)\s+)?(?:my\s+)?(?:current\s+)?stats?\b|\bwatch\s+history\b|\bhow\s+much\s+have\s+i\s+watched\b/i,
        intent: 'stats.me',
    },
