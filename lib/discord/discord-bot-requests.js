...
import { mediaEmbed, parseCustomId, truncate } from './discord-embeds.js';
...
export const createDiscordRequestHandlers = ({
    ...
    const runSearch = async (interaction, config, { query, mediaType = 'all' }) => {
        ...
        try {
            ...
            const preview = mediaEmbed(results[0], { footer: `Searching as ${member.displayName(portalUser)} · ${results.length} result(s)` });
    ...
    const presentTitleActions = async (interaction, config, { portalUser, mediaType, tmdbId }) => {
        ...
        const embed = mediaEmbed(item, { footer: `Acting as ${member.displayName(portalUser)}` });
    ...
    const submitRequest = async (interaction, config, {
        ...
        await interaction.editReply({
            ...
            embeds: detail ? [mediaEmbed({ ...detail, requested: true, canRequest: false })] : [],
            components: [],
        });
    };
    ...
    const handleMyRequestsCommand = async (interaction, config) => {
        ...
        try {
            ...
            const lines = results.map((item) => {
                const label = [item.title || 'Untitled', item.mediaType, item.requestStatusLabel || item.mediaStatusLabel || statusHint(item)]
                    .filter(Boolean)
                    .join(' · ');
                return truncate(label, 100);
            });
            const { listEmbed } = await import('./discord-embeds.js');
            await interaction.editReply({
                embeds: [listEmbed({
                    title: 'Your requests',
                    description: `Showing up to ${lines.length} for **${member.displayName(portalUser)}**`,
                    lines,
