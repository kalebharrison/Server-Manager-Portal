    ...
    const run = async (config, userText) => {
        ...
        for (let round = 0; round < MAX_ROUNDS; round += 1) {
            ...
            if (!toolCalls.length) {
                ...
                const leakedFinish = /finish\.(answer|candidates)\b/i.test(content);
                if (usedTools && leakedFinish && !nudgedForFinish) {
                    nudgedForFinish = true;
                    messages.push({
                        role: 'user',
                        content: 'Call the finish tool now with JSON arguments { "answer": "...", "candidates": [{ "mediaType", "tmdbId", "title" }] }. Do not write finish.answer or finish.candidates in plain text.',
                    });
                    continue;
                }
                ...
                finishPayload = {
                    answer: cleanAgentAnswer(content) || 'Here are some titles that may match.',
                    candidates: [],
                };
                break;
            }
