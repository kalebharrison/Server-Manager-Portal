...
/** Turn natural-language asks into a short catalog search string. */
...
const TOOLS = [
    {
        ...
        function: {
            name: 'web_search',
            description: 'Search the web for movie/TV recommendations, plot facts, or title identification. Uses configured search backends or a free DuckDuckGo fallback. Use before the catalog search for multi-constraint or “what’s that film” questions.',
            parameters: {
                type: 'object',
                properties: {
    ...
    },
    {
        ...
        function: {
            name: 'search_titles',
            description: 'Search the media server catalog (TMDB) by title or person name. Returns requestable titles with availability.',
            parameters: {
                type: 'object',
                properties: {
                    query: { type: 'string' },
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Resolve an actor/director and list their newest credits from TMDB.',
            parameters: {
                type: 'object',
                properties: {
                    name: { type: 'string' },
...
const SYSTEM_PROMPT = [
    'You are Requesty, a helpful media discovery butler for a private Plex media server.',
...
/** Multi-constraint plot/setting asks — do not keyword-dump into the catalog search. */
...
/** Score a catalog candidate against the user query — exact titles rank first. */
...
/**
 * Keep answer prose and result cards in sync when the answer names specific titles.
...
export const createDiscordMediaAgent = ({
    ...
    /** No-tools ask: return likely title strings when web/catalog keyword search fails. */
    ...
    const run = async (config, userText, { history = [] } = {}) => {
        ...
        let nudgedForCatalog = false;
        let foundCandidates = [];
        let webTitleHints = [];
        let sawCatalogTool = false;

        const resolveCatalogQueries = async (queries = []) => {
            const requestAppService = typeof getRequestAppService === 'function' ? getRequestAppService() : null;
            ...
            const pushQuery = (raw) => {
                ...
                if (!q) return;
                if (plotAsk) {
                    // Never catalog-search the raw NL plot blob.
            ...
            // Last resort: web-search for title names, then resolve against the catalog.
            if (!merged.length) {
            ...
            // If web search is blocked/empty, ask the LLM for title names only, then catalog-resolve.
            // Skip speculative LLM invents on plot riddles — they hydrate the wrong titles.
        ...
        for (let round = 0; round < MAX_ROUNDS; round += 1) {
            ...
            if (!toolCalls.length) {
                ...
                // Prefer catalog titles we already resolved over burning more rounds.
                if (usedTools && (fromContent || cleaned || foundCandidates.length)) {
                    if (!foundCandidates.length && (webTitleHints.length || query)) {
                        await resolveCatalogQueries([...webTitleHints, query]);
                    }
                    finishPayload = {
            ...
            for (const call of toolCalls) {
                ...
                if (name === 'search_titles' && isPlotDescriptionQuery(query)) {
                    ...
                    // Block keyword-soup catalog searches on plot riddles (e.g. "spanish thriller girl inherits...").
                ...
                if (toolResult?.finished) {
                    ...
                    const finishTitles = (Array.isArray(toolResult.candidates) ? toolResult.candidates : [])
                        ...
                        .filter(Boolean);
                    if (!foundCandidates.length) {
                        await resolveCatalogQueries([...finishTitles, ...webTitleHints, query]);
                    }
                    finishPayload = {
                        answer: cleanAgentAnswer(toolResult.answer) || answerFromCandidates(foundCandidates),
                        // Only trust model-provided ids when we never resolved titles via catalog tools.
            ...
            // Model often web-searches forever; nudge once, then auto-resolve via the catalog.
            if (!sawCatalogTool && !foundCandidates.length && webTitleHints.length && !nudgedForCatalog) {
                nudgedForCatalog = true;
                messages.push({
                    role: 'user',
        ...
        // Plot asks with no named identification: don't dump speculative catalog junk.
