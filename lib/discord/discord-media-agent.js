...
const PROMPT_INJECTION_RE = /\b(?:ignore|disregard|forget)\s+(?:all\s+)?(?:previous|prior|above|earlier)\s+instructions\b|\b(?:list|reveal|show|print|dump)\s+(?:your\s+)?(?:system\s+)?prompts?\b|\bjailbreak\b|\bdan\s+mode\b|\bact\s+as\s+(?:if\s+)?(?:you\s+have\s+)?no\s+restrictions\b/i;
const CODING_HELP_RE = /\b(?:write|create|generate|give)\s+(?:me\s+)?(?:a\s+)?(?:python|javascript|typescript|bash|shell)?\s*script\b|\b(?:python|javascript|typescript)\s+(?:script|code|function|program)\b|\bscrape\s+(?:plex|seerr|tmdb|imdb|the\s+web)\b|\bhow\s+(?:do\s+i|to)\s+(?:code|program|scrape|hack|write\s+a\s+script)\b|\bprogramming\s+help\b|\bcode\s+this\b/i;
const OFF_TOPIC_RE = /\b(?:take\s+out\s+(?:the\s+)?trash|trash\s*cans?|how\s+do\s+i\s+take\s+out)\b|\b(?:weather|forecast|temperature)\b|\b(?:recipe|recipes|homework|crypto|bitcoin|stocks?)\b|\b(?:buy|purchase|shopping|amazon)\b/i;
...
const PLOT_ID_RE = /\b(?:the one with|that (?:netflix |disney |hulu )?movie|that show|that anime|everyone is trapped|time loops?|sees? dead (?:people|persons)|inherits? a haunted|battle royale|solves murders|small town|cozy and british|zombie casino|haunted apartment)\b/i;
...
export const isVagueMediaRecommend = (text = '') => {
    ...
    if (PLOT_ID_RE.test(input)) return false;
    if (/\b(?:called|titled|named|where everyone|where the|who sees|who solves)\b/i.test(input)) return false;
...
export const classifyDiscoverIntent = (text = '') => {
    ...
    if (PLOT_ID_RE.test(input) || MEDIA_SIGNAL_RE.test(input) || LIKE_COMP_RE.test(input)) {
...
export const scoreCandidateForQuery = (query = '', item = {}) => {
    ...
    if (title.startsWith(q) || q.startsWith(title)) return 850;
    ...
    if (ratio >= 0.8) return 600 + Math.round(ratio * 100);
    if (ratio >= 0.5) return 300 + Math.round(ratio * 100);
    return Math.round(ratio * 100);
};

export const rankCandidatesForQuery = (query = '', candidates = []) => [...(Array.isArray(candidates) ? candidates : [])]
    .sort((a, b) => scoreCandidateForQuery(query, b) - scoreCandidateForQuery(query, a));
...
export const createDiscordMediaAgent = ({
    ...
    const run = async (config, userText, { history = [] } = {}) => {
        ...
        const resolveSeerrQueries = async (queries = []) => {
            ...
            // Theme discover helps multi-constraint asks when title search is empty.
            ...
            if (!merged.length) {
                ...
                if (webQuery.length >= 3) {
                    try {
                        ...
                        const hints = titleHintsFromWebResults(web?.results);
                        webTitleHints = [
                            ...webTitleHints,
                            ...hints,
        ...
        if (!results.length && !finishPayload.candidates.length) {
