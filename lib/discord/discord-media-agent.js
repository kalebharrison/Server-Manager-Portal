...
export const createDiscordMediaAgent = ({
    ...
    const run = async (config, userText, { history = [] } = {}) => {
        ...
        const genericAnswer = !cleanedAnswer
            || cleanedAnswer === 'Here are some titles that may match.'
            || cleanedAnswer === OUT_OF_STEPS_ANSWER;
        // Plot asks with no verified titles: prefer clarify-style identify miss over junk dumps.
        if (genericAnswer && !results.length && isPlotDescriptionQuery(query)) {
            return { ok: true, answer: COULD_NOT_IDENTIFY_ANSWER, results: [] };
        }
        return {
            ok: true,
            answer: cleanedAnswer || 'Here are some titles that may match.',
            results,
        };
    };
