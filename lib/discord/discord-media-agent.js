...
const SYSTEM_PROMPT = [
    ...
    'You MUST use tools for every user message. Never reply with plain text only.',
    ...
    'End every successful turn by calling the finish tool only (never write the word Finish in your message content).',
    'finish.answer must be a short clean sentence for Discord. finish.candidates are titles you verified via Seerr tools (up to 5).',
    'Mention which are already available vs requestable when you know.',
].join(' ');

export const cleanAgentAnswer = (answer = '') => String(answer || '')
...
export const createDiscordMediaAgent = ({
    ...
    const run = async (config, userText) => {
        ...
        for (let round = 0; round < MAX_ROUNDS; round += 1) {
            ...
            if (!toolCalls.length) {
                const fromContent = extractFinishFromContent(message.content);
                if (fromContent && usedTools) {
                    finishPayload = {
                        answer: fromContent.answer,
                        candidates: normalizeCandidates(fromContent.candidates),
                    };
                    break;
                }
                ...
                finishPayload = {
                    answer: cleanAgentAnswer(message.content) || 'Here are some titles that may match.',
                    candidates: [],
                };
                break;
            }
