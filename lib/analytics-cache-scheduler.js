...
export const createAnalyticsCacheScheduler = ({
    ...
    app.get('/api/media-stack/trending', requireAuth, requireMember, async (req, res) => {
        const stats = await loadScopedCache(TRENDING_CACHE_PATH, { movies: 0, series: 0 });
        res.json(stats);
    });
    ...
    async function calculateAnalyticsStatsNow() {
        ...
        try {
            ...
            const historyItems = historyStore && shouldBuildColdWindows
                ? await historyStore.fetchIncrementalServerHistory(uri, config, { maxItems: 250000 })
                : await fetchPlexServerHistory(uri, config, {
                    maxItems: shouldBuildColdWindows ? 250000 : 50000,
                    stopBeforeTs: shouldBuildColdWindows ? 0 : nowSec - (oldestHotCompareDays * 24 * 60 * 60),
                    log,
                });
