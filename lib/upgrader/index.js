            ...
            for (const instance of instances) {
                ...
                for (const record of seriesRecords) {
                    ...
                    const [episodeFiles, episodes] = await Promise.all([
                        ...
                        request(instance, `/api/v3/episode?seriesId=${encodeURIComponent(record.id)}&includeEpisodeFile=true`).catch(() => []),
                    ]);
                ...
                log(`[upgrader] ${instance.name}: ${sonarrWithFiles}/${seriesRecords.length} series with files indexed`
                    + ` (${items.filter((item) => item.arrInstanceId === instance.id && item.scoreUnknown).length} score-unknown)`);
            }
