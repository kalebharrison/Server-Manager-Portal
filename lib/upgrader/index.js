...
import { createDiscordMediaAnnounce } from '../discord/discord-media-announce.js';
import { createSonarrSeriesBusyCheck } from '../discord/discord-media-announce-sonarr-busy.js';
import { createLibraryDeepLinkResolver } from '../media/library-deep-link.js';
...
export const createUpgraderService = ({
    ...
    const loadIndexShared = () => loadFileShared(indexPath, { generatedAt: null, itemCount: 0, items: [] });
            ...
            for (const instance of instances) {
                ...
                if (instance.type === 'lidarr') {
                    ...
                    await mapPool(withFiles, 6, async (record) => {
                        ...
                        items.push(buildLidarrIndexItem(instance, record, trackFile, profile, trackFiles));
                    });
                ...
                await mapPool(seriesRecords, 8, async (record) => {
                    ...
                    // Pull episode rows whenever stats show an aired/file gap (missing hunt path).
                    const needsEpisodeList = episodeCount > fileCount;
                    if (!needsEpisodeList) {
    ...
    const libraryLinks = createLibraryDeepLinkResolver({
        fetchImpl: (url, options) => fetchWithTimeout(url, options || {}, 12000),
        getPlexConnectionUri,
        resolveIntegrationUrlForFetch,
    });
    const mediaAnnounce = createDiscordMediaAnnounce({
        loadPrefs,
        savePrefs,
        getDiscordNotifier: () => discordNotifier,
        loadIndex,
        resolveLibraryLinks: (config, query) => libraryLinks.resolve(config, query),
        isSeriesBusy: createSonarrSeriesBusyCheck({ request, log }),
        log,
    });
    ...
    const integrity = createQcIntegrity({
        ...
        getDiscordNotifier: () => discordNotifier,
        mediaAnnounce,
        resolvePlexUri: getPlexConnectionUri,
        fetchImpl: (url, options) => fetchWithTimeout(url, options || {}, 15000),
        actionsRemaining: (config) => hunt.actionsRemaining(config),
        markAction: () => hunt.markAction(),
        requestIndexRefresh: () => {
            if (staleIndexRefreshTimer || running) return;
            staleIndexRefreshTimer = setTimeout(() => {
                staleIndexRefreshTimer = null;
                void (async () => {
                    try {
                        if (typeof indexRefreshGetConfig !== 'function') return;
                        const config = await indexRefreshGetConfig();
                        if (config?.upgraderEnabled) await buildIndex(config);
                    } catch (error) {
                        log(`[upgrader] stale-path index refresh failed: ${error.message}`);
                    }
                })();
            }, 10 * 1000);
        },
    ...
    let indexRefreshGetConfig = null;
    let staleIndexRefreshTimer = null;

    const startIndexJob = (getConfig) => {
        indexRefreshGetConfig = getConfig;
    ...
    const startSnapshotJob = (getConfig) => downloadHealth.startSnapshotJob(getConfig);
