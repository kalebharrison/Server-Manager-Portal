        ...
        try {
            ...
            for (const instance of instances) {
                if (instance.type === 'radarr') {
                    ...
                    // /movie embeds movieFile without a real customFormatScore — pull /moviefile.
                    const movieIds = records
                        .filter((record) => record?.hasFile || record?.movieFile)
                    ...
                    for (let i = 0; i < movieIds.length; i += batchSize) {
                        ...
                        const qs = batch.map((id) => `movieId=${encodeURIComponent(id)}`).join('&');
                        const files = await request(instance, `/api/v3/moviefile?${qs}`).catch(() => []);
                    ...
                    for (const record of records) {
                        items.push(buildRadarrIndexItem(instance, record, fileByMovieId.get(Number(record.id)) || null));
                    }
                    continue;
                }
