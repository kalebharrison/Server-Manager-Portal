...
export const resolveNativeLanguage = (candidate = {}) => {
    const raw = candidate.originalLanguage
        || candidate.originalLanguageCode
        || candidate.original_language
        || candidate.seriesOriginalLanguage
        || null;
    if (raw && typeof raw === 'object') {
        return normalizeLanguageCode(raw.id || raw.name || raw.value);
    }
...
export const trimMkvFile = async (filePath, {
    ...
    if (dryRun) {
        return {
            ...
            dryRun: true,
            reason: 'trim_dry_run',
            detail: `would drop audio [${plan.audioDrop.join(',') || 'none'}] subs [${plan.subDrop.join(',') || 'none'}]`,
            plan,
        };
    }
