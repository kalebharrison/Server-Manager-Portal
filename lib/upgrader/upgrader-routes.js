...
export const registerUpgraderRoutes = ({
    ...
    app.post('/api/upgrader/customformats/repair-unkn0wn-remux', requireAdmin, requireUpgrader, async (_req, res) => {
        const { repairUnkn0wnRemuxFormats, needsUnkn0wnRemuxRepair } = await import('./cf-unkn0wn-remux.js');
        const cfg = await config();
        const instances = [
        ...
        for (const instance of instances) {
            try {
                ...
                const needingRepair = list.filter((format) => needsUnkn0wnRemuxRepair(format));
                const { repaired } = repairUnkn0wnRemuxFormats(list);
                const saved = [];
                for (const format of repaired) {
                    const updated = await upgrader.request(
                        instance,
                        `/api/v3/customformat/${encodeURIComponent(format.id)}`,
                        { method: 'PUT', body: format },
                    );
                    saved.push({ id: updated?.id ?? format.id, name: updated?.name ?? format.name });
                }
                results.push({
