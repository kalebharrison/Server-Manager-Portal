...
import { applyArrAlignment, getArrAlignment } from './qc-arr-optimize.js';
...
export const registerUpgraderRoutes = ({
    ...
    app.get('/api/upgrader/status', requireAdmin, requireUpgrader, async (_req, res) => {
        const cfg = await config();
        const index = await upgrader.loadIndexShared();
        ...
        const qcThresholds = {
            metaDlMinutes: Math.max(1, Number(cfg.qcMetaDlMinutes) || 20),
            stalledHours: Math.max(1, Number(cfg.qcStalledHours) || 2),
            slowDownloadFloorKbps: Math.max(0, Number(cfg.qcSlowDownloadFloorKbps ?? 100) || 0),
            slowDownloadMinAgeHours: Math.max(0, Number(cfg.qcSlowDownloadMinAgeHours ?? 6) || 0),
            completedNotImportingMinutes: Math.max(1, Number(cfg.qcCompletedNotImportingMinutes) || 90),
        ...
        try {
            ...
            for (const record of queueRecords) {
                ...
                if (!labelByKey.has(key)) {
                    const fallback = record.arrInstanceName || key;
                    const isMusic = String(key).startsWith('lidarr:')
                        || /^(lidarr|artists?|music)$/i.test(String(fallback));
                    labelByKey.set(key, isMusic ? 'Music' : fallback);
                }
                if (!counts.has(key)) counts.set(key, 0);
            }
        ...
        const qcCleanupAggression = detectCleanupPreset({
            qcCleanupAggression: cfg.qcCleanupAggression,
            qcMaxStrikes: maxStrikes,
            qcMetaDlMinutes: qcThresholds.metaDlMinutes,
            qcStalledHours: qcThresholds.stalledHours,
            qcCompletedNotImportingMinutes: qcThresholds.completedNotImportingMinutes,
            qcOrphanGraceMinutes: qcThresholds.orphanGraceMinutes,
            qcSlowDownloadFloorKbps: qcThresholds.slowDownloadFloorKbps,
            qcSlowDownloadMinAgeHours: qcThresholds.slowDownloadMinAgeHours,
        });
        const maxActionsPerHour = Math.max(1, Number(cfg.upgraderMaxActionsPerHour) || 25);
        const minScoreDelta = Math.max(0, Number(cfg.upgraderMinScoreDelta ?? 10) || 0);
        const upgraderHuntIntensity = detectHuntPreset({
            upgraderHuntIntensity: cfg.upgraderHuntIntensity,
            upgraderMaxActionsPerHour: maxActionsPerHour,
            upgraderMaxDownloadsPerLibrary: maxDownloadsPerLibrary,
            upgraderMinScoreDelta: minScoreDelta,
        });
        res.json({
            enabled: true,
            ...
            integrity: upgrader.integrity
                ? await upgrader.integrity.getStatus(cfg, { index })
                : null,
            qcMetrics: {
            ...
            qcThresholds,
            qcCleanupAggression,
            qcCleanupAggressionLabel: CLEANUP_PRESET_LABELS[qcCleanupAggression] || 'Custom',
            ...
            upgraderHuntIntensityLabel: HUNT_PRESET_LABELS[upgraderHuntIntensity] || 'Custom',
            preferences: prefs,
            recentUpgradeCount: upgrader.hunt.actionsRemaining
                ? Math.max(0, maxActionsPerHour - upgrader.hunt.actionsRemaining(cfg))
                : 0,
    ...
    app.get('/api/upgrader/items', requireAdmin, requireUpgrader, async (req, res) => {
        const [index, prefs, cfg] = await Promise.all([upgrader.loadIndexShared(), upgrader.loadPrefs(), config()]);
    ...
    app.get('/api/upgrader/audit', requireAdmin, requireUpgrader, async (req, res) => {
        ...
        const entries = (audit.entries || []).slice(0, limit).map((entry) => {
            ...
            return {
                ...
                // Older failed hunts omitted library stamps — backfill from index.
                libraryKey: entry.libraryKey || (item ? libraryKeyForItem(item) : null),
                libraryName: entry.libraryName || (item ? libraryLabelForItem(item) : null),
                arrType: entry.arrType || item?.arrType || null,
                arrInstanceName: entry.arrInstanceName || item?.arrInstanceName || null,
                arrInstanceId: entry.arrInstanceId || item?.arrInstanceId || null,
            };
        });
        res.json({ entries });
    });
    ...
    app.post('/api/upgrader/arr/:instanceId/customformats', requireAdmin, requireUpgrader, async (req, res) => {
        const instance = await resolveInstance(req.params.instanceId);
        if (!instance) return res.status(404).json({ error: 'Arr instance not found' });
        const { patchUnkn0wnRemuxFormat } = await import('./cf-unkn0wn-remux.js');
        const body = patchUnkn0wnRemuxFormat(req.body) || req.body;
        const created = await upgrader.request(instance, '/api/v3/customformat', { method: 'POST', body });
    ...
    app.put('/api/upgrader/arr/:instanceId/customformats/:formatId', requireAdmin, requireUpgrader, async (req, res) => {
        ...
        if (!instance) return res.status(404).json({ error: 'Arr instance not found' });
        const { patchUnkn0wnRemuxFormat } = await import('./cf-unkn0wn-remux.js');
        const body = patchUnkn0wnRemuxFormat({ ...req.body, id: Number(req.params.formatId) })
            || { ...req.body, id: Number(req.params.formatId) };
    ...
    app.get('/api/upgrader/customformats/repairs', requireAdmin, requireUpgrader, async (_req, res) => {
        const { scanCfRepairsForFormats, CF_REPAIR_CATALOG } = await import('./cf-repairs.js');
        const cfg = await config();
        const instances = [
            ...getReadyArrInstances(cfg, 'radarr'),
            ...getReadyArrInstances(cfg, 'sonarr'),
        ];
        const results = [];
        for (const instance of instances) {
            try {
                ...
                const repairs = scanCfRepairsForFormats(list, { type: instance.type });
                results.push({
                    ...
                    repairs,
                    needingCount: repairs.reduce((sum, row) => sum + Number(row.needingCount || 0), 0),
                });
            } catch (error) {
                results.push({
                    instanceId: instance.id,
        ...
        const needingCount = results.reduce((sum, row) => sum + Number(row.needingCount || 0), 0);
        res.json({
            ok: true,
            catalog: CF_REPAIR_CATALOG.map((entry) => ({
                id: entry.id,
                title: entry.title,
            ...
            needingCount,
            healthy: needingCount === 0,
            results,
        });
    });

    const runCfRepairs = async (repairIds = null) => {
        const { applyCfRepairsForFormats, scanCfRepairsForFormats } = await import('./cf-repairs.js');
        const cfg = await config();
        const instances = [
            ...getReadyArrInstances(cfg, 'radarr'),
            ...getReadyArrInstances(cfg, 'sonarr'),
        ...
        for (const instance of instances) {
            try {
                const formats = await upgrader.request(instance, '/api/v3/customformat');
                const list = Array.isArray(formats) ? formats : [];
                const before = scanCfRepairsForFormats(list, { type: instance.type });
                const patched = applyCfRepairsForFormats(list, { repairIds });
                const saved = [];
                for (const entry of patched) {
                    const format = entry.format;
                    const updated = await upgrader.request(
                        instance,
                        `/api/v3/customformat/${encodeURIComponent(format.id)}`,
                        { method: 'PUT', body: format },
                    );
                    saved.push({
                        repairId: entry.repairId,
                        id: updated?.id ?? format.id,
                        name: updated?.name ?? format.name,
                    });
                }
                results.push({
                    instanceId: instance.id,
                    ...
                    type: instance.type,
                    scanned: list.length,
                    needingRepair: before.reduce((sum, row) => sum + Number(row.needingCount || 0), 0),
                    repaired: saved,
                });
            } catch (error) {
                results.push({
        ...
        return {
            ok: true,
            repairedCount: results.reduce((sum, row) => sum + (row.repaired?.length || 0), 0),
            results,
        };
    };

    app.post('/api/upgrader/customformats/repairs', requireAdmin, requireUpgrader, async (req, res) => {
        res.json(await runCfRepairs(req.body?.repairIds || req.body?.repairId || null));
    });

    // Back-compat alias for the first catalog repair.
    app.post('/api/upgrader/customformats/repair-unkn0wn-remux', requireAdmin, requireUpgrader, async (_req, res) => {
        res.json(await runCfRepairs(['unkn0wn-remux']));
    });
    ...
    app.get('/api/upgrader/qc/downloads', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            ...
            const payload = dryRun
                ...
                : await downloadHealth().getSnapshot(cfg, { force });
            res.json(payload);
        } catch (error) {
    ...
    app.post('/api/upgrader/qc/integrity/scan', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            ...
            const libraryKey = req.body?.libraryKey != null && String(req.body.libraryKey).trim() !== ''
                ? String(req.body.libraryKey)
                : null;
            const libraryLabel = req.body?.libraryLabel != null
                ? String(req.body.libraryLabel)
                : null;
            const audit = req.body?.audit === true || req.body?.audit === 'true';
            const opts = {
                dryRun: !live || !replaceFindings || audit,
                replaceFindings: audit ? false : !!replaceFindings,
                limit: req.body?.limit ?? null,
                force: audit ? true : !!req.body?.force,
                mode: audit ? 'trim' : (req.body?.mode || 'baseline'),
                full: audit ? true : full,
                libraryKey,
                libraryLabel,
                audit,
                forceDryRun: audit || req.body?.forceDryRun === true,
            };
            // Full-library passes can take hours — start in background and let the UI poll.
            if ((opts.full || audit) && typeof upgrader.integrity.beginScanIntegrity === 'function') {
                const started = upgrader.integrity.beginScanIntegrity(cfg, opts);
                return res.json(started);
            }
            const result = await upgrader.integrity.scanIntegrity(cfg, opts);
            res.json(result);
        } catch (error) {
    ...
    app.get('/api/upgrader/qc/integrity/trim-audit', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            const status = await upgrader.integrity.getStatus(await config());
            const libraryKey = req.query?.libraryKey != null && String(req.query.libraryKey).trim() !== ''
                ? String(req.query.libraryKey)
                : null;
            let report = status?.trimAudit || status?.trimPreview || null;
            if (report && libraryKey && report.libraryKey && String(report.libraryKey) !== libraryKey) {
                // Prefer matching audit report; fall back only if preview is the same library.
                report = status?.trimAudit?.libraryKey === libraryKey
                    ? status.trimAudit
                    : (status?.trimPreview?.libraryKey === libraryKey ? status.trimPreview : report);
            }
            if (!report) {
                return res.status(404).json({ error: 'No trim audit report available yet' });
            }
            const format = String(req.query?.format || 'json').toLowerCase();
            if (format === 'csv') {
                const rows = Array.isArray(report.items) ? report.items : [];
                const header = [
                    'title', 'key', 'reason', 'path', 'wouldRemux', 'remuxed',
                    'audioKeep', 'audioDrop', 'subKeep', 'subDrop', 'nativeLanguage',
                ];
                const escape = (value) => {
                ...
                const lines = [
                    header.join(','),
                    ...rows.map((item) => header.map((col) => {
                        if (col === 'wouldRemux') return escape(item.wouldRemux !== false);
                        if (col === 'remuxed') return escape(!!item.remuxed);
                        if (['audioKeep', 'audioDrop', 'subKeep', 'subDrop'].includes(col)) {
                            const list = item[col] || item.trimPlan?.[col] || [];
                            return escape(Array.isArray(list) ? list.join('|') : list);
                        }
                        if (col === 'nativeLanguage') {
                            return escape(item.nativeLanguage || item.trimNativeLang || '');
                        }
                        if (col === 'path') return escape(item.path || item.filePath || '');
                        return escape(item[col]);
                    }).join(',')),
                ];
                res.setHeader('Content-Type', 'text/csv; charset=utf-8');
                res.setHeader('Content-Disposition', 'attachment; filename="trim-audit.csv"');
                return res.send(lines.join('\n'));
            }
            res.setHeader('Content-Disposition', 'attachment; filename="trim-audit.json"');
            res.json(report);
        } catch (error) {
            res.status(500).json({ error: error.message || 'Trim audit download failed' });
        }
    });

    app.post('/api/upgrader/qc/integrity/scan/cancel', requireAdmin, requireUpgrader, async (_req, res) => {
        try {
            if (typeof upgrader.integrity.cancelScanIntegrity !== 'function') {
                return res.status(501).json({ error: 'Scan cancel is not available' });
            }
            const result = upgrader.integrity.cancelScanIntegrity();
            res.json(result);
        } catch (error) {
            res.status(500).json({ error: error.message || 'Integrity scan cancel failed' });
        }
    });

    app.get('/api/upgrader/qc/integrity/lookup', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            const cfg = await config();
            const result = await upgrader.integrity.lookupIntegrityFiles(cfg, {
                query: req.query?.q || req.query?.query || '',
                limit: req.query?.limit,
            });
            res.json(result);
        } catch (error) {
            res.status(500).json({ error: error.message || 'Integrity lookup failed' });
        }
    });

    app.post('/api/upgrader/qc/integrity/file-check', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            const cfg = await config();
            const result = await upgrader.integrity.runFileCheck(cfg, {
                key: req.body?.key,
                mode: req.body?.mode || 'playability',
            });
            if (!result.ok && result.reason === 'File not found in library index') {
                return res.status(404).json(result);
            }
            res.json(result);
        } catch (error) {
            res.status(500).json({ error: error.message || 'Integrity file check failed' });
        }
    });
        ...
        try {
            ...
            if (!result.success) {
                return res.status(400).json({
                    error: result.reason || 'Replace failed',
                    ...result,
                });
            }
            res.json(result);
    ...
    app.get('/api/upgrader/qc/arr-alignment', requireAdmin, requireUpgrader, async (_req, res) => {
        try {
            const cfg = await config();
            res.json(await getArrAlignment(cfg, {
                resolveUrl,
                loadPrefs: upgrader.loadPrefs,
            }));
        } catch (error) {
            res.status(500).json({ error: error.message || 'Failed to read Arr alignment' });
        }
    });

    app.post('/api/upgrader/qc/arr-alignment', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            const cfg = await config();
            const result = await applyArrAlignment(cfg, {
                resolveUrl,
                loadPrefs: upgrader.loadPrefs,
                savePrefs: upgrader.savePrefs,
                log: console.log,
            });
            const { config: nextConfig, ...publicResult } = result;
            if (nextConfig && typeof saveFile === 'function') {
                await saveFile(configPath, nextConfig);
            }
            await appendAuditLog({
                action: 'qc_arr_alignment',
                actor: req.user?.username || req.user?.id || null,
                aligned: publicResult.aligned,
                instances: (publicResult.instances || []).map((entry) => ({
                    kind: entry.kind,
                    aligned: entry.aligned,
                })),
            }).catch(() => {});
            if (!publicResult.aligned) {
                const failed = (publicResult.applied?.results || publicResult.instances || [])
                    .filter((entry) => entry.ok === false || entry.aligned === false)
                    .map((entry) => `${entry.name || entry.kind}: ${entry.error || 'not aligned'}`)
                    .join('; ');
                return res.status(400).json({
                    error: publicResult.applied?.error || failed || 'Some Arr optimize steps failed.',
                    ...publicResult,
                });
            }
            res.json({
                ...publicResult,
                message: `Arrs optimized (${publicResult.instances.length} instance${publicResult.instances.length === 1 ? '' : 's'}).`,
            });
        } catch (error) {
            res.status(500).json({ error: error.message || 'Arr optimize failed' });
        }
    });

    app.post('/api/upgrader/qc/integrity/sync-arr-hooks', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            const cfg = await config();
            const result = await applyArrAlignment(cfg, {
                resolveUrl,
            ...
            if (!publicResult.aligned) {
                const failed = (publicResult.applied?.results || [])
                    .filter((entry) => !entry.ok)
                    .map((entry) => `${entry.name || entry.kind}: ${entry.error}`)
                    .join('; ');
                return res.status(400).json({
                    error: publicResult.applied?.error || failed || 'Some Arr hook syncs failed.',
                    results: publicResult.applied?.results || [],
                });
            }
            const created = (publicResult.applied?.results || []).filter((entry) => entry.action === 'create').length;
            const updated = (publicResult.applied?.results || []).filter((entry) => entry.action === 'update').length;
            res.json({
                ok: true,
                message: `Synced ${publicResult.instances.length} Arr hook${publicResult.instances.length === 1 ? '' : 's'} (${created} created, ${updated} updated).`,
                results: publicResult.applied?.results || [],
            });
        } catch (error) {
            res.status(500).json({ error: error.message || 'Arr hook sync failed' });
        }
    });

    app.post('/api/upgrader/qc/integrity/breaker/clear', requireAdmin, requireUpgrader, async (_req, res) => {
        try {
            const breaker = await upgrader.integrity.clearBreaker();
            res.json({ ok: true, breaker });
        } catch (error) {
            res.status(500).json({ error: error.message || 'Failed to clear integrity breaker' });
        }
    });

    app.post('/api/upgrader/qc/integrity/snooze', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            ...
            const hours = Math.max(
                ...
                Number(req.body?.hours)
                    || Number(cfg.qcSnoozeDefaultHours)
                    || 24,
            );
            const result = await upgrader.integrity.snoozeFinding(key, hours, {
                title: req.body?.title != null ? String(req.body.title) : null,
            });
            res.json({ ok: true, ...result });
        } catch (error) {
            res.status(500).json({ error: error.message || 'Failed to snooze integrity finding' });
        }
    });

    app.post('/api/upgrader/qc/integrity/snooze/clear', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            const key = req.body?.key != null ? String(req.body.key).trim() : '';
            const result = await upgrader.integrity.clearIntegritySnooze(key || null);
            res.json({ ok: true, ...result });
        } catch (error) {
            res.status(500).json({ error: error.message || 'Failed to clear integrity snooze' });
        }
    });

    app.get('/api/upgrader/qc/integrity/snoozes', requireAdmin, requireUpgrader, async (_req, res) => {
        try {
            const snoozes = await upgrader.integrity.listIntegritySnoozes();
            res.json({ snoozes });
        } catch (error) {
            res.status(500).json({ error: error.message || 'Failed to list integrity snoozes' });
        }
    });

    app.post('/api/upgrader/qc/integrity/recheck', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            const cfg = await config();
            const key = String(req.body?.key || req.body?.finding?.key || '').trim();
            if (!key) return res.status(400).json({ error: 'key is required' });
            const result = await upgrader.integrity.recheckFinding(cfg, {
                key,
                finding: req.body?.finding || null,
            });
            if (!result.ok && result.reason === 'Scan already in progress') {
            ...
            if (!result.ok) {
                return res.status(400).json({
                    error: result.reason || 'Integrity recheck failed',
                    ...result,
                });
            }
            res.json(result);
        } catch (error) {
            res.status(500).json({ error: error.message || 'Integrity recheck failed' });
        }
    });

    app.post('/api/upgrader/qc/integrity/accept-runtime', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            const cfg = await config();
            const key = String(req.body?.key || req.body?.finding?.key || '').trim();
            if (!key) return res.status(400).json({ error: 'key is required' });
            const result = await upgrader.integrity.acceptRuntimeFinding(cfg, {
                key,
                finding: req.body?.finding || null,
            });
            if (!result.ok) {
                return res.status(400).json({
                    error: result.reason || 'Accept runtime failed',
                    ...result,
                });
            }
            res.json(result);
        } catch (error) {
            res.status(500).json({ error: error.message || 'Accept runtime failed' });
        }
    });

    app.post('/api/upgrader/qc/integrity/native-language', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            const cfg = await config();
            const key = String(req.body?.key || req.body?.finding?.key || '').trim();
            if (!key) return res.status(400).json({ error: 'key is required' });
            const result = await upgrader.integrity.setNativeLanguageFinding(cfg, {
                key,
                code: req.body?.code,
                finding: req.body?.finding || null,
            });
            if (!result.ok) {
                return res.status(400).json({
                    error: result.reason || 'Set native language failed',
                    ...result,
                });
            }
            res.json(result);
        } catch (error) {
            res.status(500).json({ error: error.message || 'Set native language failed' });
        }
    });
    ...
    app.get('/api/upgrader/qc/client-alignment', requireAdmin, requireUpgrader, async (_req, res) => {
        try {
            const cfg = await config();
            res.json(await downloadHealth().getClientAlignment(cfg));
        } catch (error) {
            res.status(500).json({ error: error.message || 'Failed to read client alignment' });
        }
    });

    app.post('/api/upgrader/qc/client-alignment', requireAdmin, requireUpgrader, async (_req, res) => {
        try {
            const cfg = await config();
            const result = await downloadHealth().applyClientAlignment(cfg);
            const failed = Boolean(result?.applied?.errors?.sab || result?.applied?.errors?.qbit);
            if (failed) return res.status(502).json(result);
            res.json(result);
        } catch (error) {
            res.status(500).json({ error: error.message || 'Failed to apply client alignment' });
        }
    });
