...
import { syncIntegrityHooksToArr } from './arr-integrity-hook-sync.js';
...
export const registerUpgraderRoutes = ({
    ...
    app.post('/api/upgrader/qc/integrity/sync-arr-hooks', requireAdmin, requireUpgrader, async (req, res) => {
        try {
            const cfg = await config();
            const result = await syncIntegrityHooksToArr(cfg, {
            ...
            if (!result.ok) {
                ...
                return res.status(400).json({
                    error: result.error || failed || 'Some Arr hook syncs failed.',
                    results: result.results || [],
                });
            }
            const created = result.results.filter((entry) => entry.action === 'create').length;
            const updated = result.results.filter((entry) => entry.action === 'update').length;
            const testFailed = result.results.filter((entry) => entry.testOk === false);
            let message = `Synced ${result.results.length} Arr hook${result.results.length === 1 ? '' : 's'} (${created} created, ${updated} updated).`;
            if (testFailed.length) {
                message += ` Arr could not reach the portal test URL from ${testFailed.map((entry) => entry.name || entry.kind).join(', ')}.`;
            }
            res.json({
                ok: true,
                message,
