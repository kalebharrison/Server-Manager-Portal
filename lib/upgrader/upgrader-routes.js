    ...
    app.get('/api/upgrader/status', requireAdmin, requireUpgrader, async (_req, res) => {
        ...
        res.json({
            ...
            integrity: upgrader.integrity
                ...
                : null,
            qcMetrics: (await upgrader.loadPrefs()).qcMetrics || { wastedBytes: 0, killsByReason: {} },
            qcThresholds: {
                metaDlMinutes: Math.max(1, Number(cfg.qcMetaDlMinutes) || 30),
                stalledHours: Math.max(1, Number(cfg.qcStalledHours) || 6),
                completedNotImportingMinutes: Math.max(1, Number(cfg.qcCompletedNotImportingMinutes) || 60),
                orphanGraceMinutes: Math.max(0, Number(cfg.qcOrphanGraceMinutes ?? 45) || 0),
            ...
            maxDownloadsPerLibrary: Math.max(1, Number(cfg.upgraderMaxDownloadsPerLibrary) || 5),
