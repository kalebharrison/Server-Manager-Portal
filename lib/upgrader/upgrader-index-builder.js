...
import { resolveNativeLanguage } from './qc-media-trim.js';
...
export const buildRadarrIndexItem = (instance, record, movieFile = null, profile = null, { now = Date.now() } = {}) => {
    ...
    return {
        ...
        mediaType: 'movie',
        originalLanguage: resolveNativeLanguage({
            originalLanguage: record.originalLanguage,
            originalLanguageCode: record.originalLanguageCode,
            libraryBucket: library.libraryBucket,
            libraryKey: library.libraryKey,
            libraryName: library.libraryName,
        }),
...
export const buildSonarrIndexItem = (instance, record, episodeFiles = [], episodes = [], profile = null, { now = Date.now() } = {}) => {
    ...
    let seasons = [...seasonMap.entries()].map(([seasonNumber, rows]) => {
        ...
        return {
            ...
            avgCustomFormatScore: scores.length
                ...
                : 0,
            minCustomFormatScore: scores.length ? Math.min(...scores) : 0,
    ...
    const huntMissingEligible = missing.missingAiredCount > 0;
...
export const buildLidarrIndexItem = (instance, record, trackFile = null, profile = null, trackFiles = null) => {
    const files = Array.isArray(trackFiles) && trackFiles.length
        ? trackFiles
        : (trackFile ? [trackFile] : (record.trackFile ? [record.trackFile] : []));
    const primary = trackFile || files[0] || record.trackFile || null;
    const snap = fileSnapshot(primary, profile);
    const library = classifyUpgraderLibrary(instance, record);
    const artistName = record.artist?.artistName || record.artistName || '';
    const title = artistName ? `${artistName} — ${record.title}` : record.title;
    const trackFileRows = files
        .filter((file) => file?.id && (file?.path || file?.relativePath))
        .map((file) => ({
            trackFileId: Number(file.id) || null,
            filePath: file.path || resolveArrFilePath(file, record.path),
            expectedRuntimeSec: extractExpectedRuntimeSec({
                file,
                record,
                track: file,
        ...
        .filter((row) => row.trackFileId && row.filePath);
    return {
        ratingKey: `lidarr:${instance.id}:${record.id}`,
        entityId: record.id,
        artistId: record.artistId ?? record.artist?.id ?? null,
        arrType: 'lidarr',
        arrInstanceId: instance.id,
        arrInstanceName: instance.name,
        libraryKey: library.libraryKey,
        ...
        hasFile: !!record.anyReleaseHasFile || !!primary || trackFileRows.length > 0
            || Number(record.statistics?.trackFileCount || 0) > 0,
        path: record.path || null,
        trackFileId: Number(primary?.id || 0) || null,
        filePath: primary?.path || (trackFileRows[0]?.filePath || null),
        expectedRuntimeSec: trackFileRows[0]?.expectedRuntimeSec
            ?? extractExpectedRuntimeSec({ file: primary, record, track: primary, mediaKind: 'audio' }),
        trackFiles: trackFileRows,
        ...snap,
        ...
        seasons: null,
        episodeCount: trackFileRows.length || Number(record.statistics?.trackFileCount || 0) || null,
        seasonFloorResolution: snap.videoResolution,
    };
};
