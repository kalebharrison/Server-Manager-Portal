...
export const buildRadarrIndexItem = (instance, record, movieFile = null, profile = null) => {
...
export const buildSonarrIndexItem = (instance, record, episodeFiles = [], episodes = [], profile = null) => {
    ...
    if (!episodeRows.length) {
        for (const file of (Array.isArray(episodeFiles) ? episodeFiles : [])) {
            const snap = fileSnapshot(file, profile);
            const seasonNumber = Number(file.seasonNumber);
    ...
    // Last resort: Sonarr series statistics (keeps library eligible even if file payloads failed).
