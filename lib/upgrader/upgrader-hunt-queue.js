...
export const libraryLabelForItem = (item = {}) => {
    if (item.arrType === 'lidarr' || String(item.libraryKey || '').startsWith('lidarr:')) {
        return 'Music';
    }
    const label = item.libraryName
        ...
        || (item.arrType === 'radarr' ? 'Radarr'
            : item.arrType === 'sonarr' ? 'Sonarr'
                : 'Library');
    // Stale index stamps / root basenames often say Lidarr or Artists.
    if (/^(lidarr|artists?|music)$/i.test(String(label))) return 'Music';
    return label;
};
