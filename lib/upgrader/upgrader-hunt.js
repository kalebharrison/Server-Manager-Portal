...
export const createUpgraderHunt = ({
    ...
    const runHunt = async (config, { limit = null, dryRun = false } = {}) => {
        ...
        // Live: one candidate per library unless explicitly raised. The old
        // ceil(budget/2) default planned ~5–12 Arr interactive searches per
        // library per 20m cycle; GET /api/v3/release loads every indexer hit
        // into Radarr/Sonarr RAM and OOM'd unraid01 (Radarr ~28GB, 2026-09-21).
        const maxPerLibrary = dryRun
            ...
            : Math.max(
                ...
                Math.min(
                    ...
                    Number(config.upgraderMaxSearchesPerLibrary) || 1,
                ),
            );

        const maxSearchesPerCycle = dryRun
            ? Number.POSITIVE_INFINITY
            : Math.max(1, Number(config.upgraderMaxSearchesPerCycle) || 2);
        ...
        for (const item of planned.queue) {
            ...
            if (!dryRun && searched >= maxSearchesPerCycle) break;
        ...
        return {
            ...
            libraries: [...libraryStats.values()]
                ...
                .sort((a, b) => a.label.localeCompare(b.label)),
            maxPerLibrary: planned.maxPerLibrary,
            maxSearchesPerCycle: dryRun ? null : maxSearchesPerCycle,
        };
    };
