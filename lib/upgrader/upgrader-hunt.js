...
export const createUpgraderHunt = ({
    ...
    const runHunt = async (config, { limit = null, dryRun = false } = {}) => {
        ...
        const maxPerLibrary = dryRun
            ? Math.max(1, Math.min(2, Number(config.upgraderDryRunPerLibrary) || 2))
            : Math.max(
                2,
                Number(config.upgraderMaxSearchesPerLibrary) || Math.max(3, Math.ceil(budget / 2)),
            );
        const dryRunSearchCap = dryRun
            ? Math.max(1, Math.min(12, Number(limit) || Number(config.upgraderDryRunLimit) || 8))
            : null;
        ...
        if (!planned.queue.length) {
            return {
                ...
                reason: 'No eligible titles in the hunt queue (empty index, all excluded/snoozed, or no files on disk).',
            };
        }
        ...
        for (const item of planned.queue) {
            ...
            if (dryRun && searched >= dryRunSearchCap) break;
