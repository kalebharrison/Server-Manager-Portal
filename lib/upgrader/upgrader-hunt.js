...
const pickAcquireRelease = (releases = []) => {
    const allowed = (Array.isArray(releases) ? releases : []).filter((release) => (
        release?.rejected !== true && release?.downloadAllowed !== false
    ));
...
export const createUpgraderHunt = ({
    ...
    const fetchReleases = async (instance, item, { seasonNumber = null } = {}) => {
        if (item.arrType === 'lidarr') {
            return request(instance, `/api/v1/release?albumId=${encodeURIComponent(item.entityId)}`);
        }
    ...
    const previewMissingItem = async (config, item) => {
        ...
        try {
            const releases = await fetchReleases(instance, item);
    ...
    const runHunt = async (config, { limit = null, dryRun = false } = {}) => {
        ...
        if (!planned.queue.length) {
            return {
                ...
                reason: 'No eligible titles in the hunt queue (empty index, all excluded/snoozed, or nothing missing/upgradeable). Refresh index if Sonarr/Radarr looks empty here.',
            };
        }
