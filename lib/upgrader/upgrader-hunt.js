...
export const createUpgraderHunt = ({
    ...
    const runHunt = async (config, { limit = null, dryRun = false } = {}) => {
        ...
        for (const item of indexItems) {
            const key = `${item.arrType || 'arr'}:${item.arrInstanceId || ''}`;
            if (!libraryStats.has(key)) {
                libraryStats.set(key, {
                    key,
                    label: item.arrInstanceName || (item.arrType === 'radarr' ? 'Radarr' : 'Sonarr'),
                    indexed: 0,
                    withFiles: 0,
                    available: 0,
                    considered: 0,
        ...
        if (!planned.queue.length) {
            return {
                ...
                searched: 0,
                skipped: 0,
                libraries: [...libraryStats.values()].sort((a, b) => a.label.localeCompare(b.label)),
