    ...
    const collectSnapshot = async (config) => {
        ...
        const attachStrikes = (item, strikeEligible) => {
            ...
            const state = nextStrikeState({
                ...
                now,
                gapMs: strikeGapMsForReason(item.reason, thresholds),
                maxStrikes,
                award: false,
            });
            item.strikeEligible = strikeEligible;
            item.strikes = state.count;
            item.maxStrikes = maxStrikes;
        ...
        const orphans = findOrphans({
            ...
            const base = {
                ...
                safetyHold: withinGrace ? 'orphanGrace' : null,
            };
            const strikeEligible = !withinGrace
                && !base.snoozed
                && !base.seedingProtected
                && !base.condemned;
        ...
        for (const item of candidates) {
            ...
            try {
                ...
                if (!forceKill) {
                    ...
                    result.strikes = Math.max(strikeState.count, maxStrikes);
                    result.maxStrikes = maxStrikes;
                    result.forced = true;
                }
