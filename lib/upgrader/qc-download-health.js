...
export const createQcDownloadHealth = ({
    ...
    const fetchClientItems = async (config) => {
        ...
        return { clientItems: [...byId.values()], clients, torrents, sabQueue, sabHistory };
    };
    ...
    const collectSnapshot = async (config) => {
        ...
        const { clientItems, clients } = clientBundle;
    ...
    const planAction = (item, prefs, thresholds, now) => {
        const researchKey = item.title || item.key;
    ...
    const removeFromArr = async (config, item) => {
        ...
        await request(
            instance,
            `/api/${version}/queue/${encodeURIComponent(item.arrQueueId)}?removeFromClient=true&blocklist=true`,
            { method: 'DELETE' },
        );
        return true;
    };
    ...
    const tryResearch = async (config, item, prefs, thresholds) => {
        ...
        if (isResearchThrottled(prefs, item.title || item.key)) return false;
        const instance = findArrInstance(config, item);
        ...
        for (const item of candidates) {
            ...
            try {
                ...
                if (plan.research) {
                    ...
                    await setResearchCooldown(item.title || item.key, thresholds.researchThrottleHours, now);
                }
    ...
    const applyExtensionPolicy = async (config, extensions = []) => {
        ...
        await appendAudit({ action: 'qc_extension_policy', extensions: normalized });
        return results;
    };
