...
import { buildQcCleanupDigest } from './qc-cleanup-digest.js';
import {
    QC_REASONS,
    classifyQueueItem,
...
import { isFakeRemuxContainer } from './upgrader-quality.js';
...
export const createQcDownloadHealth = ({
    ...
    const fetchRecentArrImports = async (config, { now = Date.now(), sinceMs = 24 * HOUR_MS } = {}) => {
        const cutoff = now - Math.max(0, Number(sinceMs) || 0);
        const imports = [];
        const types = ['sonarr', 'radarr', 'lidarr'];
        for (const type of types) {
            const instances = getReadyArrInstances(config, type);
            ...
            for (const instance of instances) {
                try {
                    const payload = await request(
                        instance,
                        `/api/${version}/history?page=1&pageSize=100&sortKey=date&sortDirection=descending&${historyQueryFor(type)}`,
                    );
                    for (const record of recordsOf(payload)) {
                        const at = Date.parse(record?.date || '');
                        if (Number.isFinite(at) && at < cutoff) continue;
                        if (!isSuccessfulImportEvent(record?.eventType)) continue;
                        imports.push({
                            downloadId: record.downloadId || null,
                            title: record.sourceTitle || record.title || '',
                            name: record.sourceTitle || record.title || '',
                        });
                    }
                } catch (error) {
                    log(`[qc] ${instance.name || type} history failed: ${error.message}`);
                }
            }
        }
        return imports;
    ...
    const buildSnapshot = async (config) => {
        ...
        const [arrItems, clientBundle, blockedExtensions, importedItems] = await Promise.all([
            fetchArrQueues(config),
            fetchClientItems(config),
            loadBlockedExtensions(config),
            fetchRecentArrImports(config, { now }),
        ]);
        ...
        const orphanRows = findOrphans({
            ...
            arrDownloadIds,
            arrItems,
            importedDownloadIds: importedItems.map((item) => item.downloadId).filter(Boolean),
            importedItems,
            now,
        ...
        const orphans = orphanRows.map((orphan, index) => {
            ...
            const fakeRemux = isFakeRemuxContainer({ title: orphan.name });
            const reason = orphanJunkFlags[index]
                ? QC_REASONS.blockedExtension
                : fakeRemux
                    ? QC_REASONS.fakeRemux
                    : orphan.reason;
            const instantKill = reason === QC_REASONS.blockedExtension || reason === QC_REASONS.fakeRemux;
            const base = {
                ...orphan,
                key,
                reason,
                ...
                safetyHold: withinGrace && !instantKill ? 'orphanGrace' : null,
                ageMs: clientItemAgeMs(orphan, now),
            };
            const strikeEligible = (instantKill || !withinGrace)
                && !base.snoozed
                && !base.condemned;
    ...
    const runCleanup = async (config, { dryRun = false, itemKeys = null } = {}) => {
        ...
        for (const item of candidates) {
            ...
            const result = {
                ...
                client: item.client?.client || item.client || null,
                success: false,
                plan,
                errors: [],
            };
    ...
    const postDigestIfEnabled = async (config, results) => {
        ...
        const notifier = typeof getDiscordNotifier === 'function'
            ? getDiscordNotifier()
            : getDiscordNotifier;
        if (!notifier?.postEvent && !notifier?.postAdminEvent) return false;
        const payload = buildQcCleanupDigest(results);
