...
export const thresholdsFromConfig = (config = {}) => ({
    // Per-strike windows (default maxStrikes=3 ⇒ totals match classic 30m / 6h / 60m / 45m).
    metaDlMinutes: asNumber(config.qcMetaDlMinutes, 20),
    stalledHours: asNumber(config.qcStalledHours, 2),
    completedNotImportingMinutes: asNumber(config.qcCompletedNotImportingMinutes, 20),
...
export const strikeGapMsForReason = (reason, thresholds = thresholdsFromConfig()) => {
    ...
    if (reason === QC_REASONS.completedNotImporting) {
        return Math.max(MINUTE_MS, (Number(thresholds.completedNotImportingMinutes) || 20) * MINUTE_MS);
    }
 ...
 * actively copying — Radarr/Sonarr serialize large remux imports.
 */
export const isImportPendingBehindActiveImport = (arrItem, peerArrItems = []) => {
    ...
    return peers.some((peer) => {
        ...
        if (String(peer.arrQueueId ?? peer.id) === String(arrItem.arrQueueId ?? arrItem.id)) return false;
        return isActivelyImporting(peer);
    });
};
...
export const classifyQueueItem = ({
    ...
    if (
        isCompletedNotImporting(arrItem)
        && completedAgeMs({ arrItem, clientItem, now }) >= thresholds.completedNotImportingMinutes * MINUTE_MS
    ) {
        // Still in line behind an active remux copy — not stuck.
