...
export const findOrphans = ({
    ...
    const known = new Set(
        [...(arrDownloadIds || []), ...(arrItems || []).map((item) => item?.downloadId)]
    ...
    return (clientItems || [])
        .filter((item) => {
            ...
            // imports (downloadId still in queue) and recent hunt grabs are held.
            ...
            if ((arrItems || []).some((arrItem) => arrItemMatchesClientRelease(arrItem, item))) return false;
            return true;
        })
        .map((item) => {
