...
const isStalledState = ({ arrItem, clientItem }) => {
    const state = String(clientItem?.state || '').toLowerCase();
    if (state === 'stalleddl' || state === 'stalled' || state === 'error') return true;
    ...
    if (trackedState === 'downloadclientunavailable') return true;
    if (clientItem?.dlspeed === 0 && progressOf({ arrItem, clientItem }) < 1 && state.includes('dl')) {
        return /stall|no seeds|unavailable/.test(statusMessagesText(arrItem)) || state.includes('stalled');
    }
    return false;
};
