...
const isActiveIncompleteDownload = (item) => {
    if (!item) return false;
    if (isSeedingProtected(item)) return false;
    const progress = Number(item.progress);
    if (Number.isFinite(progress) && progress >= 1) return false;
    const state = String(item.state || '').toLowerCase();
    if (state === 'pauseddl' || state === 'paused' || state === 'pausedup') return false;
    if (state === 'uploading' || state === 'stalledup' || state === 'forcedup' || state === 'queuedup') return false;
    if (state === 'completed' || state === 'checking' || state === 'moving') return false;
    return true;
};

/**
 * True when ≥2 active incomplete downloads on a client (or set) all show zero download speed —
 * typical ISP / WAN outage rather than one dead torrent.
 */
export const isGlobalStallOutage = (clientItems = []) => {
    const active = (clientItems || []).filter(isActiveIncompleteDownload);
    if (active.length < 2) return false;
    return !active.some((item) => Number(item.dlspeed) > 0);
};
...
export const isStallActionable = ({
    ...
    if (clientName === 'qbit') {
        if (qbitConfigured && !clients?.qbit) return false;
        if (clients?.qbit && isGlobalStallOutage(clientItems.filter((item) => item.client === 'qbit'))) {
            return false;
        }
    } else if (clientName === 'sab') {
        if (sabConfigured && !clients?.sab) return false;
        if (clients?.sab && isGlobalStallOutage(clientItems.filter((item) => item.client === 'sab'))) {
            return false;
        }
    } else if (anyConfigured && !anyHealthy) {
        return false;
    } else if (clients?.qbit && isGlobalStallOutage(clientItems.filter((item) => item.client === 'qbit'))) {
        // Unmatched Arr row while qBit looks fully stalled — hold.
        return false;
    } else if (clients?.sab && isGlobalStallOutage(clientItems.filter((item) => item.client === 'sab'))) {
        return false;
    }
...
export const isReasonActionable = (item = {}, context = {}) => {
    ...
    if (reason === QC_REASONS.stalled) {
        return isStallActionable({
            ...
            clientItems: context.clientItems,
...
export const findOrphans = ({ clientItems = [], arrDownloadIds = [] } = {}) => {
    const known = new Set(
        (arrDownloadIds || [])
