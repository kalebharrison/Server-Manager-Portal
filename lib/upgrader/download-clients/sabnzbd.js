...
export const createSabClient = ({
    ...
    const getConfigSection = async (config, section) => {
        const payload = await api(config, 'get_config', { section });
    ...
    const getBlockedExtensions = async (config) => {
        ...
        const raw = misc?.ext_blacklist
            ?? misc?.cleanup_list
            ?? misc?.ext_blacklist_list
            ?? misc?.misc?.ext_blacklist
            ?? '';
    ...
    const setBlockedExtensions = async (config, extensions = []) => {
        ...
        // SABnzbd accepts set_config with section/keyword/value.
        await api(config, 'set_config', {
            section: 'misc',
            keyword: 'ext_blacklist',
            value,
        });
        return normalized;
    };
