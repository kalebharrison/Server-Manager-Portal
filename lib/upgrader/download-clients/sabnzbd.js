...
/** SAB fullstatus.dnslookup is a boolean on current versions; older builds used "OK"/"Failed". */
export const interpretSabDnsLookup = (value) => {
    if (value === true || value === 1) return { failed: false, label: 'OK' };
    if (value === false || value === 0) return { failed: true, label: 'Failed' };
    const text = String(value ?? '').trim();
    if (!text) return { failed: false, label: null };
    const lower = text.toLowerCase();
    if (lower === 'ok' || lower === 'true' || lower === 'yes' || lower === '1') {
        return { failed: false, label: 'OK' };
    }
    if (lower === 'failed' || lower === 'false' || lower === 'no' || lower === '0' || lower === 'fail') {
        return { failed: true, label: text };
    }
    // Hostname / other non-empty strings mean lookup succeeded.
    return { failed: false, label: text };
};
...
export const createSabClient = ({
    ...
    const listJobFiles = async (config, nzoId) => {
        if (!nzoId) return [];
        const payload = await api(config, 'get_files', { value: String(nzoId) });
        const rows = payload?.files || payload?.result || payload;
        const list = Array.isArray(rows) ? rows : [];
        return list
            .map((row) => String(row?.filename || row?.name || '').trim())
            .filter(Boolean);
    };
    ...
    const getNetworkHealth = async (config) => {
        ...
        try {
            ...
            const root = payload?.status && typeof payload.status === 'object' && !Array.isArray(payload.status)
                ? payload.status
                : payload;
            const dnslookup = interpretSabDnsLookup(root?.dnslookup);
    ...
    const setMiscValue = async (config, keyword, value) => {
        const payload = await api(config, 'set_config', {
            section: 'misc',
        ...
        if (payload?.status === false) {
            throw new Error(payload?.error || `SABnzbd set_config failed for ${keyword}`);
        }
        return payload?.config?.misc?.[keyword] ?? value;
    };

    const getDupeSettings = async (config) => {
        if (!isConfigured(config)) return null;
        const misc = await getConfigSection(config, 'misc');
        return {
            no_dupes: Number(misc?.no_dupes),
            no_smart_dupes: Number(misc?.no_smart_dupes),
            no_series_dupes: Number(misc?.no_series_dupes),
            dupes_propercheck: Number(misc?.dupes_propercheck),
            warn_dupl_jobs: Number(misc?.warn_dupl_jobs),
        };
    };
    ...
    const applyQcAlignment = async (config, recommended = {}) => {
        if (!isConfigured(config)) return null;
        const target = {
            no_dupes: recommended.no_dupes ?? 0,
            no_smart_dupes: recommended.no_smart_dupes ?? 0,
            no_series_dupes: recommended.no_series_dupes ?? 0,
        };
        for (const [keyword, value] of Object.entries(target)) {
            await setMiscValue(config, keyword, value);
        }
        return getDupeSettings(config);
    };
