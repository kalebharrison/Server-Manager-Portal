...
export const createQbitClient = ({
    ...
    const listTorrents = async (config) => {
        ...
        return (Array.isArray(rows) ? rows : []).map((row) => {
            ...
            const infohashV1 = String(row?.infohash_v1 || '').toLowerCase();
            const infohashV2 = String(row?.infohash_v2 || '').toLowerCase();
            return {
                id: hash,
                hash,
                infohashV1: infohashV1 || null,
                infohashV2: infohashV2 || null,
    ...
    const listTorrentFiles = async (config, hash) => {
        if (!hash) return [];
        await ensureSession(config);
        const response = await request(config, `/api/v2/torrents/files?hash=${encodeURIComponent(String(hash))}`);
        if (!response.ok) {
            const detail = await response.text().catch(() => '');
            throw new Error(`qBittorrent torrents/files failed (${response.status}${detail ? `: ${detail.slice(0, 160)}` : ''})`);
        }
        const rows = await response.json().catch(() => []);
    ...
    const getNetworkHealth = async (config, { incompleteCount = 0 } = {}) => {
        ...
        try {
            ...
            return {
                ...
                connectionStatus: status || null,
                dhtNodes: Number.isFinite(dhtNodes) ? dhtNodes : null,
                dlSpeed: Number(info?.dl_info_speed) || 0,
                upSpeed: Number(info?.up_info_speed) || 0,
                reason: null,
            };
        } catch (error) {
            return {
    ...
    const applyQcAlignment = async (config, recommended = {}) => {
        ...
        if (recommended.max_ratio != null) {
            patch.max_ratio = Number(recommended.max_ratio);
        }
