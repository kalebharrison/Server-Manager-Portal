...
import {
    RECENT_IMPORT_CATCHUP_DEBOUNCE_MS,
    RECENT_IMPORT_CATCHUP_MIN_INTERVAL_MS,
...
import { shouldAutoReplaceFinding } from './qc-integrity-severity.js';
...
/**
 * Which Integrity check a finding belongs to (playability / trim / fingerprint / hash).
 * Used so a pass of one check does not wipe findings from another.
 */
export const integrityFindingCheck = (finding = {}) => {
    const stored = String(finding?.mode || '').toLowerCase();
    if (['playability', 'imohash', 'xxhash', 'trim'].includes(stored)) return stored;
    if (stored === 'baseline') return 'playability';
...
/** True when a successful scan/recheck of `passedMode` should clear this finding. */
export const passedModeClearsFinding = (passedMode, finding = {}) => {
    const mode = String(passedMode || '').toLowerCase();
    if (!mode) return true; // explicit single-key clear (import / stale drop) with no mode filter
    const check = integrityFindingCheck(finding);
    if (mode === 'baseline') return check === 'playability' || check === 'imohash';
    return mode === check;
};
 ...
 * - Drops findings that passed on a later recheck of the *same* check family.
 * - Drops keys in `clearedKeys` regardless of mode (stale Arr file id / gone from index).
 * - Merges newly discovered findings.
 *
 * `passedMode` scopes passedKeys: an imohash pass must not wipe playability
 * duration_mismatch rows (and vice versa). Omit passedMode only for deliberate
 * full clears of a key (successful import baseline).
 */
export const reconcileIntegrityFindings = (existing = [], {
    ...
    const cleared = toKeySet(clearedKeys);
    const kept = [];
    for (const finding of (Array.isArray(existing) ? existing : [])) {
        if (!finding?.key) continue;
        if (shouldIgnoreRuntimeFinding(finding)) continue;
        const key = String(finding.key);
        if (live && !live.has(key)) continue;
        if (cleared && cleared.has(key)) continue;
        if (passed && passed.has(key) && passedModeClearsFinding(passedMode, finding)) continue;
        kept.push(finding);
    }
    const mergedIncoming = (Array.isArray(incoming) ? incoming : [])
        .filter((finding) => !shouldIgnoreRuntimeFinding(finding));
...
export const createQcIntegrity = ({
    ...
    let progress = null;
    let importCatchUpTimer = null;
    /** Filled when startIntegrityJob wires the forced catch-up debounce. */
    let scheduleImportCatchUpImpl = null;
    const scheduleImportCatchUp = (opts = {}) => {
        if (typeof scheduleImportCatchUpImpl === 'function') {
            scheduleImportCatchUpImpl(opts);
            return;
        }
        // Job not started yet — still debounce once startIntegrityJob wires the impl.
        const wait = Math.max(5_000, Number(opts.delayMs) || RECENT_IMPORT_CATCHUP_DEBOUNCE_MS);
        if (importCatchUpTimer) clearTimeout(importCatchUpTimer);
        importCatchUpTimer = setTimeout(() => {
            importCatchUpTimer = null;
            if (typeof scheduleImportCatchUpImpl === 'function') {
                scheduleImportCatchUpImpl({ delayMs: 5_000 });
            }
        }, wait);
    };
    ...
    const normalizeReleaseTitle = (value) => String(value || '')
        .toLowerCase()
        .replace(/[.\s_\-]+/g, '.')
        .replace(/[^a-z0-9.]/g, '');

    const releaseTitlesMatch = (a, b) => {
    ...
    const historyRecords = (payload) => (
        Array.isArray(payload?.records) ? payload.records : (Array.isArray(payload) ? payload : [])
    );

    const markHistoryFailed = async (instance, base, records) => {
        let count = 0;
    ...
    const blocklistRelease = async (config, candidate) => {
        ...
        const sourceTitle = candidate.sourceTitle || candidate.sceneName || candidate.title || null;
        try {
            // Arr has no POST /blocklist. Releases enter the blocklist via
            // history/failed or queue delete?blocklist=true.
            if (candidate.downloadId) {
                ...
                const matches = historyRecords(history).filter((entry) => (
                    String(entry?.downloadId || '').toLowerCase() === String(candidate.downloadId).toLowerCase()
                ));
                if (matches.length) {
                    const count = await markHistoryFailed(instance, base, matches);
                    await appendAudit({
                    ...
                    return { success: true, blocked: true, via: 'history_failed', count };
                }
            }

            if (sourceTitle && candidate.entityId != null && candidate.entityId !== '') {
                const qs = new URLSearchParams({
                    pageSize: '50',
                    sortKey: 'date',
                    sortDirection: 'descending',
                });
                if (candidate.arrType === 'radarr') qs.set('movieIds', String(candidate.entityId));
                else if (candidate.arrType === 'sonarr') qs.set('seriesIds', String(candidate.entityId));
                else if (candidate.arrType === 'lidarr') {
                    const artistId = candidate.artistId ?? candidate.artist?.id ?? null;
                    if (artistId != null) qs.set('artistIds', String(artistId));
                }
                const history = await request(instance, `${base}/history?${qs.toString()}`);
                const matches = historyRecords(history).filter((entry) => {
                    if (!releaseTitlesMatch(entry?.sourceTitle, sourceTitle)) return false;
                    const event = String(entry?.eventType || '').toLowerCase();
                    return !event || event === 'grabbed' || event.includes('grab');
                });
                if (matches.length) {
                    const count = await markHistoryFailed(instance, base, matches.slice(0, 10));
                    await appendAudit({
                        action: 'qc_integrity_blocklist',
                        success: true,
                        ratingKey: candidate.ratingKey,
                        ...
                        sourceTitle,
                        arrType: candidate.arrType,
                        via: 'history_failed_title',
                    });
                    return { success: true, blocked: true, via: 'history_failed_title', count };
                }
            }

            if (sourceTitle) {
                const queueQs = new URLSearchParams({ pageSize: '100' });
                if (candidate.arrType === 'radarr' && candidate.entityId != null) {
                    queueQs.set('movieIds', String(candidate.entityId));
                } else if (candidate.arrType === 'sonarr' && candidate.entityId != null) {
                    queueQs.set('seriesIds', String(candidate.entityId));
                }
                const queue = await request(instance, `${base}/queue?${queueQs.toString()}`);
                const queueItems = historyRecords(queue).filter((item) => (
                    releaseTitlesMatch(item?.title || item?.sourceTitle, sourceTitle)
                    || (candidate.entityId != null && (
                        Number(item?.movieId) === Number(candidate.entityId)
                        || Number(item?.seriesId) === Number(candidate.entityId)
                    ) && releaseTitlesMatch(item?.title, sourceTitle))
                ));
                if (queueItems.length) {
                    for (const item of queueItems) {
                        ...
                        await request(
                            instance,
                            `${base}/queue/${encodeURIComponent(item.id)}?${delQs.toString()}`,
                            { method: 'DELETE' },
                        );
                    }
                    await appendAudit({
            ...
            const reason = sourceTitle
                ? `No Arr history/queue match to blocklist for ${sourceTitle}`
                : 'No downloadId or sourceTitle available to blocklist';
            await appendAudit({
                action: 'qc_integrity_blocklist',
                success: false,
                ratingKey: candidate.ratingKey,
    ...
    const runPlayability = async (config, candidate, localPath, st, priorEntry = null) => {
        ...
        const catalogRuntimeSec = candidate.expectedRuntimeSec ?? null;
        const isMovie = String(candidate.mediaType || '').toLowerCase() === 'movie';
        const alternateCut = isMovie && isLikelyAlternateMovieCut({
            filePath: candidate.filePath,
            title: candidate.title,
            qualityName: candidate.qualityName,
            sourceTitle: candidate.sourceTitle,
        });
        const expectedRuntimeSec = isMovie
            ? resolveMovieExpectedRuntimeSec({
                catalogSec: catalogRuntimeSec,
                priorDurationSec: priorEntry?.durationSec,
                priorExpectedRuntimeSec: priorEntry?.expectedRuntimeSec,
            })
            : catalogRuntimeSec;
        const durationCheck = durationMatchesExpected(probe.durationSec, expectedRuntimeSec, {
            mediaKind,
            mediaType: candidate.mediaType || null,
            alternateCut,
            specialEpisode: isSpecialEpisode(candidate),
        ...
        const storedExpectedRuntimeSec = isMovie
            ? resolveMovieExpectedRuntimeSec({
                catalogSec: catalogRuntimeSec,
                priorDurationSec: probe.durationSec,
                priorExpectedRuntimeSec: expectedRuntimeSec,
            })
            : expectedRuntimeSec;
        ...
        return {
            ...
            reason: null,
            durationSec: probe.durationSec,
            expectedRuntimeSec: storedExpectedRuntimeSec,
            playabilityOk: true,
            size: st.size,
            mtimeMs: st.mtimeMs,
        };
    ...
    const buildCacheEntry = (candidate, localPath, st, extras = {}) => ({
        ...
        runtimeAcceptedAt: extras.runtimeAcceptedAt ?? null,
    ...
    const validateCandidate = async (config, candidate, {
        ...
        if (mode === 'playability') {
            const play = await runPlayability(config, candidate, localPath, st, prior);
            if (!play.ok) {
                return {
                    ...candidate,
                    localPath,
        ...
        if (mode === 'trim') {
            ...
            const prefsForNative = await loadPrefs();
            const nativeOverride = prefsForNative?.integrityNativeOverrides?.[candidate.key] || null;
            ...
            const playAfterTrim = await runPlayability(config, candidate, localPath, liveSt, prior);
            if (!playAfterTrim.ok) {
                return {
                    ...candidate,
                    localPath,
    ...
    const enrichFindingForReplace = async (finding = {}) => {
        let next = { ...(finding && typeof finding === 'object' ? finding : {}) };
        const findingKey = String(next.key || '').trim();

        if (findingKey) {
            try {
                const prefs = await loadPrefs();
                const stored = (prefs.integrityFindings || [])
                    .find((entry) => String(entry?.key) === findingKey);
                if (stored) {
                    next = {
                        ...stored,
                        ...next,
                        movieFileId: next.movieFileId || stored.movieFileId || null,
                        episodeFileId: next.episodeFileId || stored.episodeFileId || null,
                        trackFileId: next.trackFileId || stored.trackFileId || null,
                        episodeId: next.episodeId || stored.episodeId || null,
                        entityId: next.entityId ?? stored.entityId ?? null,
                        arrInstanceId: next.arrInstanceId || stored.arrInstanceId || null,
                        arrType: next.arrType || stored.arrType || null,
                        ratingKey: next.ratingKey || stored.ratingKey || null,
                        filePath: next.filePath || stored.filePath || stored.localPath || null,
                    };
                }
            } catch {
                // Prefs optional — still try key / index enrichment.
            }
        }

        const fileIdMatch = String(next.key || '').match(/:file:(\d+)\s*$/);
        if (fileIdMatch) {
            const fileId = Number(fileIdMatch[1]);
            if (Number.isFinite(fileId) && fileId > 0) {
                if (next.arrType === 'radarr' && !next.movieFileId) next.movieFileId = fileId;
                else if (next.arrType === 'sonarr' && !next.episodeFileId) next.episodeFileId = fileId;
                else if (next.arrType === 'lidarr' && !next.trackFileId) next.trackFileId = fileId;
                else if (!next.arrType && !next.movieFileId && !next.episodeFileId && !next.trackFileId) {
                    next.movieFileId = fileId;
                }
            }
        }

        const ratingKey = String(next.ratingKey || next.key || '');
        const rkMatch = ratingKey.match(/^(radarr|sonarr|lidarr):([^:]+):(\d+)/i);
        if (rkMatch) {
            if (!next.arrType) next.arrType = rkMatch[1].toLowerCase();
            if (!next.arrInstanceId) next.arrInstanceId = rkMatch[2];
            if (next.entityId == null || next.entityId === '') {
                const entityId = Number(rkMatch[3]);
                if (Number.isFinite(entityId) && entityId > 0) next.entityId = entityId;
            }
        }

        const needsIndex = !next.arrInstanceId
            || next.entityId == null
            || (next.arrType === 'radarr' && !next.movieFileId)
            || (next.arrType === 'sonarr' && !next.episodeFileId)
            || (next.arrType === 'lidarr' && !next.trackFileId);
        if (needsIndex && typeof loadIndex === 'function') {
            try {
                const index = await loadIndex();
                const items = Array.isArray(index?.items) ? index.items : [];
                const candidates = collectIntegrityCandidates(items, { includeMusic: true });
                const hit = candidates.find((entry) => (
                    (findingKey && String(entry.key) === findingKey)
                    || (next.filePath && normalizePath(entry.filePath) === normalizePath(next.filePath))
                    || (next.localPath && normalizePath(entry.filePath) === normalizePath(next.localPath))
                )) || null;
                if (hit) {
                    next = {
                        ...hit,
                        ...next,
                        key: next.key || hit.key,
                        title: next.title || hit.title,
                        movieFileId: next.movieFileId || hit.movieFileId || null,
                        episodeFileId: next.episodeFileId || hit.episodeFileId || null,
                        trackFileId: next.trackFileId || hit.trackFileId || null,
                        episodeId: next.episodeId || hit.episodeId || null,
                        entityId: next.entityId ?? hit.entityId ?? null,
                        arrInstanceId: next.arrInstanceId || hit.arrInstanceId || null,
                        arrType: next.arrType || hit.arrType || null,
                        ratingKey: next.ratingKey || hit.ratingKey || null,
                        filePath: next.filePath || hit.filePath || null,
                    };
                }
            } catch {
                // Index optional.
    ...
    /** Prefer scene/release name so Arr blocklist actually prevents the same NZB/torrent. */
    const resolveReplaceSourceTitle = async (instance, finding) => {
        const existing = String(finding?.sourceTitle || finding?.sceneName || '').trim();
        if (existing) return existing;
        if (!instance || typeof request !== 'function') return finding?.title || null;
        try {
            if (finding.arrType === 'radarr' && finding.movieFileId) {
                const file = await request(instance, `/api/v3/moviefile/${encodeURIComponent(finding.movieFileId)}`);
                return file?.sceneName || file?.relativePath || finding.title || null;
            }
            if (finding.arrType === 'sonarr' && finding.episodeFileId) {
                const file = await request(instance, `/api/v3/episodefile/${encodeURIComponent(finding.episodeFileId)}`);
                return file?.sceneName || file?.relativePath || finding.title || null;
            }
            if (finding.arrType === 'lidarr' && finding.trackFileId) {
                const file = await request(instance, `/api/v1/trackfile/${encodeURIComponent(finding.trackFileId)}`);
                return file?.sceneName || file?.relativePath || finding.title || null;
            }
        } catch {
            /* Arr lookup optional — fall through */
        }
        return finding?.title || null;
    };

    const replaceCorrupt = async (config, findingInput, { dryRun = false } = {}) => {
        const finding = await enrichFindingForReplace(findingInput);
        const instance = findInstance(config, finding);
        if (!instance) {
            return {
                success: false,
                reason: finding.arrInstanceId
                    ? `Arr instance not found (${finding.arrType || '?'}:${finding.arrInstanceId})`
                    : 'Arr instance not found (missing arrInstanceId on finding)',
        ...
        try {
            const sourceTitle = await resolveReplaceSourceTitle(instance, finding);
            const block = await blocklistRelease(config, {
                ...finding,
                sourceTitle,
            });

            if (finding.arrType === 'radarr') {
                if (!finding.movieFileId) return { success: false, reason: 'Missing movieFileId', key: finding.key };
                if (finding.entityId == null || finding.entityId === '') {
                    return { success: false, reason: 'Missing entityId (Radarr movie id)', key: finding.key };
                }
                ...
                await request(instance, '/api/v3/command', {
                    method: 'POST',
                    body: { name: 'MoviesSearch', movieIds: [Number(finding.entityId)] },
                });
            } else if (finding.arrType === 'lidarr') {
                if (!finding.trackFileId) return { success: false, reason: 'Missing trackFileId', key: finding.key };
                if (finding.entityId == null || finding.entityId === '') {
                    return { success: false, reason: 'Missing entityId (Lidarr album id)', key: finding.key };
                }
                ...
                await request(instance, '/api/v1/command', {
                    method: 'POST',
                    body: { name: 'AlbumSearch', albumIds: [Number(finding.entityId)] },
                });
            } else {
                if (!finding.episodeFileId) return { success: false, reason: 'Missing episodeFileId', key: finding.key };
                ...
                const command = finding.episodeId
                    ? { name: 'EpisodeSearch', episodeIds: [Number(finding.episodeId)] }
                    : { name: 'SeriesSearch', seriesId: Number(finding.entityId) };
                if (command.name === 'SeriesSearch' && (finding.entityId == null || finding.entityId === '')) {
                    return { success: false, reason: 'Missing entityId (Sonarr series id)', key: finding.key };
                }
            ...
            await appendAudit({
                ...
                sourceTitle,
                blocklisted: !!block?.blocked,
                blocklistVia: block?.via || null,
                blocklistReason: block?.success === false ? (block.reason || null) : null,
            });
            return {
                success: true,
                replaced: true,
    ...
    const resolveRecheckMode = (finding = {}) => {
        ...
        return integrityFindingCheck(finding);
    };
    ...
    const candidateFromFinding = (finding = {}) => ({
        ...
        qualityName: finding.qualityName || null,
    ...
    const acceptRuntimeFinding = async (config, { key = null, finding: findingInput = null } = {}) => {
        if (!config.upgraderEnabled || !config.qcIntegrityEnabled) {
            return { ok: false, cleared: false, reason: 'Integrity disabled' };
        }
        const prefs = await loadPrefs();
        ...
        const storedFinding = (prefs.integrityFindings || []).find((entry) => String(entry?.key) === findingKey)
            || (findingInput?.key ? findingInput : null);
        if (!storedFinding?.key) {
            return { ok: false, cleared: false, reason: 'Finding not found' };
        }
        if (String(storedFinding.reason || '') !== 'duration_mismatch') {
            return { ok: false, cleared: false, reason: 'Only runtime drift findings can accept measured length' };
        }

        const candidate = candidateFromFinding(storedFinding);
        const maps = Array.isArray(config.qcIntegrityPathMaps) ? config.qcIntegrityPathMaps : [];
        const resolved = await resolveMappedPath(candidate.filePath, maps, { config });
        if (!resolved.ok) {
            return { ok: false, cleared: false, reason: resolved.reason || 'missing_path', detail: resolved.detail || null };
        }
        const localPath = resolved.path;
        const st = await readStat(localPath);
        if (!st.ok) {
            return { ok: false, cleared: false, reason: st.reason || 'missing_file', detail: st.detail || null };
        }

        let durationSec = Number(storedFinding.durationSec);
        if (!Number.isFinite(durationSec) || durationSec <= 0) {
            const mediaKind = candidate.mediaKind === 'audio' ? 'audio' : 'video';
            const probe = await probeMediaFile(localPath, {
                requireAudio: mediaKind === 'audio' ? true : config.qcIntegrityRequireAudio !== false,
                mediaKind,
                timeoutMs: Math.max(1000, Number(config.qcIntegrityDecodeTimeoutMs) || DEFAULT_TIMEOUT_MS),
                execImpl,
            });
            if (!Number.isFinite(Number(probe.durationSec)) || Number(probe.durationSec) <= 0) {
                return { ok: false, cleared: false, reason: probe.reason || 'Could not probe file duration' };
            }
            durationSec = Number(probe.durationSec);
        }

        const catalogSec = storedFinding.expectedRuntimeSec ?? candidate.expectedRuntimeSec ?? null;
        const isMovie = String(candidate.mediaType || '').toLowerCase() === 'movie';
        const acceptedExpected = isMovie
            ? resolveMovieExpectedRuntimeSec({
                catalogSec,
                priorDurationSec: durationSec,
            })
            : durationSec;
        ...
        entries[findingKey] = buildCacheEntry(candidate, localPath, st, {
            ...(prior || {}),
            durationSec,
            expectedRuntimeSec: acceptedExpected,
            runtimeAcceptedAt: nowIso,
            playabilityAt: prior?.playabilityAt || nowIso,
            playabilityOk: true,
            ok: prior?.ok !== false,
            baselinedAt: prior?.baselinedAt || nowIso,
            baselineSource: prior?.baselineSource || 'runtime_accept',
        });
        ...
        prefs.integrityFindings = reconcileIntegrityFindings(prefs.integrityFindings || [], {
            clearedKeys: [findingKey],
            passedMode: 'playability',
        });
        await savePrefs(prefs);
        await appendAudit({
            action: 'qc_integrity_accept_runtime',
            success: true,
            cleared: true,
            key: findingKey,
            ...
            durationSec,
            expectedRuntimeSec: acceptedExpected,
            mediaType: storedFinding.mediaType || candidate.mediaType || null,
        });
    ...
    const setNativeLanguageFinding = async (config, {
        key = null,
        code = null,
        ...
        if (!normalized) {
            return { ok: false, cleared: false, reason: 'Invalid language code' };
        }

        const prefs = await loadPrefs();
        const storedFinding = (prefs.integrityFindings || []).find((entry) => String(entry?.key) === findingKey)
            || (findingInput?.key ? findingInput : null);
        const title = storedFinding?.title || findingInput?.title || null;
        const nowIso = new Date().toISOString();

        prefs.integrityNativeOverrides = {
            ...(prefs.integrityNativeOverrides && typeof prefs.integrityNativeOverrides === 'object'
                ? prefs.integrityNativeOverrides
                : {}),
            [findingKey]: {
                code: normalized,
        ...
        if (prior) {
            entries[findingKey] = {
                ...prior,
                trimNativeLang: normalized,
                trimNativeSource: 'manual',
            };
            await saveCache({
                updatedAt: nowIso,
                schemaVersion: SCHEMA_VERSION,
        ...
        await appendAudit({
            action: 'qc_integrity_set_native_language',
            success: true,
            cleared: true,
            key: findingKey,
            title,
    ...
    const scanIntegrity = async (config, {
        ...
        try {
            ...
            if (!cancelled) {
                await mapPool(batch, concurrency, async ({ candidate, prior }) => {
                    ...
                    if (trimPreviewHit) {
                        ...
                        passed += 1;
                        if (key) {
                            if (result.staleIndex) clearedKeys.add(String(key));
                            else passedKeys.add(String(key));
                        }
                        ...
                        if (findings.length > 2000) findings.splice(0, findings.length - 2000);
                        ...
                        if (liveReplace && !result?.softTimeout) {
                            ...
                            if (!breaker?.tripped && shouldAutoReplaceFinding(config, result)) {
            ...
            const mergedFindings = reconcileIntegrityFindings(freshPrefs.integrityFindings || [], {
                ...
                passedKeys,
                clearedKeys,
                // Only clear findings that this scan mode actually re-validated.
                // Nightly imohash must not wipe playability duration_mismatch rows.
                passedMode: scanMode,
                incoming: findings,
            }).slice(0, 2000);
        ...
        if (allowPlay) {
            ...
            if (play.ok && play.cacheEntry) {
                ...
                if (play.shouldBlocklist) {
                    ...
                    if (shouldAutoReplaceFinding(config, play) && !play.softTimeout) {
                ...
                await notifyIntegrityIssue(config, {
                    ...
                    description: `Hard fail on import/upgrade for **${play.title || 'a title'}**`
                        ...
                        + (shouldAutoReplaceFinding(config, play) ? ' — replace/search triggered' : '')
                        + '.',
                    finding: play,
    ...
    const catchUpRecentImports = async (config, {
        ...
        try {
            const includeMusic = config.qcIntegrityIncludeMusic !== false;
            const historyPayloads = await collectRecentImportPayloads(config, {
                request,
                now,
                sinceMs,
                includeMusic,
            ...
            const entries = cache?.entries && typeof cache.entries === 'object' ? { ...cache.entries } : {};
            const prefs = await loadPrefs();
            const softTimeoutKeys = new Set(
                (Array.isArray(prefs.integrityFindings) ? prefs.integrityFindings : [])
                    .filter((finding) => finding?.softTimeout && finding?.key)
                    .map((finding) => String(finding.key)),
            );

            const index = await loadIndexShared();
            const candidates = collectIntegrityCandidates(index.items || [], { includeMusic });
            const gapPayloads = collectCoverageGapPayloads(candidates, entries, {
                max: maxCoverageGaps,
            });

            const seen = new Set();
            const payloads = [];
            for (const payload of [...historyPayloads, ...gapPayloads]) {
            ...
            // Soft decode timeouts already have a finding — do not monopolize the
            // single playability slot every 10 minutes (Crow-style loops).
            const actionable = [];
            const deferredSoft = [];
            for (const payload of payloads) {
                if (softTimeoutKeys.has(String(payload.key))) deferredSoft.push(payload);
                else actionable.push(payload);
            }
            const ordered = deferredSoft.length && !actionable.length
                ? deferredSoft.slice(0, 1)
                : actionable;

            let skipped = 0;
            let ranImport = 0;
            let skippedSoftTimeout = deferredSoft.length && actionable.length ? deferredSoft.length : 0;
            for (const payload of ordered) {
                const key = payload?.key;
                const fromGap = !!payload.fromCoverageGap;
                const missing = fromGap
                    ? missingCoverageChecks(key ? entries[key] : null)
                    : missingImportChecks(key ? entries[key] : null, config, payload);
                if (!missing.length) {
                    skipped += 1;
                    continue;
                }
                if (missing.includes('playability') && !fromGap) {
                    await baselineImport(config, payload);
                } else if (fromGap) {
                    // File-id churn / wiped fingerprint: restore Coverage bars only.
                    let prior = key ? entries[key] : null;
                    const modes = missing.includes('playability')
                        ? ['playability', 'imohash']
                        : missing.filter((item) => item === 'imohash');
                    let aborted = false;
                    for (const mode of modes) {
                        ...
                        if (result?.cacheEntry && key) {
                            ...
                            entries[key] = result.cacheEntry;
                            continue;
                        }
                        if (!result?.ok && result?.reason) {
                            const nextPrefs = await loadPrefs();
                            nextPrefs.integrityFindings = reconcileIntegrityFindings(nextPrefs.integrityFindings || [], {
                                incoming: [{
                                    ...payload,
                                    ...result,
                                    key: result.key || key,
                                    softTimeout: !!result.softTimeout,
                                    shouldBlocklist: false,
                                    baselineSource: 'import',
                                }],
                            }).slice(0, 500);
                            invalidateIntegrityCoverage(nextPrefs);
            ...
            log(`[upgrader] integrity recent-import catch-up: ${payloads.length} in window`
                + ` (history=${historyPayloads.length} gaps=${gapPayloads.length}), filled ${ranImport}`
                + `, complete ${skipped}`
                + (skippedSoftTimeout ? `, deferredSoftTimeout ${skippedSoftTimeout}` : ''));
            return {
                ran: true,
                checked: payloads.length,
                ranImport,
                skipped,
                history: historyPayloads.length,
                gaps: gapPayloads.length,
                skippedSoftTimeout,
            };
        } catch (error) {
    ...
    const getStatus = async (config, opts = {}) => {
        ...
        let coverage;
        let findings = reconcileIntegrityFindings(priorFindings, {});
    ...
    const startIntegrityJob = (getConfig) => {
        ...
        const maybeCatchUpRecentImports = async ({ force = false } = {}) => {
            try {
                ...
                if (
                    !force
                    && lastRecentImportCatchupAt
                    && (now - lastRecentImportCatchupAt) < RECENT_IMPORT_CATCHUP_MIN_INTERVAL_MS
                ) {
                    return;
                }
        ...
        // Wire deferred catch-up scheduled from webhooks before startIntegrityJob ran.
        scheduleImportCatchUpImpl = ({ delayMs = RECENT_IMPORT_CATCHUP_DEBOUNCE_MS } = {}) => {
            const wait = Math.max(5_000, Number(delayMs) || RECENT_IMPORT_CATCHUP_DEBOUNCE_MS);
            if (importCatchUpTimer) clearTimeout(importCatchUpTimer);
            importCatchUpTimer = setTimeout(() => {
                importCatchUpTimer = null;
                void maybeCatchUpRecentImports({ force: true });
            }, wait);
        };
            ...
            try {
                ...
                for (const finding of mismatches) {
                    ...
                    if (replace && !escalated.ok && !escalated.softTimeout
                        && shouldAutoReplaceFinding(config, escalated)) {
                ...
                if (escalatedFindings.length) {
                    ...
                    prefs.integrityFindings = reconcileIntegrityFindings(prefs.integrityFindings || [], {
                        ...
                        passedKeys: mismatches
                            ...
                            .map((entry) => entry.key),
                        // Escalate re-runs playability → trim → fingerprint.
                        passedMode: 'baseline',
                    }).slice(0, 2000);
                    await savePrefs(prefs);
                    await notifyIntegrityIssue(config, {
    ...
    return {
        ...
        recheckFinding,
        acceptRuntimeFinding,
        setNativeLanguageFinding,
