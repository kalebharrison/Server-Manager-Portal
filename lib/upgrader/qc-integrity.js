...
export const createQcIntegrity = ({
    ...
    const validateCandidate = async (config, candidate, {
        ...
        if (mode === 'trim') {
            const trimCfgBase = resolveTrimConfig(config, candidate, { forceRewrite });
    ...
    const scanIntegrity = async (config, {
        ...
        try {
            ...
            for (const candidate of ordered) {
                ...
                if (scanMode === 'trim' && !isTrimVideoPath(candidate.filePath, candidate.mediaKind)) {
                    skipped += 1;
                    continue;
                }
            ...
            const trimPreview = scanMode === 'trim' ? {
                at: new Date().toISOString(),
                libraryKey: libraryFilter,
                dryRun: resolveTrimConfig(config).dryRun,
                summary: {
                    scanned,
                    skipped,
                    skippedPlaying,
    ...
    const startIntegrityJob = (getConfig) => {
        ...
        const trimTmpRootsFromConfig = (config) => {
            const maps = Array.isArray(config?.qcIntegrityPathMaps) ? config.qcIntegrityPathMaps : [];
            const roots = maps
                .map((entry) => String(entry?.to || entry?.containerPrefix || '').replace(/\/+$/, ''))
                .filter(Boolean);
            if (!roots.includes('/media')) roots.push('/media');
            return roots;
        };
