...
export const createQcIntegrity = ({
    ...
    const listIntegritySnoozes = async (now = Date.now()) => {
        ...
        try {
            ...
            /* index/cache optional for labels */
        }
        ...
        for (const [key, raw] of Object.entries(current)) {
            ...
            const title = resolveTitle(key, snoozeTitleValue(raw));
            if (!snoozeTitleValue(raw) && title) dirty = true;
            nextStored[key] = { until, title };
            rows.push({ key, until, title });
        }
        if (dirty || Object.keys(nextStored).length !== Object.keys(current).length) {
            prefs.integritySnoozed = nextStored;
            await savePrefs(prefs);
        } else {
            // Normalize legacy string until values in place when titles were already present.
            const needsNormalize = Object.values(current).some((raw) => typeof raw === 'string');
            if (needsNormalize) {
                prefs.integritySnoozed = nextStored;
                await savePrefs(prefs);
            }
