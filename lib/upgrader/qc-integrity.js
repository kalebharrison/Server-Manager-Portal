...
export const createQcIntegrity = ({
    ...
    const validateCandidate = async (config, candidate, {
        ...
        if (mode === 'trim') {
            ...
            const trimResult = await trimMkvFile(localPath, { ...trimCfg, execImpl, log });
    ...
    const scanIntegrity = async (config, {
        ...
        if (scanMode === 'trim' && !config.qcTrimEnabled) {
            if (_lockHeld) {
                scanning = false;
                progress = null;
            }
        ...
        try {
            ...
            const concurrency = (scanMode === 'playability' || scanMode === 'baseline' || scanMode === 'trim')
                ? playConcurrency
                : hashConcurrency;
    ...
    const baselineImport = async (config, payload = {}) => {
        ...
        if (trimCfg.enabled) {
            ...
            if (TRIM_SOFT_REASONS.has(trimOutcome.reason) || trimOutcome.reason === 'trim_pending') {
                await persistFinding({
                    ...candidate,
                    ...trimOutcome,
                    key: trimOutcome.key || play.key || candidate.key,
                    ok: true,
                    reason: 'trim_pending',
                    shouldBlocklist: false,
                    baselineSource: 'import',
    ...
    const catchUpRecentImports = async (config, {
        ...
        try {
            ...
            for (const payload of payloads) {
                ...
                if (key && importBaselineSatisfied(entries[key])) {
                    skipped += 1;
                    continue;
                }
            ...
            log(`[upgrader] integrity recent-import catch-up: ${payloads.length} in window, ran ${ranImport}, skipped ${skipped}`);
            return {
                ran: true,
            ...
            try {
                ...
                if (config.qcTrimEnabled) {
                    try {
                        ...
                        log(`[upgrader] integrity nightly trim: scanned ${trimScan.scanned || 0}, remuxed pending ${trimScan.findingCount || 0}, skipped ${trimScan.skipped || 0}`);
                    } catch (error) {
