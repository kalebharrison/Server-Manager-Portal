...
export const createQcIntegrity = ({
    ...
    const baselineImport = async (config, payload = {}) => {
        ...
        // Fingerprint first and persist immediately so library coverage does not dip
        // while the slower playback/hash stages run on import/upgrade.
        ...
        if (finger.ok && finger.cacheEntry) {
            ...
            // Missing/unreadable file — same failure path as a full baseline miss.
            let blocklisted = false;
            if (finger.shouldBlocklist) {
                const blocked = await blocklistRelease(config, finger);
                blocklisted = !!blocked?.success;
                if (config.qcIntegrityAutomationEnabled) {
                    await replaceCorrupt(config, finger, { dryRun: false });
                }
            }
            const prefs = await loadPrefs();
            ...
            await notifyIntegrityIssue(config, {
                title: finger.shouldBlocklist ? 'Integrity import failure' : 'Integrity import finding',
                description: `Fingerprint failed on import/upgrade for **${finger.title || 'a title'}**.`,
                finding: finger,
                color: finger.shouldBlocklist ? undefined : 0xf59e0b,
            });
            return { ran: true, ok: false, blocklisted, result: finger };
        }

        const play = await validateCandidate(config, candidate, {
            mode: 'playability',
            baselineSource: 'import',
            prior: finger.cacheEntry || null,
        });
        ...
        if (play.ok && play.cacheEntry) {
            ...
            const prefs = await loadPrefs();
            prefs.integrityFindings = reconcileIntegrityFindings(prefs.integrityFindings || [], {
                incoming: [{ ...play, softTimeout: true, shouldBlocklist: false }],
            }).slice(0, 500);
            invalidateIntegrityCoverage(prefs);
            await savePrefs(prefs);
        ...
        let finalEntry = play.cacheEntry;
        let finalResult = play;
        if (config.qcIntegrityXxhashEnabled) {
