...
/** Default confine when path maps are empty — remux/read must stay under /media. */
 ...
 * Without maps, absolute paths must stay under /media (no free-for-all).
 */
export const assertSafeMediaPath = async (localPath, maps = [], { realpathImpl = null } = {}) => {
    ...
    const allowedRoots = toPrefixes.length > 0 ? toPrefixes : [DEFAULT_INTEGRITY_MEDIA_ROOT];
    if (!path.posix.isAbsolute(normalizedResolved)) {
        return { ok: false, reason: 'unsafe_path', detail: 'relative path not allowed' };
    }
    const allowed = allowedRoots.some((prefix) => isUnderPrefix(normalizedResolved, prefix));
    if (!allowed) {
        return {
            ok: false,
            reason: 'unsafe_path',
            detail: toPrefixes.length > 0
                ? 'path outside configured media roots'
                : `path outside ${DEFAULT_INTEGRITY_MEDIA_ROOT}`,
        };
    }
...
export const createQcIntegrity = ({
    ...
    const listIntegritySnoozes = async (now = Date.now()) => {
        ...
        for (const [key, until] of Object.entries(current)) {
            ...
            rows.push({ key, until: new Date(ms).toISOString() });
        }
        if (dirty) {
            prefs.integritySnoozed = Object.fromEntries(rows.map((row) => [row.key, row.until]));
            await savePrefs(prefs);
        }
