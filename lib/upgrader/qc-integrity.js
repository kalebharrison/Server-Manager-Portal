...
const EMPTY_COVERAGE = () => ({
    movie: { total: 0, playability: 0, imohash: 0, xxhash: 0 },
    show: { total: 0, playability: 0, imohash: 0, xxhash: 0 },
    album: { total: 0, playability: 0, imohash: 0, xxhash: 0 },
});
...
const computeCoverage = (candidates, cacheEntries = {}) => {
    ...
    for (const candidate of candidates) {
        ...
        if (prior.playabilityAt) coverage[bucket].playability += 1;
...
export const createQcIntegrity = ({
    ...
    const beginScanIntegrity = (config, opts = {}) => {
        ...
        void scanIntegrity(config, { ...opts, mode, _lockHeld: true }).catch((error) => {
    ...
    const baselineImport = async (config, payload = {}) => {
        ...
        if (result.ok && result.cacheEntry) {
            ...
            const next = reconcileIntegrityFindings(prior, {
                passedKeys: result.key ? [result.key] : [],
            });
        ...
        if (result.softTimeout) {
            ...
            prefs.integrityFindings = reconcileIntegrityFindings(prefs.integrityFindings || [], {
                incoming: [{ ...result, softTimeout: true, shouldBlocklist: false }],
            }).slice(0, 500);
            await savePrefs(prefs);
            await queueSoftRecheck(result.key);
            await notifyIntegrityIssue(config, {
                title: 'Integrity soft timeout (import)',
                description: `Playback timed out for **${result.title || 'a title'}** — queued for recheck (not blocklisted).`,
                finding: result,
                color: 0xf59e0b,
            });
            return { ran: true, ok: false, blocklisted: false, softTimeout: true, result };
        }
        ...
        if (result.shouldBlocklist) {
            ...
            await savePrefs(prefs);
            await notifyIntegrityIssue(config, {
                title: 'Integrity import failure',
                description: `Hard fail on import/upgrade for **${result.title || 'a title'}**`
            ...
            await notifyIntegrityIssue(config, {
                ...
                description: `Finding on import/upgrade for **${result.title || 'a title'}**.`,
                finding: result,
                color: 0xf59e0b,
            });
        }
