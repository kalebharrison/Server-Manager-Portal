...
export const createQcIntegrity = ({
    ...
    const classifyMissingAgainstArr = async (config, candidate, maps = []) => {
        ...
        const liveStat = mapped ? await readStat(mapped) : { ok: false };
        if (liveStat.ok) {
            return {
                kind: 'stale_path',
                livePath,
    ...
    const getStatus = async (config) => {
        ...
        const cache = await loadCache();
        const includeMusic = config.qcIntegrityIncludeMusic !== false;
        // Always recompute from the live index — prefs coverage goes stale after re-index.
        const index = await loadIndex();
        const candidates = collectIntegrityCandidates(index.items || [], { includeMusic });
        ...
        const findings = reconcileIntegrityFindings(priorFindings, { liveKeys });
