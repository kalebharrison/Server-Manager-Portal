...
const EMPTY_BUCKET = () => ({ total: 0, playability: 0, imohash: 0, xxhash: 0 });
 ...
 * When path maps exist the resolved path must stay under a map `to` prefix.
 * Without maps, only absolute paths with no `..` segments are allowed.
 */
export const assertSafeMediaPath = async (localPath, maps = [], { realpathImpl = null } = {}) => {
    ...
    const toPrefixes = normalizeIntegrityPathMaps(maps).map((entry) => entry.to);
    let resolved;
    try {
    ...
    if (toPrefixes.length > 0) {
        const allowed = toPrefixes.some((prefix) => isUnderPrefix(normalizedResolved, prefix));
        if (!allowed) {
            return { ok: false, reason: 'unsafe_path', detail: 'path outside configured media roots' };
        }
    } else if (!path.posix.isAbsolute(normalizedResolved)) {
        return { ok: false, reason: 'unsafe_path', detail: 'relative path without path maps' };
    }
...
export const createQcIntegrity = ({
    ...
    const resolveMappedPath = async (arrPath, maps = []) => {
        const mapped = mapArrPath(arrPath, maps);
        if (!mapped) return { ok: false, path: null, reason: 'missing_path' };
        const safe = await assertSafeMediaPath(mapped, maps, { realpathImpl });
        if (!safe.ok) {
            return {
                ok: false,
                path: mapped,
    ...
    const validateCandidate = async (config, candidate, {
        ...
        const maps = Array.isArray(config.qcIntegrityPathMaps) ? config.qcIntegrityPathMaps : [];
        const resolved = await resolveMappedPath(candidate.filePath, maps);
    ...
    const resolveRecheckMode = (finding = {}) => {
        ...
        if (['playability', 'imohash', 'xxhash', 'baseline'].includes(stored)) return stored;
    ...
    const recheckFinding = async (config, { key = null, finding: findingInput = null } = {}) => {
        ...
        const mode = resolveRecheckMode(finding);
    ...
    const scanIntegrity = async (config, {
        ...
        try {
            ...
            const concurrency = (scanMode === 'playability' || scanMode === 'baseline')
                ? playConcurrency
                : hashConcurrency;
            ...
            const maps = Array.isArray(config.qcIntegrityPathMaps) ? config.qcIntegrityPathMaps : [];
            const needsPreStat = scanMode === 'baseline' || scanMode === 'playability';
            ...
            for (const candidate of ordered) {
                ...
                if (walked === 1 || walked % 250 === 0) {
                    progress = {
                        ...progress,
                        currentTitle: `Selecting files… ${walked}/${progress.target}`,
                        skipped,
                        skippedPlaying,
                    };
                }
                ...
                const mappedResult = await resolveMappedPath(candidate.filePath, maps);
                const mapped = mappedResult.ok ? mappedResult.path : null;
                if (!mappedResult.ok && mappedResult.reason === 'unsafe_path') {
                    skipped += 1;
                    continue;
                }
            ...
            progress = {
                ...
                passed: 0,
                findingCount: 0,
                currentTitle: batch.length ? 'Hashing…' : 'Nothing to scan',
                full: !!full,
            };
            ...
            // Drop cache rows for Arr file ids that no longer exist after rename/upgrade.
            for (const key of Object.keys(cacheEntries)) {
                if (!liveKeys.has(key)) delete cacheEntries[key];
            }
            ...
            const mergedFindings = reconcileIntegrityFindings(prefs.integrityFindings || [], {
                liveKeys,
                passedKeys,
                incoming: findings,
            }).slice(0, 500);
    ...
    const beginScanIntegrity = (config, opts = {}) => {
        ...
        const mode = ['baseline', 'playability', 'imohash', 'xxhash'].includes(opts.mode)
            ? opts.mode
            : 'baseline';
        ...
        void scanIntegrity(config, { ...opts, mode, libraryKey, _lockHeld: true }).catch((error) => {
    ...
    const baselineImport = async (config, payload = {}) => {
        ...
        if (trimCfg.enabled && isTrimVideoPath(candidate.filePath, candidate.mediaKind)) {
            const maps = normalizeIntegrityPathMaps(config.qcIntegrityPathMaps);
            const mapped = await resolveMappedPath(candidate.filePath, maps);
            const trimResult = mapped.ok
                ? await trimMkvFile(mapped.path, { ...trimCfg, execImpl, log })
                : { ok: false, reason: mapped.reason || 'trim_path', detail: mapped.detail };
            if (!trimResult.ok) {
                suppressAnnounce = true;
                await persistFinding({
                    ...candidate,
                    key: play.key || candidate.key,
                    ok: false,
            ...
            } else if (trimResult.changed) {
                const playAfter = await validateCandidate(config, candidate, {
                    mode: 'playability',
                    baselineSource: 'import',
                    prior: play.cacheEntry || null,
                });
                if (playAfter.ok && playAfter.cacheEntry) {
                    await writeImportCacheEntry(playAfter.key || candidate.key, playAfter.cacheEntry);
                    play.cacheEntry = playAfter.cacheEntry;
                    play.ok = true;
                } else if (!playAfter.ok) {
                    suppressAnnounce = true;
                    await persistFinding({
                        ...playAfter,
                        shouldBlocklist: false,
                        reason: playAfter.reason || 'trim_playback_failed',
                        detail: 'Playback failed after media trim.',
                    });
                }
            } else if (trimResult.dryRun && trimResult.reason === 'trim_dry_run') {
                await persistFinding({
                    ...candidate,
                    key: play.key || candidate.key,
                    ok: true,
                    reason: 'trim_pending',
                    detail: trimResult.detail || 'Dry-run: would remux extra audio/subs',
                    shouldBlocklist: false,
                    baselineSource: 'import',
                });
            }
    ...
    const escalateMismatch = async (config, candidate, prior = null, { replace = false } = {}) => {
        ...
        // Playback passed after fingerprint drift — refresh fingerprint (+ optional full hash).
        const refreshed = await validateCandidate(config, candidate, {
            mode: 'baseline',
            baselineSource: 'recheck',
            prior: play.cacheEntry || prior,
        });
        if (!refreshed.ok) {
            return { ...refreshed, shouldBlocklist: false, escalatedFrom: 'imohash_mismatch' };
        }

        if (replace && refreshed.ok) {
            // Nothing to replace — cache update handled by caller.
        }
        return { ...refreshed, shouldBlocklist: false, escalatedFrom: 'imohash_mismatch' };
    };
    ...
    const processSoftRecheckQueue = async (config) => {
        ...
        prefs.integritySoftRecheckQueue = remaining;
        await savePrefs(prefs);
        try {
    ...
    const startIntegrityJob = (getConfig) => {
        ...
        const runFingerprintBackfill = async () => {
            try {
                ...
                if (result.ran) {
                    log(`[upgrader] integrity fingerprint backfill scanned ${result.scanned}, findings ${result.findingCount}`);
                }
            } catch (error) {
                log(`[upgrader] integrity fingerprint backfill failed: ${error.message}`);
            }
        };
            ...
            try {
                ...
                if (config.qcTrimEnabled) {
                    try {
                        ...
                        const maps = normalizeIntegrityPathMaps(config.qcIntegrityPathMaps);
                        const playing = await loadPlayingPaths(config);
                        const trimCandidates = collectIntegrityCandidates(items, {
                            includeMusic: false,
                        }).filter((row) => isTrimVideoPath(row.filePath, row.mediaKind));
                        let trimmed = 0;
                        for (const row of trimCandidates) {
                            const mapped = await resolveMappedPath(row.filePath, maps);
                            if (!mapped.ok) continue;
                            if (playing.paths.some((p) => pathsMatch(p, mapped.path) || pathsMatch(p, row.filePath))) {
                                continue;
                            }
                            const trimCfg = resolveTrimConfig(config, row);
                            const result = await trimMkvFile(mapped.path, { ...trimCfg, execImpl, log });
                            if (result?.changed) trimmed += 1;
                        }
                        log(`[upgrader] integrity nightly trim: ${trimCandidates.length} mkv, remuxed ${trimmed}`);
                    } catch (error) {
                        log(`[upgrader] integrity nightly trim failed: ${error.message}`);
                    }
                }
                // Full-library fingerprint compare — report-only; escalate mismatches separately.
                ...
                const mismatches = (imo.findings || []).filter((entry) => (
                    entry?.reason === 'imohash_mismatch' || entry?.reason === 'imohash_failed'
                ));
                log(`[upgrader] integrity nightly fingerprint: scanned ${imo.scanned || 0}, mismatches ${mismatches.length}`);
        ...
        setTimeout(() => { void runFingerprintBackfill(); }, 3 * 60 * 1000);
