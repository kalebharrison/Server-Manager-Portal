...
export const createQcIntegrity = ({
    ...
    const scanIntegrity = async (config, {
        ...
        try {
            ...
            const cursor = Math.max(0, Number(prefs.integrityScanCursor) || 0) % Math.max(1, candidates.length || 1);
            ...
            for (const candidate of ordered) {
                ...
                if (
                    ...
                    skipped += 1;
                    progress = { ...progress, skipped, currentTitle: candidate.title };
                    continue;
                }
            ...
            prefs.integrityScanCursor = candidates.length
