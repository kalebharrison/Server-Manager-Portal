...
const normalizeHostUrl = (value) => String(value || '')
    .trim()
    .replace(/\/+$/, '')
    .toLowerCase();

/** Match webhook ApplicationUrl / instance name to a configured Arr instance. */
export const matchArrInstanceFromWebhook = (config, arrType, body = {}) => {
    const instances = getReadyArrInstances(config, arrType);
    if (!instances.length) return null;
    const appUrl = normalizeHostUrl(
        body.ApplicationUrl
        || body.applicationUrl
        || body.InstanceUrl
        || body.instanceUrl
    ...
    if (appUrl) {
        const byUrl = instances.find((instance) => {
            const url = normalizeHostUrl(instance.url);
            return url && (url === appUrl || appUrl.startsWith(url) || url.startsWith(appUrl));
        });
    ...
    if (instanceName) {
        const byName = instances.find((instance) => String(instance.name || '').trim().toLowerCase() === instanceName);
        if (byName) return byName;
    }
    return getDefaultArrInstance(config, arrType) || instances[0] || null;
};

const coverageBucketForPayload = (payload) => {
    const mediaType = String(payload?.mediaType || '').toLowerCase();
    if (mediaType === 'album' || payload?.arrType === 'lidarr') return 'album';
    if (mediaType === 'show' || mediaType === 'tv' || payload?.arrType === 'sonarr') return 'show';
    return 'movie';
};

export const libraryKeyForIntegrityPayload = (payload) => {
    if (payload?.libraryKey) return String(payload.libraryKey);
    const arrType = payload?.arrType || 'unknown';
    const arrId = payload?.arrInstanceId || 'default';
    return `${arrType}:${arrId}:${coverageBucketForPayload(payload)}`;
};
...
const pickPosterUrl = (record = {}) => {
    const images = record.images || record.Images || [];
    const poster = (Array.isArray(images) ? images : []).find((image) => (
        String(image?.coverType || image?.CoverType || '').toLowerCase() === 'poster'
    ));
    return poster?.remoteUrl || poster?.RemoteUrl || poster?.url || poster?.Url || null;
};
    ...
    if (k === 'sonarr') {
        ...
        return files.map((file, index) => {
            ...
            return {
                ...
                seasonNumber: episode?.seasonNumber ?? episode?.SeasonNumber ?? file.seasonNumber ?? file.SeasonNumber ?? null,
 ...
 */
export const enrichArrIntegrityPayload = (config, payload, webhookBody = null) => {
    if (!payload || typeof payload !== 'object') return null;
    const arrType = String(payload.arrType || '').toLowerCase();
    if (!arrType) return payload;
    const instance = (webhookBody
        ? matchArrInstanceFromWebhook(config, arrType, webhookBody)
        : null)
    ...
    enriched.libraryKey = enriched.libraryKey || libraryKeyForIntegrityPayload(enriched);
    enriched.libraryName = enriched.libraryName || enriched.arrInstanceName || null;
    return enriched;
};
...
export const registerArrIntegrityTriggerRoutes = ({
    ...
    const requireIntegrityTriggers = async (_req, res, next) => {
        try {
            ...
            if (!authCache.username || !authCache.password) {
                log('[integrity] webhook rejected: hook username/password not configured');
                return res.status(503).json({ error: 'Integrity webhook auth is not configured' });
            }
            return next();
        } catch {
        ...
        try {
            ...
            if (
                ...
                const payloads = collectArrIntegrityPayloads(kind, req.body || {})
                    .map((payload) => enrichArrIntegrityPayload(config, payload, req.body || {}))
                    .filter(Boolean)
                    .map((payload) => ({
                        ...payload,
                ...
                // Pack/season imports often omit sibling webhooks — catch-up fills gaps.
                if (baselined > 0 && typeof upgrader?.integrity?.scheduleImportCatchUp === 'function') {
                    upgrader.integrity.scheduleImportCatchUp();
                }
            }
            // Flip portal request badges (Processing → Available) when Arr finishes an import.
            if (
                (event.action === 'import' || event.action === 'upgrade')
                && typeof onImportComplete === 'function'
            ) {
                void Promise.resolve(onImportComplete({
                    ...
                    log(`[portal-request] import status sync failed: ${err?.message || err}`);
                });
            }
            log(`[integrity] ${kind} webhook event=${event.eventType || 'unknown'} action=${event.action} baselined=${baselined}`);
