...
export const TRUSTED_NATIVE_SOURCES = new Set(['tmdb', 'tvdb', 'imdb', 'arr', 'cache', 'manual']);
...
const DETAIL_MEMO_TTL_MS = 6 * 60 * 60 * 1000;
...
const memoSet = (key, value) => {
    detailMemo.set(key, {
        value,
        // Negative lookups expire so a transient API miss can retry later.
        expiresAt: Date.now() + (value == null ? (15 * 60 * 1000) : DETAIL_MEMO_TTL_MS),
    });
};
...
export const lookupTrimNativeLanguage = async (config = {}, candidate = {}, {
    ...
    // Manual override (Findings UI) wins over TMDb/TVDB/IMDb and cached prior.
    const overrideRaw = nativeOverride?.code ?? nativeOverride;
    if (overrideRaw != null && String(overrideRaw).trim()) {
        const code = normalizeLanguageCode(overrideRaw);
        if (code) return { code, source: 'manual' };
    }
    ...
    const fromArr = resolveNativeLanguage(candidate);
    if (fromArr) {
        return { code: fromArr, source: 'arr' };
    }
