...
/** Queue rows nest the movie/series; index/history rows are flat. */
const nestedArrEntity = (record = {}) => (
    record.movie || record.series || record.album || record.artist || null
);

const recordTags = (record = {}) => {
    const nested = nestedArrEntity(record);
    const raw = [
...
const pathForLibraryClassify = (record = {}) => {
    const nested = nestedArrEntity(record);
    return String(
        record.path
        || record.rootFolderPath
        || nested?.path
        || nested?.rootFolderPath
        || '',
    );
};
...
export const classifyUpgraderLibrary = (instance = {}, record = {}) => {
    ...
    const itemPath = pathForLibraryClassify(record);
    const matches = roots
