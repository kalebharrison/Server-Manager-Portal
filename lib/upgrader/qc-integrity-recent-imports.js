...
import { isTrimVideoPath, resolveTrimConfig, trimProfileKey } from './qc-media-trim.js';
import { filterMissingImportChecks } from './qc-integrity-schedule-policy.js';

export const RECENT_IMPORT_WINDOW_MS = 24 * 60 * 60 * 1000;
/** How often the timer may run catch-up when nothing forced it. */
export const RECENT_IMPORT_CATCHUP_MIN_INTERVAL_MS = 10 * 60 * 1000;
/** After a webhook import, wait this long then force catch-up (season packs miss siblings). */
export const RECENT_IMPORT_CATCHUP_DEBOUNCE_MS = 90 * 1000;
...
export const missingImportChecks = (entry, config = {}, candidate = {}) => {
    const missing = [];
    const dead = !entry || entry.ok === false;
    if (dead || !entry.playabilityAt) missing.push('playability');
    const trimCfg = resolveTrimConfig(config, candidate);
    const mediaKind = String(candidate.mediaKind || 'video').toLowerCase();
    if (trimCfg.enabled && mediaKind !== 'audio') {
        // Non-MKV: need a free trim_not_mkv stamp (even while dry-run is on).
        // MKV: only require a live trim pass when remux is actually enabled.
        if (!isTrimVideoPath(candidate.filePath, candidate.mediaKind)) {
            if (dead || !entry.trimAt) missing.push('trim');
        } else if (!trimCfg.dryRun) {
            const profile = trimProfileKey(trimCfg);
            if (dead || !entry.trimAt || (entry.trimProfile && entry.trimProfile !== profile)) {
                missing.push('trim');
            }
        }
    }
    if (dead || !entry.imohash) missing.push('imohash');
    if (config.qcIntegrityXxhashEnabled && (dead || !entry.xxhash)) missing.push('xxhash');
    return filterMissingImportChecks(missing, config, candidate.libraryKey || null);
};

/**
 * Coverage KPI gaps only (playability + fingerprint). Used when Arr upgraded the
 * file id without a downloadFolderImported history row — full import trim/xxhash
 * can wait for nightly; the Coverage bars should not stay broken.
 */
export const missingCoverageChecks = (entry = null) => {
    const missing = [];
    const dead = !entry || entry.ok === false;
    if (dead || !entry.playabilityAt) missing.push('playability');
    if (dead || !entry.imohash) missing.push('imohash');
...
/** Same Arr entity, older integrity cache key (upgrade replaced movieFileId / trackFileId). */
export const hasOrphanIntegrityCache = (cacheEntries = {}, candidate = {}) => {
    const key = String(candidate?.key || '');
    const ratingKey = String(candidate?.ratingKey || key.split(':file:')[0] || '');
    if (!ratingKey || !key.includes(':file:')) return false;
    const prefix = `${ratingKey}:file:`;
    return Object.keys(cacheEntries || {}).some((entryKey) => (
        entryKey.startsWith(prefix) && entryKey !== key
    ));
};

/**
 * Live-index payloads that lost coverage after Arr file-id churn (or trim wiped
 * the fingerprint). Prefer orphans; cap so catch-up stays a patch, not a backfill.
 */
export const collectCoverageGapPayloads = (candidates = [], cacheEntries = {}, {
    max = 40,
} = {}) => {
    const limit = Math.max(1, Math.min(200, Number(max) || 40));
    const orphans = [];
    const other = [];
    for (const candidate of (Array.isArray(candidates) ? candidates : [])) {
        if (!candidate?.key || !candidate?.filePath) continue;
        const entry = cacheEntries?.[candidate.key] || null;
        const missing = missingCoverageChecks(entry);
        if (!missing.length) continue;
        const payload = {
            ...candidate,
            fromCoverageGap: true,
            coverageMissing: missing,
            isUpgrade: hasOrphanIntegrityCache(cacheEntries, candidate),
        };
        if (payload.isUpgrade) orphans.push(payload);
        else other.push(payload);
    }
    return [...orphans, ...other].slice(0, limit);
};

export const importBaselineSatisfied = (entry, config = {}, candidate = {}) => (
    missingImportChecks(entry, config, candidate).length === 0
);
