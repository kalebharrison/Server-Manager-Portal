...
/** Softcore / AV often ships adult=false and without keyword bags on list endpoints. */
...
export const isAdultMediaItem = (item = {}) => {
    ...
    if (isSoftcoreMediaItem(item)) return true;
    // Trending/popular list rows rarely include keywords — title is the last line of defense.
    if (ADULTISH_TITLE_RE.test(titleBlob(item))) return true;
    return false;
};
