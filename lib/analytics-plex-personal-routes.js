...
const emptyPersonalAnalytics = () => ({ totalPlays: 0, topLibraries: [], topWatched: [], topMusic: [], recentHistory: [] });

const normalizeAnalyticsPeriod = (value) => value === 'all'
    ? 'all'
...
export const buildRecentHistory = (historyItems = [], serverIdentifier, limit = 50) => {
    ...
    historyItems.slice(0, limit).forEach((item) => {
        ...
        if (seriesKey && seriesEntries.has(seriesKey)) {
            seriesEntries.get(seriesKey).watchedCount++;
            return;
        }
        const targetKey = isEpisode ? (item.grandparentKey || item.parentKey || item.key) : item.key;
        const entry = {
            historyKey: `${item.type}:${seriesKey || item.ratingKey || item.key || item.viewedAt}`,
            title: isEpisode ? (item.grandparentTitle || item.parentTitle || item.title) : item.type === 'track' ? (item.parentTitle || item.grandparentTitle || item.title) : item.title,
            episodeTitle: item.type === 'track' ? item.title : null,
            viewedAt: item.viewedAt,
            thumb: isEpisode ? (item.grandparentThumb || item.parentThumb || item.thumb) : item.type === 'track' ? (item.parentThumb || item.grandparentThumb || item.thumb) : item.thumb,
            type: item.type,
            watchedCount: 1,
            plexUrl: `https://app.plex.tv/desktop/#!/server/${serverIdentifier}/details?key=${encodeURIComponent(targetKey)}`,
        };
        recent.push(entry);
        if (seriesKey) seriesEntries.set(seriesKey, entry);
    });
    recent.forEach((entry) => {
        if (entry.type === 'episode') entry.episodeTitle = entry.watchedCount > 1 ? `${entry.watchedCount} episodes watched` : 'TV series';
    });
    return recent;
};
...
export const registerPlexPersonalAnalyticsRoutes = ({
    ...
    const buildPersonalAnalytics = async ({ config, uri, accountID, period }) => {
    ...
    app.get('/api/plex/analytics/me', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            const identity = { serverIdentifier: config.serverIdentifier, accountId: accountID, period };
            const data = await personalCache.get(identity, () => buildPersonalAnalytics({ config, uri, accountID, period }));
            res.json(data);
        } catch (e) {
