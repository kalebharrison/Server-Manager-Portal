export const createPublicStatusPayload = (statusConfig = {}, healthData = {}) => {
    const publicServiceName = (service = {}) => {
        ...
        if (service.id === 'ombi' && (!name || name.toLowerCase() === 'ombi')) return 'Request Service';
        return name || 'Service';
    };
    const publicServiceDescription = (service = {}) => {
        ...
        if (service.id === 'ombi' && (!description || ['requests portal', 'request portal'].includes(description.toLowerCase()))) return 'Media requests';
        return description;
    };
