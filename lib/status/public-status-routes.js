...
export const registerPublicStatusRoutes = ({
    ...
    app.get('/api/health', publicReadRateLimit, (req, res) => res.json({ status: 'ok', uptime: process.uptime() }));
        ...
        try {
            ...
            for (const service of services) {
                ...
                if (service.url || !['portal', 'plex'].includes(id)) {
                    try {
                        if (!sanitizeIntegrationUrl) throw new Error('URL sanitizer unavailable');
                        normalizedUrl = await sanitizeIntegrationUrl(service.url);
