...
/** Seerr/Overseerr openapi validator rejects `+` (URLSearchParams space encoding). Use %20. */
export const encodeSeerrQueryPath = (path) => {
    const value = String(path || '');
    const q = value.indexOf('?');
    if (q < 0) return value;
    return `${value.slice(0, q + 1)}${value.slice(q + 1).replace(/\+/g, '%20')}`;
};
...
export const createRequestAppClient = ({
    ...
    const getCredentials = async (config = {}) => {
        ...
        const publicBaseUrl = await resolveIntegrationUrlForFetch(config.requestAppUrl);
        // Only the env override is honored; sticky config.requestAppFetchUrl is ignored/stripped.
        const fetchUrlOverride = requestAppInternalUrl;
        return {
            type: String(config.requestAppType || '').toLowerCase(),
            baseUrl: fetchUrlOverride ? await resolveIntegrationUrlForFetch(fetchUrlOverride) : publicBaseUrl,
            publicBaseUrl,
    ...
    const fetchSeerrJson = async (config, path, { method = 'GET', body = null } = {}) => {
        const { baseUrl, apiKey } = await getCredentials(config);
        const safePath = encodeSeerrQueryPath(path);
        let response;
        try {
            response = await fetchWithTimeout(`${baseUrl}${safePath}`, {
                method,
                headers: {
    ...
    const cachedSeerrJson = async (config, path, ttlMs, staleIfErrorMs = 0) => {
        const { baseUrl, type } = await getCredentials(config);
        const safePath = encodeSeerrQueryPath(path);
        return cache.getOrSet(`${type}:${baseUrl}:${safePath}`, ttlMs, () => fetchSeerrJson(config, safePath), { staleIfErrorMs });
    };
