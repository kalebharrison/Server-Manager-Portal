...
export const normalizeRequestItem = (request = {}, publicBaseUrl = '', proxyPoster = (value) => value) => {
    ...
    return {
        ...
        genres: normalizeArray(media.genres || media.genre || media.genreIds || [], normalizeGenre, 12),
        originalLanguage: media.originalLanguage || media.original_language || null,
        seasons: Array.isArray(request.seasons || media.seasons) ? (request.seasons || media.seasons).map(mapSeason).filter(Boolean) : [],
        requestedBy: {
