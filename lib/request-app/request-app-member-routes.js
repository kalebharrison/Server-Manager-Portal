import { ensurePortalAgentApiKey, tryAttachPortalAgent } from '../auth/portal-agent-key.js';
...
export const registerRequestAppMemberRoutes = ({
    ...
    const requireMemberOrPortalAgent = async (req, res, next) => {
        try {
            let config = await loadConfig();
            const ensured = await ensurePortalAgentApiKey(config, {
                save: typeof saveConfig === 'function' ? saveConfig : null,
            });
            config = ensured.config;
            if (tryAttachPortalAgent(req, { expectedKey: config.portalAgentApiKey })) return next();
