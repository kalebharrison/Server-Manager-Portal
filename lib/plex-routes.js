...
import { presentStreamAddress, presentStreamUser } from './stream-privacy.js';

const SESSION_SNAPSHOT_TTL_MS = 3000;
...
export const registerPlexRoutes = ({
    ...
    const sessionSnapshotCache = createTtlCache({ maxEntries: 8 });
    ...
    const fetchSessionsContainer = async (config, uri) => sessionSnapshotCache.getOrSet(
        `${config.serverIdentifier}:${uri}`,
        ...
        async () => {
            const sessionsData = await fetch(`${uri}/status/sessions?X-Plex-Token=${config.plexToken}`, { headers: { Accept: 'application/json' } })
                ...
                .catch(() => null);
            return sessionsData?.MediaContainer?.Metadata || [];
        },
    );

    const loadActiveSessions = async (config, uri, user) => {
        const metadata = await fetchSessionsContainer(config, uri);
