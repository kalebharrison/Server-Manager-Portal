...
export const registerPlexRoutes = ({
    ...
    app.get('/api/plex/dashboard', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            const cacheKey = scopedCacheKey('plex_dashboard_data', [config.serverIdentifier, uri, config.plexToken, limit]);
            const cachedData = await withCache(cacheKey, 2000, async () => {
                ...
                const recentSectionResults = await Promise.all(sections.map(section =>
                ...
                return { sessionsData, sectionsData, recentSectionResults };
            });
            const { sessionsData, sectionsData, recentSectionResults = [] } = cachedData;
