...
export const createTvdbService = ({ fetchWithTimeout, log = () => {} }) => {
    ...
    const resolveSeriesId = async (config, remoteId) => {
        const key = String(remoteId || '').trim();
        if (!key || !config?.tvdbApiKey) return null;
        const cached = remoteIds.get(key);
        if (cached?.expiresAt > Date.now()) return cached.id;
        try {
            const token = await getToken(config);
            const response = await fetchWithTimeout(`${TVDB_API_BASE}/search/remoteid/${encodeURIComponent(key)}`, {
                headers: { Accept: 'application/json', Authorization: `Bearer ${token}` },
            }, 12000);
            if (!response.ok) throw new Error(`TVDB remote ID lookup returned HTTP ${response.status}`);
            const payload = await response.json().catch(() => ({}));
            const matches = Array.isArray(payload?.data) ? payload.data : payload?.data ? [payload.data] : [];
            const match = matches.map((result) => result?.series || result).find((series) => Number(series?.id) > 0);
            const id = Number(match?.id) || null;
            remoteIds.set(key, { id, expiresAt: Date.now() + DETAIL_TTL_MS });
            return id;
        } catch (error) {
            log(`[TVDB] Remote ID lookup failed: ${error.message}`);
    ...
    const checkHealth = async (config) => {
        const token = await getToken(config);
        if (!token) return { ok: false, httpCode: 0 };
        const request = (bearer) => fetchWithTimeout(`${TVDB_API_BASE}/languages`, {
            headers: { Accept: 'application/json', Authorization: `Bearer ${bearer}` },
        }, 12000);
        let response = await request(token);
        if (response.status === 401) {
            auth = { identity: '', token: '', expiresAt: 0 };
            response = await request(await getToken(config));
        }
        return { ok: response.ok, httpCode: response.status };
    };

    const enrichSeries = async (config, item) => {
        if (item?.mediaType !== 'tv' || !config?.tvdbApiKey) return item;
        const tvdbId = item.tvdbId || await resolveSeriesId(config, item.imdbId);
        if (!tvdbId) return item;
        const tvdb = await fetchSeries(config, tvdbId);
        if (!tvdb) return item;
        ...
        return {
            ...item,
            tvdbId,
            title: tvdb.name || item.title,
            overview: tvdb.overview || item.overview || '',
            firstAirDate: tvdb.firstAired || item.firstAirDate || null,
            lastAirDate: tvdb.lastAired || item.lastAirDate || null,
            status: tvdb.status?.name || item.status || null,
            network: network || item.network || null,
            genres: genres.length ? genres : item.genres,
            numberOfSeasons: (Array.isArray(tvdb.seasons) ? tvdb.seasons.filter((season) => season.type?.type === 'Aired Order').length : 0) || item.numberOfSeasons || null,
            metadataSources: [...new Set(['TVDB', ...(item.metadataSources || ['TMDB'])])],
        };
    };

    return { checkHealth, enrichSeries, fetchSeries, resolveSeriesId };
};
