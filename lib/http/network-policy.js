...
/** Link-local / cloud-metadata ranges — blocked even when LAN private URLs are allowed. */
export const isAlwaysBlockedIp = (host) => {
    const normalized = String(host || '').replace(/^::ffff:/i, '').toLowerCase();
    if (!net.isIP(normalized)) return false;
    if (net.isIP(normalized) === 4) {
        return normalized.startsWith('169.254.') || normalized === '0.0.0.0';
    }
    const expanded = expandIpv6(normalized);
    if (!expanded) return /^fe80/i.test(normalized);
    const first = parseInt(expanded.slice(0, 4), 16);
    if (!Number.isFinite(first)) return false;
...
/**
 * Normalize an integration URL for outbound fetches, then re-check DNS when
 * private hosts are disallowed (mitigates rebinding after save-time checks).
 * Link-local/metadata addresses are always re-checked even when LAN is allowed.
 */
export const resolveIntegrationUrlForFetch = async (rawUrl, { allowPrivate = true } = {}) => {
    if (!rawUrl) return '';
    ...
    // Literal metadata/link-local already rejected in normalize; re-check DNS when private hosts are disallowed.
    if (!allowPrivate) {
        await assertHostnameAllowed(hostname, { allowPrivate: false });
    } else if (!net.isIP(hostname)) {
        // LAN allowed, but still refuse hostnames that resolve to link-local/metadata.
        await assertHostnameAllowed(hostname, { allowPrivate: true, blockAlwaysBlocked: true });
    }
    return normalized;
};

export const createResolveIntegrationUrlForFetch = ({ allowPrivate = false } = {}) => (
    async (rawUrl) => resolveIntegrationUrlForFetch(rawUrl, { allowPrivate })
);
