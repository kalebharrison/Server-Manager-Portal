    ...
    const syncUsers = async (config) => {
        ...
        const syncedUsers = plexUsers.map(pUser => {
            ...
            if (existingUser) {
                ...
                return { ...existingUser, id: pUser.id, plexId: existingUser.plexId || pUser.id, username: pUser.username, email: pUser.email, thumb: pUser.thumb, plexAccessStatus: 'active' };
            }
