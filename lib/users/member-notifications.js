...
export const createMemberNotifications = ({
    ...
    const notifyRequestUpdate = async (config, {
        ...
        try {
            ...
            return sendMemberNotice(config, user, {
                ...
                replyTo: requestId ? buildInboundReplyTo(config, { kind: 'request', id: requestId }) : '',
    ...
    const notifyIssueReply = async (config, { issue, replyAuthor, statusLabel } = {}) => {
        try {
            ...
            return sendMemberNotice(config, user, {
                ...
                replyTo: issue?.id ? buildInboundReplyTo(config, { kind: 'issue', id: issue.id }) : '',
                subject: resolved
    ...
    const sendAvailableNotice = async (config, user, { title, uniqueKey, mediaType, tmdbId, posterPath, requestId }) => {
        const portalUrl = buildPortalMediaUrl(config, { mediaType, tmdbId });
    ...
    const checkPortalWatchlistAvailability = async (config) => {
        ...
        for (const request of candidates) {
            ...
            if (request.id && typeof service.markAvailabilityNotified === 'function') {
                await service.markAvailabilityNotified(request.id).catch((error) => {
                    log?.(`Portal availability notify stamp failed: ${error.message}`);
                });
            }
        }
    };
