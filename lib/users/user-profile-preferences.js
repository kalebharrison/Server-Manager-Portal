 ...
 */
export const applyAccountPreferencePatch = (user, patch = {}, { users = [] } = {}) => {
    const errors = [];
    let changed = false;
    ...
    if (patch.discordId !== undefined) {
        const result = applyDiscordIdToUser(users.length ? users : [user], user, patch.discordId);
        if (result.error) errors.push(result.error);
        else if (result.changed) changed = true;
    }
