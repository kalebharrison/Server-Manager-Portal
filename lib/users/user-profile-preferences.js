...
export const applyAccountPreferencePatch = (user, patch = {}) => {
    ...
    if (patch.discordId !== undefined) {
        const nextDiscordId = sanitizeDiscordUserId(patch.discordId);
        if (nextDiscordId === null) {
            errors.push('Discord user ID must be a numeric snowflake (enable Developer Mode → Copy User ID).');
        } else {
            const previous = String(user.discordId || '');
            if (nextDiscordId !== previous) {
                if (nextDiscordId) user.discordId = nextDiscordId;
                else delete user.discordId;
                changed = true;
            }
        }
    }
