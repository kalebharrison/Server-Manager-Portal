...
export const registerAdminUserWriteRoutes = ({
    ...
    app.delete('/api/users/:id', requireAdmin, async (req, res) => {
        ...
        if (membershipSync?.remove && config) {
            membershipSync.remove(user, config).catch((e) => log(`Request app remove on delete skipped: ${e.message}`));
        }
    ...
    app.post('/api/users/:id/revoke', requireAdmin, async (req, res) => {
        ...
        if (revoked) {
            ...
            await saveFile(usersPath, users);
            if (membershipSync?.remove) {
                membershipSync.remove(user, config).catch((e) => log(`Request app remove on revoke skipped: ${e.message}`));
            }
