...
import { applyDiscordIdToUser } from './user-discord-id.js';
...
export const registerAdminUserWriteRoutes = ({
    ...
    app.put('/api/users/:id', requireAdmin, async (req, res) => {
        const { id } = req.params;
        const { expiryDate, exemptFromCleanup, newsletterOptIn, requestOverrides, discordId } = req.body;
        ...
        if (discordId !== undefined) {
            const result = applyDiscordIdToUser(users, users[userIndex], discordId);
            if (result.error) return res.status(400).json({ error: result.error });
            discordIdChanged = !!result.changed;
        }
        ...
        if (discordIdChanged) {
            await appendAuditLog('discord_id_updated', req.user, users[userIndex], {
                discordId: users[userIndex].discordId || null,
            });
        }
        ...
        if (expiryDate !== undefined && expiryDate !== previousExpiryDate) {
            ...
            await sendAdjustmentEmail(config, users[userIndex]);
    ...
    app.post('/api/users/bulk-update', requireAdmin, async (req, res) => {
        ...
        try {
            ...
            for (const user of users) {
                ...
                await sendAdjustmentEmail(config, user);
