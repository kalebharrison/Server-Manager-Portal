...
import { buildWatchStatsFromHistory } from './analytics-history.js';
    ...
    const fetchMaintenanceWatchStats = async (config, uri) => {
        if (historyStore) {
            ...
            if (Array.isArray(items) && items.length) {
                log(`[MaintenanceIndex] Using analytics history store (${items.length} items) for watch stats.`);
                return buildWatchStatsFromHistory(items);
            }
        }
