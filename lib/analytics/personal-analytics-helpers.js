...
export const buildRecentHistory = (historyItems = [], serverIdentifier, limit = 50) => {
    ...
    for (const item of (Array.isArray(historyItems) ? historyItems : [])) {
        ...
        const targetKey = isEpisode
            ...
            : metadataKey;
        const ratingKeyRaw = isEpisode
            ? (item.grandparentRatingKey || item.grandparentKey || item.parentRatingKey || item.ratingKey)
            : item.ratingKey;
        const ratingKeyMatch = String(ratingKeyRaw || '').match(/(\d+)\s*$/);
        const ratingKey = ratingKeyMatch ? ratingKeyMatch[1] : null;
        const entry = {
