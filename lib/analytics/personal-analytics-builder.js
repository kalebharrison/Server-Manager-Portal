...
export const buildPersonalAnalytics = async ({
    ...
    const topWatchedRaw = Object.values(aggregates.contentCounts)
        .filter((c) => c.type !== 'track')
        .sort((a, b) => b.plays - a.plays)
        .slice(0, 30);
    const topMusicRaw = Object.values(aggregates.contentCounts)
        .filter((c) => c.type === 'track')
        .sort((a, b) => b.plays - a.plays)
        .slice(0, 30);
    ...
    // Fill posters for home Most Watched / music when history rows lacked thumbs.
    await enrichTopMediaMetadata({
        uri,
        config,
        items: topWatchedRaw.filter((item) => !item.thumb).slice(0, 24),
        getCachedPlexMetadata,
        setCachedPlexMetadata,
        mediaType: 'show',
    });
    await enrichTopMediaMetadata({
        uri,
        config,
        items: topMusicRaw.filter((item) => !item.thumb).slice(0, 12),
        getCachedPlexMetadata,
        setCachedPlexMetadata,
        mediaType: 'show',
    });

    const topWatched = topWatchedRaw.map((c) => {
        if (c.thumb) c.thumbUrl = plexImageUrl(c.thumb);
        return c;
    });
    const topMusic = topMusicRaw.map((c) => {
        if (c.thumb) c.thumbUrl = plexImageUrl(c.thumb);
        return c;
    });
    ...
    await enrichTopMediaMetadata({
        uri,
        config,
        items: recentHistory.filter((h) => !h.thumb).slice(0, 20),
        getCachedPlexMetadata,
        setCachedPlexMetadata,
        mediaType: 'show',
    });
