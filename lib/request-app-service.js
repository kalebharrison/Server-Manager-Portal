...
export const createRequestAppService = ({ fetchWithTimeout, resolveIntegrationUrlForFetch, requestAppInternalUrl = '', log = () => {} }) => {
    ...
    const normalizeDiscoverPayload = (payload, page, publicBaseUrl, mediaType = 'all') => {
    ...
    const discover = async (config, { category = 'trending', mediaType = 'all', page = 1 } = {}) => {
        ...
        if (payloads.length === 1) return normalizeDiscoverPayload(payloads[0], page, publicBaseUrl, effectiveType);
        const normalized = payloads.flatMap((payload) => normalizeDiscoverPayload(payload, page, publicBaseUrl, effectiveType).results);
        return {
            results: normalized,
