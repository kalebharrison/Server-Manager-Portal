...
export const createRequestAppService = ({ fetchWithTimeout, resolveIntegrationUrlForFetch, requestAppInternalUrl = '', log = () => {} }) => {
    ...
    const filterMediaItems = (items, mediaType, anime, genreId = null) => items
    ...
    const search = async (config, { query, mediaType = 'all', anime = false, genreId = null, page = 1 } = {}) => {
        ...
        return {
            results: filterMediaItems(results
                ...
                .map((item) => normalizeMediaItem(item, publicBaseUrl)), normalizeDiscoverMediaType(mediaType), anime, normalizeGenreId(genreId)),
            pageInfo: pageInfoWithNext(payload, page, results.length),
        };
    };

    const normalizeDiscoverPayload = (payload, page, publicBaseUrl, mediaType = 'all', anime = false, genreId = null) => {
        const results = Array.isArray(payload?.results) ? payload.results : Array.isArray(payload) ? payload : [];
        return {
            results: filterMediaItems(results
                .filter((item) => ['movie', 'tv'].includes(String(item.mediaType || item.type || '').toLowerCase()))
                .map((item) => normalizeMediaItem(item, publicBaseUrl)), mediaType, anime, genreId),
            sourceResultCount: results.length,
            pageInfo: pageInfoWithNext(payload, page, results.length),
        };
    };

    const discover = async (config, { category = 'trending', mediaType = 'all', anime = false, genreId = null, page = 1 } = {}) => {
        ...
        const { publicBaseUrl } = getCredentials(config);
        const sourcePageBatch = anime || requestedGenreId ? ANIME_SOURCE_PAGE_BATCH : 1;
        ...
        const normalizedPayloads = payloadGroups.flatMap((payloads, index) => payloads.map((payload) => (
            normalizeDiscoverPayload(payload, sourcePages[index], publicBaseUrl, effectiveType, anime, requestedGenreId)
        )));
