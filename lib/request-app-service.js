...
export const createRequestAppService = ({ fetchWithTimeout, resolveIntegrationUrlForFetch, requestAppInternalUrl = '', log = () => {} }) => {
    ...
    const filterMediaItems = (items, mediaType, anime) => items
        .filter((item) => mediaType === 'all' || item.mediaType === mediaType)
        .filter((item) => !anime || isAnimeItem(item));

    const search = async (config, { query, mediaType = 'all', anime = false, page = 1 } = {}) => {
        ...
        return {
            results: filterMediaItems(results
                ...
                .map((item) => normalizeMediaItem(item, publicBaseUrl)), normalizeDiscoverMediaType(mediaType), anime),
            pageInfo: payload?.pageInfo || { page: safePage(page), results: results.length },
        };
    };

    const normalizeDiscoverPayload = (payload, page, publicBaseUrl, mediaType = 'all', anime = false) => {
        const results = Array.isArray(payload?.results) ? payload.results : Array.isArray(payload) ? payload : [];
        return {
            results: filterMediaItems(results
                .filter((item) => ['movie', 'tv'].includes(String(item.mediaType || item.type || '').toLowerCase()))
                .map((item) => normalizeMediaItem(item, publicBaseUrl)), mediaType, anime),
            pageInfo: payload?.pageInfo || { page: safePage(page), results: results.length },
        };
    };

    const discover = async (config, { category = 'trending', mediaType = 'all', anime = false, page = 1 } = {}) => {
        ...
        if (anime && safeCategory !== 'trending') params.set('genre', '16');
        const { publicBaseUrl } = getCredentials(config);
        const payloads = await Promise.all(endpointMap[safeCategory][effectiveType].map((endpoint) => (
            cachedSeerrJson(config, `${endpoint}?${params.toString()}`, 60_000)
        )));
        if (payloads.length === 1) return normalizeDiscoverPayload(payloads[0], page, publicBaseUrl, effectiveType, anime);
        const normalizedPayloads = payloads.map((payload) => normalizeDiscoverPayload(payload, page, publicBaseUrl, effectiveType, anime));
        const normalized = normalizedPayloads.flatMap((payload) => payload.results);
        return {
            results: normalized,
            pageInfo: {
                page: safePage(page),
                pages: Math.max(...normalizedPayloads.map((payload) => Number(payload.pageInfo?.pages) || 1)),
