...
const normalizeDiscoverMediaType = (value) => {
    const type = String(value || '').toLowerCase();
...
export const createRequestAppService = ({ fetchWithTimeout, resolveIntegrationUrlForFetch, tvdbService = null, requestAppInternalUrl = '', log = () => {} }) => {
    ...
    const isForeignLanguageItem = (item) => {
        const language = String(item?.originalLanguage || '').toLowerCase();
        return !!language && language !== 'en' && !isAnimeItem(item);
    };

    const normalizeGenreId = (value) => {
        const id = Number(value);
        return Number.isFinite(id) && id > 0 ? id : null;
    };

    const filterMediaItems = (items, mediaType, anime, foreign, genreId = null) => items
    ...
    const search = async (config, { query, mediaType = 'all', anime = false, foreign = false, genreId = null, page = 1 } = {}) => {
        ...
        return {
            results: filterMediaItems(results
                ...
                .map((item) => normalizeMediaItem(item, publicBaseUrl)), normalizeDiscoverMediaType(mediaType), anime, foreign, normalizeGenreId(genreId)),
    ...
    const normalizeDiscoverPayload = (payload, page, publicBaseUrl, mediaType = 'all', anime = false, foreign = false, genreId = null) => {
        ...
        return {
            results: filterMediaItems(results
                .filter((item) => ['movie', 'tv'].includes(String(item.mediaType || item.type || '').toLowerCase()))
                .map((item) => normalizeMediaItem(item, publicBaseUrl)), mediaType, anime, foreign, genreId),
    ...
    const discover = async (config, { category = 'trending', mediaType = 'all', anime = false, foreign = false, genreId = null, page = 1 } = {}) => {
        ...
        const payloadGroups = await Promise.all(sourcePages.map((sourcePage) => {
            ...
            return Promise.all(endpointMap[safeCategory][effectiveType].map((endpoint) => (
                cachedSeerrJson(config, `${endpoint}?${params.toString()}`, 60_000, 10 * 60_000)
            )));
        }));
    ...
    const getMediaDetails = async (config, { mediaType, tmdbId }) => {
        ...
        const item = normalizeMediaItem({ ...payload, mediaType: type, id }, publicBaseUrl);
        return tvdbService ? tvdbService.enrichSeries(config, item) : item;
    };

    const listRequestUsers = async (config) => {
        const payload = await cachedSeerrJson(config, '/api/v1/user?take=1000&sort=displayname', 300_000, 30 * 60_000);
        const results = Array.isArray(payload?.results) ? payload.results : [];
    ...
    const startCacheWarmer = (loadConfig) => {
        const warm = async () => {
            ...
            await Promise.all(['trending', 'popular', 'upcoming'].map((category) => (
                discover(config, { category, mediaType: 'all', page: 1 })
            )));
        };
        void warm().catch((error) => log(`Request discovery cache warm failed: ${error.message}`));
        return setInterval(() => void warm().catch((error) => log(`Request discovery cache warm failed: ${error.message}`)), 5 * 60 * 1000);
    };
