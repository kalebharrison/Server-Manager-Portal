...
const normalizeMediaItem = (item = {}, publicBaseUrl = '', proxyPoster = (value) => value) => {
...
const normalizeRequestItem = (request = {}, publicBaseUrl = '', proxyPoster = (value) => value) => {
...
export const createRequestAppService = ({
    ...
    const listRequests = async (config, { filter = 'pending', take = 30, skip = 0 } = {}) => {
        ...
        const results = Array.isArray(payload?.results) ? payload.results : [];
        return {
            results: results.map((item) => normalizeRequestItem(item, publicBaseUrl, proxyPoster)),
    ...
    return {
        ...
        getPosterImage: (remoteUrl) => imageCache.load(remoteUrl),
        startCacheWarmer,
    };
};
