...
export const createRequestAppService = ({
    ...
    /** Backfill/import missing Seerr users for a list of active portal members (one list + imports). */
        ...
        try {
            ...
            for (const portalUser of candidates) {
                ...
                try {
                    ...
                    if (findRequestAppUser(users, portalUser)?.id) {
                        created += 1;
                        log(`Request app user backfilled for ${portalUser.username || portalUser.email || plexId || jellyfinId}`);
                    } else {
                        failed += 1;
                    }
                } catch (err) {
                    failed += 1;
                    log(`Request app user backfill failed for ${portalUser.username || portalUser.email || 'user'}: ${err.message}`);
                }
            }
