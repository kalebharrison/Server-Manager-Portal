...
const normalizeMediaItem = (item = {}, publicBaseUrl = '') => {
    ...
    const title = item.title || item.name || item.originalTitle || item.originalName || 'Unknown title';
    const date = item.releaseDate || item.firstAirDate || '';
    const posterPath = item.posterPath || mediaInfo.posterPath || item.poster || '';
    const backdropPath = item.backdropPath || mediaInfo.backdropPath || item.backdrop || '';
    ...
    return {
        ...
        rating: Number(item.voteAverage) || null,
...
export const createRequestAppService = ({ fetchWithTimeout, resolveIntegrationUrlForFetch, requestAppInternalUrl = '', log = () => {} }) => {
    ...
    const discover = async (config, { category = 'trending', page = 1 } = {}) => {
        const safeCategory = ['trending', 'movies', 'tv'].includes(category) ? category : 'trending';
        const endpoint = safeCategory === 'movies'
        ...
        const { publicBaseUrl } = getCredentials(config);
        const payload = await cachedSeerrJson(config, `${endpoint}?${params.toString()}`, 60_000);
