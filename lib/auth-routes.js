...
import { sanitizeDisplayName } from './user-profile.js';
...
export const registerAuthRoutes = ({
    ...
    app.post('/api/users/preferences', requireAuth, requireMember, async (req, res) => {
        ...
        try {
            const { newsletterOptIn, displayName } = req.body || {};
            const users = await loadFile(USERS_PATH, []);
            const localUser = findLocalUserForSession(users, req.user);
            ...
            if (newsletterOptIn !== undefined) {
                const nextOptIn = !!newsletterOptIn;
                const oldOptIn = users[userIndex].newsletterOptIn === true;
                if (oldOptIn !== nextOptIn) {
                    users[userIndex].newsletterOptIn = nextOptIn;
                    // Clear legacy opt-out flag so it cannot re-enable mail by accident.
                    delete users[userIndex].optOutNewsletter;
                    changed = true;
                    await appendAuditLog(nextOptIn ? 'newsletter_opt_in' : 'newsletter_opt_out', req.user, req.user);
                }
            }

            if (displayName !== undefined) {
                const nextName = sanitizeDisplayName(displayName);
                const previous = sanitizeDisplayName(users[userIndex].displayName);
                if (nextName !== previous) {
                    if (nextName) users[userIndex].displayName = nextName;
                    else delete users[userIndex].displayName;
                    changed = true;
                }
            }
