...
const resolveHomeCacheRailKey = (restPath, query = {}) => {
    ...
    // Anime / filtered catalogs must never reuse the mixed home rails.
...
export const registerDiscoveryRoutes = ({
    ...
    app.get('/api/discovery/trending', requireAuth, requireMember, memberGate, (req, res) => run(res, async () => {
        ...
        const animeMode = req.query.anime === '1' || req.query.anime === 'true';
        // Home cache is the mixed rail — anime-only must hit live TMDB.
        if (pageNum === 1 && homeCache && !animeMode) {
