...
const discoveryPreferences = (config = {}) => ({
    ...
    showWatchlist: config.showWatchlist !== false,
...
export const registerDiscoveryRoutes = ({
    ...
    app.get('/api/discovery/proxy/*', requireAuth, requireMember, memberGate, (req, res) => run(res, async () => {
        const { config } = await requireTmdb();
        ...
        res.json(await router.fetchPath(`/${req.params[0] || ''}`, req.query));
    }));
