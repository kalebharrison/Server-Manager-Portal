/**
 * Map raw TMDB (snake_case) payloads to camelCase
...
const isAdultMappedItem = (item) => {
    if (
        item?.adult === true
        || item?.adult === 1
        || String(item?.adult || '').toLowerCase() === 'true'
    ) return true;
...
export const mapPaginatedResults = (payload = {}, mapItem) => {
    const mapped = Array.isArray(payload.results) ? payload.results.map(mapItem) : [];
    return {
        page: payload.page || 1,
        totalPages: payload.total_pages || payload.totalPages || 1,
        totalResults: payload.total_results || payload.totalResults || 0,
        // Defense in depth — strip adult even if TMDB ignored include_adult=false.
        results: mapped.filter((item) => item && !isAdultMappedItem(item)),
    };
};
...
export const mapCombinedCredits = (credits = {}) => {
    ...
    }).filter((item) => !isAdultMappedItem(item));
