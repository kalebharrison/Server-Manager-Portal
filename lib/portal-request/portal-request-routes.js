...
export const registerPortalRequestRoutes = ({
    ...
    /**
     * Scan Radarr/Sonarr: migrate legacy ownership/notify tags → stable portal `{user.id}` /
     * `n-{user.id}`, removing superseded labels from each title. Also migrates portal
     * notifyUserIds → Arr notify tags. Does not dump ownership into JSON.
     */
