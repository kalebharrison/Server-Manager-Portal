...
import { findLocalUserById } from '../users/session-user.js';
import {
...
export const registerPortalRequestRoutes = ({
    ...
    const loadPortalService = async () => {
        ...
        return {
            ...
            service: createPortalRequestService({
                ...
                resolveUser: async (id) => findLocalUserById(Array.isArray(users) ? users : [], id),
    ...
    /**
     * Scan Radarr/Sonarr: ensure stable portal `{user.id}` / `n-{user.id}` tags, and
     * correct portal/media legacy id-username labels onto that form. Seerr
     * `{seerrId}-{username}` tags are never removed. Also migrates portal notifyUserIds
     * → Arr notify tags. Does not dump ownership into JSON.
     */
