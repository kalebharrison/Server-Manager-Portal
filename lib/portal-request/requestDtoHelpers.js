...
/** Dedupe key for a member's request list (HD and 4K stay separate). */
...
/**
 * Collapse duplicate member request rows for the same title/quality.
 * Keeps HD and 4K as separate entries.
 */
export const dedupeMemberRequestRows = (rows = []) => {
    ...
    for (const row of rows) {
        ...
        winnerByKey.set(key, prev ? pickPreferredMemberRequest(prev, row) : row);
    }
