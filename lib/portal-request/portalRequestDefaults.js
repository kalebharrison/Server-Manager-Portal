/**
 * Portal request discovery defaults (globals + per-user overrides).
...
export const getPortalRequestDefaults = (config = {}) => ({
    ...
    // Members should not pick profile/root — portal routes movie/TV/anime automatically.
    allowAdvancedRequests: config.portalAllowAdvancedRequests === true,
    showRecentlyAdded: config.portalShowRecentlyAdded !== false,
    showWatchlist: config.portalShowWatchlist === true,
...
export const resolveMemberRequestPolicy = (config = {}, user = null) => {
    ...
    return {
        ...
        autoApproveMovies: pick('autoApproveMovies'),
        autoApproveTv: pick('autoApproveTv'),
        autoApproveMovies4k: pick('autoApproveMovies4k'),
        autoApproveTv4k: pick('autoApproveTv4k'),
        // Keep globals for auto-request / UI gates (not per-user yet)
...
export const shouldPortalAutoApprove = (config, mediaType, { is4k = false, user = null } = {}) => {
    const policy = resolveMemberRequestPolicy(config, user);
    const type = mediaType === 'tv' ? 'tv' : 'movie';
    if (is4k) {
        return type === 'tv' ? !!policy.autoApproveTv4k : !!policy.autoApproveMovies4k;
    }
    return type === 'tv' ? !!policy.autoApproveTv : !!policy.autoApproveMovies;
};
...
export const pickPortalRequestDefaultsForSave = (body = {}, existingConfig = {}) => {
    ...
    return {
        ...
        portalAllowAdvancedRequests: pickBool('portalAllowAdvancedRequests'),
        portalShowRecentlyAdded: pickBoolDefaultTrue('portalShowRecentlyAdded'),
        portalShowWatchlist: pickBool('portalShowWatchlist'),
        autoApproveMovies: pickBool('autoApproveMovies'),
        autoApproveTv: pickBool('autoApproveTv'),
