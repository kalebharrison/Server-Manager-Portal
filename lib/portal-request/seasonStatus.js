...
/**
 * Per-season rows are often missing even when Sonarr is monitoring a season.
...
export const inferMissingLibrarySeasonStatus = (details = {}, mediaInfo = {}, seasonRow = {}) => {
    ...
    const hasTrackedSeason = librarySeasons.some(
        (s) => [MEDIA_STATUS_PROCESSING, MEDIA_STATUS_PARTIAL, MEDIA_STATUS_AVAILABLE, 2].includes(Number(s?.status)),
    );
    const showInLibrary = showStatus === MEDIA_STATUS_PROCESSING
        || showStatus === MEDIA_STATUS_PARTIAL
        || showStatus === MEDIA_STATUS_AVAILABLE
        || hasTrackedSeason;
    ...
    if (!knownSeasonNumbers.length) {
        if (showStatus === MEDIA_STATUS_PARTIAL && isReturningSeries(details)) {
    ...
    if (isEndedShow(details) && showInLibrary && seasonNumber > 0 && seasonNumber <= lastAiredSeason) {
        ...
        if (mainLibrarySeasons.length > 0 || showStatus === MEDIA_STATUS_PARTIAL || showStatus === MEDIA_STATUS_AVAILABLE) {
