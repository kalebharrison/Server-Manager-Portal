...
export const mergeAvailabilityEntryOntoItem = (item, entry) => {
    ...
    const coerced = entry;
    const displayTags = Array.isArray(coerced.displayTags)
        ? coerced.displayTags
        : (Array.isArray(coerced.mediaInfo?.displayTags) ? coerced.mediaInfo.displayTags : []);
    return {
        ...item,
        mediaInfo: {
            ...(item.mediaInfo || {}),
...
export const rebuildDiscoveryAvailabilityCache = async ({
    ...
    // Disk snapshot runs ~12h; in-memory catalogs are warmed every 5m separately.
