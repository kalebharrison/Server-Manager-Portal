 ...
 * Returns camelCase payloads via tmdbMapper.
 */
...
/** Map Discover query params → TMDB discover query. */
export const buildTmdbDiscoverQuery = (mediaType, opts = {}, defaults = {}) => {
    ...
    // Softcore / erotic often have adult=false on TMDB — exclude by keyword (OR).
    // 155477 softcore · 190370 erotic movie · 445 pornography
    // (Do NOT ban keyword 210024 — TMDB uses that id for general "anime".)
    const SOFTCORE_KEYWORD_BAN = '155477|190370|445';
    ...
    if (excludeKeywords) {
        ...
        for (const id of SOFTCORE_KEYWORD_BAN.split('|')) parts.add(id);
        params.set('without_keywords', [...parts].join('|'));
    } else {
        params.set('without_keywords', SOFTCORE_KEYWORD_BAN);
    }

    // Discover movie/TV lists use query.language as original-language filter.
    let originalLang = '';
    ...
    // Hard ban — never surface adult titles from TMDB discover.
...
export const createTmdbClient = (config = {}) => {
    ...
    const search = async (query, opts = {}) => {
        const raw = await get('/search/multi', {
            ...
            // Hard ban — never request adult search results.
            include_adult: 'false',
        });
    ...
    const upcomingMovies = async (opts = {}) => discoverMovies({
        ...
        primaryReleaseDateLte: opts.primaryReleaseDateLte || undefined,
    ...
    const upcomingTv = async (opts = {}) => discoverTv({
        ...
        firstAirDateLte: opts.firstAirDateLte || undefined,
        sortBy: opts.sortBy || 'popularity.desc',
        originalLanguageFromLanguageQuery: false,
        originalLanguage: opts.originalLanguage,
        metadataLanguage: opts.language || opts.metadataLanguage || defaultLanguage,
    ...
    const genreSlider = async (mediaType, opts = {}) => {
        ...
        // (doing this server-side often timed out). Client uses static gradients
