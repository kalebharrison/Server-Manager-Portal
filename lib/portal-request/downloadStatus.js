/** Media status: sent to Radarr/Sonarr but not necessarily downloading. */
export const MEDIA_STATUS_PROCESSING = 3;
...
/**
 * Live Radarr/Sonarr queue rows are attached as downloadStatus.
 * Those use string statuses + sizeLeft — not media status integers.
 */
export const isActiveDownloadItem = (item = {}) => {
 ...
 */
export const hasActiveDownloads = (mediaInfo = {}, opts = {}) => {
...
export const isMediaActivelyProcessing = (mediaInfo = {}, mediaStatus) => {
    ...
    if (status !== MEDIA_STATUS_PROCESSING) return false;
    return hasActiveDownloads(mediaInfo);
};

export const resolveInProgressDisplay = (mediaInfo = {}, mediaStatus) => {
    const status = Number(mediaStatus ?? mediaInfo?.status);
    if (status !== MEDIA_STATUS_PROCESSING) return null;
