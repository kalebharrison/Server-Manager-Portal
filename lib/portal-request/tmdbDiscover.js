...
/**
 * Popular non-English catalogs rolled into default Movies/Series.
 * Japanese animation stays behind the Anime toggle.
 */
const ROLLED_IN_ORIGINAL_LANGUAGES = [
...
const mergeDiscoverPages = (pages, {
    page = 1,
    pageSize = PAGE_SIZE,
    excludeAnime = false,
} = {}) => {
...
export const createTmdbDiscoverRouter = (config = {}, options = {}) => {
    ...
    const fetchPopularBrowse = async (mediaType, baseOpts, page) => {
        ...
        return mergeDiscoverPages(pages, { page, pageSize: PAGE_SIZE, excludeAnime: true });
    };
    ...
    const fetchPath = async (path, query = {}) => {
        ...
        const listOpts = {
            ...
            metadataLanguage,
            region: options.region || '',
            originalLanguageFromLanguageQuery: !useMixedPopular,
