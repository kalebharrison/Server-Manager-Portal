...
/**
 * Upcoming titles rarely have votes yet — use a near-term window + popularity floor instead.
 * 180 days keeps the rail to “coming soon,” not multi-year announcements.
 */
export const DISCOVER_UPCOMING_WINDOW_DAYS = 180;
export const DISCOVER_UPCOMING_MIN_POPULARITY = 25;
/** How many TMDB pages to scan when building a filtered upcoming page. */
const UPCOMING_FETCH_PAGES = 3;
...
export const createTmdbDiscoverRouter = (config = {}, options = {}) => {
    ...
    const fetchPath = async (path, query = {}) => {
        ...
        const fetchUpcomingPage = async (kind, requestedPage) => {
            ...
            const startTmdbPage = ((pageNum - 1) * UPCOMING_FETCH_PAGES) + 1;
            ...
            const pages = await Promise.all(
                Array.from({ length: UPCOMING_FETCH_PAGES }, (_, i) => (
                    fetchOne(startTmdbPage + i).catch(() => emptyPage())
                )),
            );
            const merged = [];
            const seen = new Set();
            ...
            return {
                page: pageNum,
                totalPages: Math.max(1, Math.ceil(totalPages / UPCOMING_FETCH_PAGES)),
