...
/** Popular non-English catalogs for International browse (ja left to Anime). */
const INTERNATIONAL_ORIGINAL_LANGUAGES = [
    'ko', 'zh', 'fr', 'es', 'hi', 'de', 'it', 'pt', 'sv', 'pl', 'th', 'tr', 'nl', 'da', 'no',
];
...
const mergeDiscoverPages = (pages, { page = 1, pageSize = PAGE_SIZE } = {}) => {
    const byKey = new Map();
    let maxTotalPages = 1;
    let totalResults = 0;
    for (const payload of pages) {
        if (!payload) continue;
        maxTotalPages = Math.max(maxTotalPages, Number(payload.totalPages) || 1);
    ...
    // Enough virtual pages to keep infinite scroll advancing across the merge pool.
    const mergedPages = Math.max(maxTotalPages, Math.ceil(sorted.length / pageSize) || 1);
    return {
...
export const createTmdbDiscoverRouter = (config = {}, options = {}) => {
    ...
    const fetchInternationalBrowse = async (mediaType, baseOpts, page) => {
        const discover = mediaType === 'tv' ? client.discoverTv : client.discoverMovies;
        const pages = await Promise.all(
            INTERNATIONAL_ORIGINAL_LANGUAGES.map((lang) => discover({
                ...baseOpts,
                metadataLanguage,
                // Explicit original-language per catalog — not the metadata locale.
                originalLanguageFromLanguageQuery: false,
                originalLanguage: lang,
                voteCountGte: baseOpts.voteCountGte || INTERNATIONAL_VOTE_COUNT_GTE,
                page: 1,
            }).catch(() => emptyPage())),
        );
        // Page advances through the merged popularity pool (sources always fetch page 1).
        return mergeDiscoverPages(pages, { page, pageSize: PAGE_SIZE });
    };
    ...
    const fetchPath = async (path, query = {}) => {
        ...
        const international = asFlag(params.international);
        ...
        // Default Popular: English originals only so foreign softcore can't pollute the main grid.
        // International / Anime / explicit drawer language override this.
        const forcedEnglish = !international && !anime
            && !params.language
            && !originalLanguagePref;
        ...
        const listOpts = {
            ...
            originalLanguage: international
                ? undefined
                : (params.language || originalLanguagePref || (forcedEnglish ? 'en' : undefined)),
        };
        if (forcedEnglish) {
            listOpts.language = 'en';
            listOpts.originalLanguageFromLanguageQuery = true;
        }
        ...
        if (path === '/discover/trending') {
            ...
            // Keep home/default rails English-first when no international browse is requested.
            if (!international && !anime) {
                return {
                    ...trending,
                    results: (trending.results || []).filter((item) => {
                        if (isAdultMediaItem(item)) return false;
        ...
        if (path === '/discover/movies/upcoming') {
            return respond(client.upcomingMovies({
                ...
                originalLanguage: params.language || originalLanguagePref || (forcedEnglish ? 'en' : undefined),
            }));
        }
        ...
        if (movieStudioMatch) {
            return respond(client.discoverMovies({
                ...
                originalLanguageFromLanguageQuery: Boolean(params.language || originalLanguagePref || forcedEnglish),
            }));
        }
        ...
        if (path === '/discover/movies' || /^\/discover\/movies\//i.test(path)) {
            ...
            if (international && !anime) {
                return respond(fetchInternationalBrowse('movie', listOpts, page));
            }
            if (international && anime) {
                const discover = client.discoverMovies;
                const pages = await Promise.all([
                    ...INTERNATIONAL_ORIGINAL_LANGUAGES.map((lang) => discover({
                        ...listOpts,
                        metadataLanguage,
                        originalLanguageFromLanguageQuery: false,
                        originalLanguage: lang,
                        voteCountGte: listOpts.voteCountGte || INTERNATIONAL_VOTE_COUNT_GTE,
                        page: 1,
                    }).catch(() => emptyPage())),
                    discover({
                        ...listOpts,
                        ...
                        originalLanguage: 'ja',
                        genre: '16',
                        voteCountGte: listOpts.voteCountGte || ANIME_VOTE_COUNT_GTE,
                        page: 1,
                    }).catch(() => emptyPage()),
                ]);
                return respond(Promise.resolve(mergeDiscoverPages(pages, { page })));
            }
        ...
        if (path === '/discover/tv' || /^\/discover\/tv\//i.test(path)) {
            ...
            if (international && !anime) {
                return respond(fetchInternationalBrowse('tv', listOpts, page));
            }
            if (international && anime) {
                const discover = client.discoverTv;
                const pages = await Promise.all([
                    ...INTERNATIONAL_ORIGINAL_LANGUAGES.map((lang) => discover({
                        ...listOpts,
                        metadataLanguage,
