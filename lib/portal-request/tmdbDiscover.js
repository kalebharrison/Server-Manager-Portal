...
export const createTmdbDiscoverRouter = (config = {}, options = {}) => {
    ...
    const fetchPath = async (path, query = {}) => {
        ...
        if (anime && !international) {
            // Japanese animation discover (client still applies isAnimeItem).
            params.language = 'ja';
        ...
        const listOpts = {
            ...
            // International (alone or with anime) skips the English-original lock.
            // Anime-only uses language=ja as the original-language filter.
