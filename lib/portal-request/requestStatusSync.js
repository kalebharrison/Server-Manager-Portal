...
import {
    ...
    invalidateLibraryAvailabilityCaches,
} from './libraryAvailability.js';
...
export const syncPortalRequestStatuses = async ({
    ...
    const shouldForce = forceRefresh
        || targets.some((record) => !!record?.meta?.isDownloading);

    // Downloading stamps go stale when Arr imports finish — refresh catalogs before polling.
    if (shouldForce) invalidateLibraryAvailabilityCaches();
    ...
    const library = createLibraryAvailability(config, {
        ...
        forceCatalogRefresh: shouldForce,
    });
    ...
    if (typeof log === 'function' && (summary.updated > 0 || summary.errors > 0)) {
        log(`[portal-request] Status sync: checked ${summary.checked}, updated ${summary.updated}, available ${summary.available}, errors ${summary.errors}`);
    }
