 ...
 * Write: {mediaUserId}-{sanitizedUsername} and n-{mediaUserId}-{sanitizedUsername}
 * mediaUserId = plexId (plex mode) or jellyfinId (jellyfin mode), with fallbacks.
 * Read: media-server ids, legacy portal/Seerr prefixes, bare usernames, notify prefix.
 */
...
/**
 * Durable media-server id for Arr tag writes.
...
export const mediaUserIdForTag = (user, mediaServerType = 'plex') => {
    ...
    if (type === 'jellyfin') {
        return jellyfinId || plexId || portalId;
    }
    return plexId || jellyfinId || portalId;
};
...
/** Canonical requester label for a users.json entry under the active media server. */
...
/** Notify subscriber label: `n-{mediaUserId}-{username}`. */
...
/** True when `label` is this user's canonical requester tag for the active server. */
...
/** True when `label` is this user's notify tag for the active server. */
