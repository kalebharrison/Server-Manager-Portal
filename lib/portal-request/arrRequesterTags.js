 ...
 * Read: portal-id / legacy-id prefixes, bare usernames, optional legacy requester id.
 */
...
export const parseArrRequesterTagLabel = (label) => {
    ...
    // Legacy / portal: "{numericId}-{username}" (e.g. 16-i2ach, 123-kalebharrison)
 ...
 * Prefers stable id matches (portal id / plexId / legacy requester id), then username.
 */
export const resolvePortalUserFromArrTag = (label, portalUsers = []) => {
    ...
    if (parsed.idPrefix) {
        ...
        const byLegacyId = users.find((user) => (
...
/**
 * Collect portal users implied by Arr tag labels (legacy, bare, or portal).
