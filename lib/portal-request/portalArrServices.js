...
export const getPortalArrServiceOptions = async (config, type, serverId, {
    ...
    const value = {
        server: {
            ...
            activeProfileId: (
                Number.isFinite(configuredProfileId) && profileIds.has(configuredProfileId)
                    ? configuredProfileId
                    : (profiles[0]?.id ?? null)
            ),
            activeDirectory: (
                configuredDirectory && rootPaths.has(configuredDirectory)
                    ? configuredDirectory
                    : (rootFolders[0]?.path || null)
            ),
            ...
            activeAnimeProfileId: (
                Number.isFinite(configuredAnimeProfileId) && profileIds.has(configuredAnimeProfileId)
                    ? configuredAnimeProfileId
                    : (Number.isFinite(configuredProfileId) && profileIds.has(configuredProfileId)
                        ? configuredProfileId
                        : (profiles[0]?.id ?? null))
            ),
            activeAnimeDirectory: (
                configuredAnimeDirectory && rootPaths.has(configuredAnimeDirectory)
                    ? configuredAnimeDirectory
                    : (configuredDirectory && rootPaths.has(configuredDirectory)
                        ? configuredDirectory
                        : (rootFolders[0]?.path || null))
            ),
