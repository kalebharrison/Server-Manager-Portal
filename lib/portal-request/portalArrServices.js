 ...
 * Emits server / options DTOs from portal Arr instances.
 */
...
export const inferArrInstanceIs4k = (instance) => {
    // Name wins when present — these flags are often wrong or default false.
    if (nameLooksLike4kServer(instance?.name)) return true;
    if (instance?.is4k === true) return true;
    return false;
};

/** Normalize server DTOs the same way. */
 ...
 * rest to HD — fixes inverted "4K server" checkboxes.
 */
...
export const getPortalArrServiceOptions = async (config, type, serverId, {
    ...
    const configuredProfileId = Number(instance.activeProfileId);
    const configuredLanguageProfileId = Number(instance.activeLanguageProfileId);
    const configuredAnimeProfileId = Number(instance.activeAnimeProfileId);
    const configuredDirectory = String(instance.activeDirectory || '').trim();
    const configuredAnimeDirectory = String(instance.activeAnimeDirectory || '').trim();
    const profileIds = new Set(profiles.map((profile) => Number(profile.id)));
    const rootPaths = new Set(rootFolders.map((folder) => String(folder.path || '').trim()).filter(Boolean));
    // Prefer live *arr lists when present; otherwise keep configured defaults
    // so the request modal can still submit if a transient *arr fetch fails.
    const pickProfileId = (preferred, fallback = null) => {
        const preferredId = Number(preferred);
        if (Number.isFinite(preferredId) && (profileIds.size === 0 || profileIds.has(preferredId))) {
            return preferredId;
        }
        const fallbackId = Number(fallback);
        if (Number.isFinite(fallbackId) && (profileIds.size === 0 || profileIds.has(fallbackId))) {
            return fallbackId;
        }
        return profiles[0]?.id ?? null;
    };
    const pickDirectory = (preferred, fallback = '') => {
        const preferredPath = String(preferred || '').trim();
        if (preferredPath && (rootPaths.size === 0 || rootPaths.has(preferredPath))) {
            return preferredPath;
        }
        const fallbackPath = String(fallback || '').trim();
        if (fallbackPath && (rootPaths.size === 0 || rootPaths.has(fallbackPath))) {
            return fallbackPath;
        }
        return rootFolders[0]?.path || null;
    };

    const activeProfileId = pickProfileId(configuredProfileId);
    const activeDirectory = pickDirectory(configuredDirectory);
    const activeAnimeProfileId = pickProfileId(configuredAnimeProfileId, configuredProfileId);
    const activeAnimeDirectory = pickDirectory(configuredAnimeDirectory, configuredDirectory);
    const effectiveProfiles = profiles.length
        ? profiles
        : (Number.isFinite(Number(activeProfileId))
            ? [{ id: Number(activeProfileId), name: `Profile #${activeProfileId}` }]
            : []);
    const folderPaths = new Set(rootFolders.map((folder) => String(folder.path || '').trim()).filter(Boolean));
    const effectiveRootFolders = [...rootFolders];
    for (const path of [activeDirectory, activeAnimeDirectory]) {
        const folder = String(path || '').trim();
        if (folder && !folderPaths.has(folder)) {
            folderPaths.add(folder);
            effectiveRootFolders.push({ id: effectiveRootFolders.length + 1, path: folder, freeSpace: null });
        }
    }
    ...
    const value = {
        server: {
            ...
            activeLanguageProfileId: (
                Number.isFinite(configuredLanguageProfileId)
                    ? configuredLanguageProfileId
                    : (languageProfiles[0]?.id ?? null)
            ),
            activeAnimeProfileId,
            activeAnimeDirectory,
            activeAnimeLanguageProfileId: null,
            tags: Array.isArray(instance.tags) ? instance.tags : [],
            animeTags: Array.isArray(instance.animeTags) ? instance.animeTags : [],
            instanceId: instance.id,
        },
