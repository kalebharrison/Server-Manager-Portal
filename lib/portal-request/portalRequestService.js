...
export const createPortalRequestService = ({
    ...
    const createMemberRequest = async (user, body = {}) => {
        ...
        // Mirror Overseerr: when auto-approve is on, push to *arr immediately.
        if (shouldPortalAutoApprove(config, mediaType, { is4k: want4k })) {
            try {
    ...
    const pushRecordToArr = async (record) => {
        ...
        const isAnime = mediaType === 'tv' && !!(record?.meta?.isAnime || record?.isAnime);
