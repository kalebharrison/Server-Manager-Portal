...
export const createPortalRequestService = ({
    ...
    const pushRecordToArr = async (record) => {
        ...
        // Mirror Overseerr's requester tagging. Tag creation is intentionally
        // idempotent: *arr returns the existing label after a duplicate POST.
    ...
    const loadOpenNotifyStampIndex = async () => {
        ...
        notifyStampInflight = (async () => {
            ...
            for (const row of all) {
                if (!isOpenNotifyRequest(row)) continue;
