...
export const createPortalRequestService = ({
    ...
    const emitRequestStatusChange = async (record, statusLabel) => {
        if (typeof onRequestStatusChange !== 'function' || !record) return;
        try {
            const requestedBy = (await resolveUser(record.userId).catch(() => null)) || {
                id: record.userId,
                username: record.meta?.requestedByName || null,
                email: record.meta?.requestedByEmail || null,
            };
            await onRequestStatusChange(config, {
                requestedBy,
                title: record.title,
                statusLabel,
                requestId: record.id,
            ...
            // Never fail approve/decline because a notice failed.
        }
    };
    ...
    const createMemberRequest = async (user, body = {}) => {
        ...
        // Session ids are sometimes a Plex account uuid while users.json stores the
        // numeric plexId — fall back to full session matching (email / plexId).
        if (!resolved) {
            const all = await listUsers().catch(() => []);
            resolved = findLocalUserForSession(all, user);
        }
        ...
        invalidateMemberRequestListCache(user);
        invalidateMemberRequestListCache(resolved || user);
    ...
    const MEMBER_REQUEST_LIST_TTL_MS = 15_000;
    /** @type {Map<string, { at: number, dtos: any[], counts: object, records: any[] }>} */
    const memberRequestListCache = new Map();
    /** @type {Map<string, Promise<{ at: number, dtos: any[], counts: object, records: any[] }>>} */
    const memberRequestListInflight = new Map();

    const memberRequestListCacheKey = (user) => String(
        user?.id || user?.plexId || user?.jellyfinId || user?.username || '',
    );

    const invalidateMemberRequestListCache = (user) => {
        const key = memberRequestListCacheKey(user);
        if (key) memberRequestListCache.delete(key);
    };

    const loadMergedMemberRequestDtos = async (user) => {
        const key = memberRequestListCacheKey(user);
        const cached = key ? memberRequestListCache.get(key) : null;
        if (cached && Date.now() - cached.at < MEMBER_REQUEST_LIST_TTL_MS) return cached;
        if (key && memberRequestListInflight.has(key)) return memberRequestListInflight.get(key);

        const pending = (async () => {
            // Do not block the Discover list on disk deletes.
            void pruneAvailablePortalOwnershipRows({
                requestsDir: dataDir,
                onlyImported: true,
            }).catch(() => null);
            ...
            const value = {
                at: Date.now(),
                dtos,
                counts: countMemberRequests(dtos),
                records,
            };
            if (key) memberRequestListCache.set(key, value);
            return value;
        })();

        if (key) memberRequestListInflight.set(key, pending);
        try {
            return await pending;
        } finally {
            if (key && memberRequestListInflight.get(key) === pending) {
                memberRequestListInflight.delete(key);
            }
        }
    };

    const listMemberRequests = async (user, {
        filter = 'all',
        take = 20,
        skip = 0,
        backfillPosters = true,
    } = {}) => {
        const bundled = await loadMergedMemberRequestDtos(user);
        const filtered = bundled.dtos.filter((item) => filterMemberRequestTab(item, filter));
        const page = filtered.slice(skip, skip + take);
        if (backfillPosters) {
            const recordById = new Map(bundled.records.map((row) => [String(row.id), row]));
    ...
    /**
     * Community “Recently requested” rail — recent portal requests from anyone,
     * excluding declined. Viewer notify stamps applied by the route layer.
     */
    const listRecentCommunityRequests = async (viewer, { take = 40 } = {}) => {
        const limit = Math.min(80, Math.max(1, Number(take) || 40));
        ...
        for (const record of sorted) {
            ...
            if (status === REQUEST_STATUS_DECLINED) continue;
            ...
            if (seen.has(key)) continue;
            seen.add(key);
            const dto = mapPortalRecordToDto(record);
            results.push({
                id: tmdbId,
                tmdbId,
                mediaType,
                type: mediaType,
                title: dto.title || record.title || 'Untitled',
                name: dto.title || record.title || 'Untitled',
                year: dto.year || record.year || null,
                posterPath: dto.posterPath || record.posterPath || null,
                overview: dto.overview || record.overview || '',
                requestId: dto.id,
                requestedByName: dto.requestedBy?.displayName || record?.meta?.requestedByName || null,
                mediaInfo: {
                    status: Number(dto.mediaStatus) || (status === REQUEST_STATUS_PENDING ? 2 : 3),
                    requests: [{
                        id: dto.id,
                        status,
                        is4k: !!record.is4k,
                        seasons: Array.isArray(record.seasons) ? record.seasons : [],
                    }],
                    requestAttribution: {
                        requestedByName: dto.requestedBy?.displayName || record?.meta?.requestedByName || null,
                    },
                    ...(dto.isDownloading ? { downloadStatus: [{ status: 'downloading' }] } : {}),
                },
            });
    ...
    const pushRecordToArr = async (record) => {
        ...
        // Requester tagging: one stable portal-user tag `{user.id}` (no username).
        ...
        if (config?.tagRequests !== false) {
            ...
            const tagLabel = buildPortalRequesterTagForUser(tagUser);
            if (tagLabel) {
                try {
    ...
    const approveAdminRequest = async (requestId, overrides = null, adminUser = null) => {
        ...
        // Already on disk (existing Arr title) → Available immediately; don't stick on Processing.
        const entity = pushResult.entity || null;
        const alreadyOnDisk = record.mediaType === 'tv'
            ? (Number(entity?.statistics?.episodeFileCount) > 0)
            : (entity?.hasFile === true
                || Number(entity?.movieFileId) > 0
                || !!(entity?.movieFile?.id || entity?.movieFile?.relativePath));
        ...
        void emitRequestStatusChange(approved, 'approved');
    ...
    const declineAdminRequest = async (requestId, reason = '', adminUser = null) => {
        ...
        void emitRequestStatusChange(declined, 'declined');
    ...
    const retryMemberRequest = async (user, requestId) => {
        ...
        const retried = await retryAdminRequest(requestId, user);
        invalidateMemberRequestListCache(user);
        return retried;
    };
    ...
    /**
     * Portal requests ready for “now available” notices.
     * Movies: full Available. TV: Season 1 complete (later seasons may still be partial).
     * Skips rows already stamped with availabilityNotifiedAt (one-shot).
     */
    const listAvailableNotifyCandidates = async () => {
        ...
        for (const row of rows) {
            if (row?.meta?.availabilityNotifiedAt) continue;
            if (row?.meta?.isDownloading) continue;
            const mediaType = row.mediaType === 'tv' ? 'tv' : 'movie';
            const tmdbId = Number(row.tmdbId) || null;
            const mediaStatus = Number(row?.meta?.mediaStatus);

            if (mediaType === 'movie') {
                if (mediaStatus !== 5) continue;
            } else {
                let seasonOneReady = false;
                if (tmdbId) {
                    try {
                        const availability = await library.getTvListStatus(tmdbId, {
                            title: row.title || '',
                            networkLookups: true,
                        });
                        if (availability?.downloading) continue;
                        const seasonRows = Array.isArray(availability?.mediaInfo?.seasons)
                            ? availability.mediaInfo.seasons
                            : [];
                        const s1 = seasonRows.find((season) => Number(season?.seasonNumber) === 1);
                        seasonOneReady = Number(s1?.status) === 5;
                    } catch {
                        seasonOneReady = mediaStatus === 5;
                    }
                } else {
                    seasonOneReady = mediaStatus === 5;
                }
                if (!seasonOneReady) continue;
            }
    ...
    const markAvailabilityNotified = async (requestId) => {
        const id = Number(requestId);
        if (!Number.isFinite(id) || id <= 0) return null;
        const existing = typeof store.get === 'function'
            ? await store.get(id)
            : (await store.list({})).find((entry) => Number(entry?.id) === id);
    ...
    return {
        ...
        syncRequestStatuses: (options = {}) => syncPortalRequestStatuses({
            config,
            store,
            resolveUrl,
            fetchImpl,
