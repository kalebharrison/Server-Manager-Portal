...
import { isAnimeItem } from '../request-app/request-app-media.js';
...
import { lookupArrRequesterForTmdb } from './arrTagOwnershipImport.js';
...
export const createPortalRequestService = ({
    ...
    const pushRecordToArr = async (record) => {
        ...
        // Fill missing routing from live *arr defaults (Overseerr-parity instance prefs first).
        ...
        // Mirror Overseerr's requester tagging as {portalUserId}-{username}.
        ...
        if (config?.tagRequests !== false) {
            ...
            const username = usernameRaw.includes('@')
                ? usernameRaw.split('@')[0]
                : usernameRaw;
            const requesterUserId = requester?.id ?? record.userId;
            const tagLabel = buildPortalRequesterTagLabel(requesterUserId, username);
            if (tagLabel) {
                try {
    ...
    const getMemberRequestOptions = async (user, { mediaType, mediaId } = {}) => {
        ...
        if (openPortalRequest) {
            ...
            // (Seerr-era library titles, media added outside the portal).
    ...
    const getNotifyState = async (user, mediaType, tmdbId) => {
        ...
        // Fallback when meta.mediaStatus was marked available early but the request
        // is still pending/approved — members should still be able to subscribe.
        ...
        if (!open.length) {
            return { canNotify: false, notifying: false, requestId: null, isOwner: false };
        }
        ...
        const notifying = memberUserIds(user).some((id) => notifyIds.includes(id));
        return {
            canNotify: !notifying,
            notifying,
    ...
    const subscribeNotify = async (user, body = {}) => {
        ...
        if (!open.length) {
            const err = new Error('No open request exists for this title yet.');
            err.status = 404;
            throw err;
        }
        const ownerIds = new Set(memberUserIds(user));
        if (open.some((row) => ownerIds.has(String(row.userId || '')))) {
    ...
    const unsubscribeNotify = async (user, body = {}) => {
        ...
        if (!open.length) {
            return { success: true, notifying: false, canNotify: false, requestId: null };
        }
