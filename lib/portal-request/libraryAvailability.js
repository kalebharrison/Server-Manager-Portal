...
export const movieHasFile = (movie = {}) => {
...
/** Drop in-memory Arr catalogs/queues so request status sync sees fresh hasFile state. */
export const invalidateLibraryAvailabilityCaches = () => {
    movieCatalogCache.clear();
    seriesCatalogCache.clear();
    queueCache.clear();
...
/**
 * Season 1 fully on disk (aired/monitored episode files).
 * Specials never count. Later seasons do not gate this check.
 */
export const isSeasonOneComplete = (series = {}) => {
    const seasons = Array.isArray(series.seasons) ? series.seasons : [];
    const seasonOne = seasons.find((season) => Number(season?.seasonNumber) === 1);
    if (!seasonOne) return false;
    const stats = seasonOne.statistics || {};
    const files = Number(stats.episodeFileCount) || 0;
    const expected = seasonExpectedFileCount(stats);
    const percent = Number(stats.percentOfEpisodes);
    if (expected > 0) return files >= expected;
    if (Number.isFinite(percent) && percent >= 100 && files > 0) return true;
    return false;
};
...
export const createLibraryAvailability = (config = {}, options = {}) => {
        ...
        const seasons = Array.isArray(series.seasons)
            ? series.seasons
                .map((season) => {
                    ...
                    const expected = seasonExpectedFileCount(stats);
                    const percent = Number(stats.percentOfEpisodes);
                    const complete = (expected > 0 && episodeFileCount >= expected)
                        || (Number.isFinite(percent) && percent >= 100);
                    return {
                        seasonNumber,
                        status: complete ? MEDIA_STATUS_AVAILABLE : MEDIA_STATUS_PARTIAL,
                    };
                })
                .filter(Boolean)
            : [];
    ...
    const snapshotLibraryStatuses = async () => {
        ...
        // Reuse warm in-memory catalogs when fresh (5m CATALOG_CACHE_MS); disk snapshot
        // aligns to ~12h and catalogs are warmed every 5m separately. Empty catalogs
        // expire after EMPTY_CATALOG_CACHE_MS (30s); 0 titles still throws below.
...
/**
 * Shared Arr catalog list for ownership / request rails.
 * Reuses the Discover warm cache so My Requests does not download /movie + /series again.
 */
export const loadArrInstanceCatalogList = async (instance, fetchOpts = {}) => {
    if (!isArrInstanceReady(instance)) return [];
    const type = String(instance?.type || '').toLowerCase();
    const key = instanceCacheKey(instance);
    const peek = type === 'sonarr' ? seriesCatalogCache.get(key) : movieCatalogCache.get(key);
    const forceRefresh = fetchOpts.forceRefresh === true;
    if (!forceRefresh && Array.isArray(peek?.list) && peek.list.length) {
        if (!isCatalogFresh(peek)) {
            const refreshOpts = { timeoutMs: 8000, ...fetchOpts, forceRefresh: false };
            if (type === 'sonarr') loadInstanceSeriesCatalog(instance, refreshOpts).catch(() => {});
            else loadInstanceMovieCatalog(instance, refreshOpts).catch(() => {});
        }
        return peek.list;
    }
    const loadOpts = { timeoutMs: 8000, ...fetchOpts };
    try {
        const cached = type === 'sonarr'
            ? await loadInstanceSeriesCatalog(instance, loadOpts)
            : await loadInstanceMovieCatalog(instance, loadOpts);
        return Array.isArray(cached?.list) ? cached.list : [];
    } catch {
        return Array.isArray(peek?.list) ? peek.list : [];
    }
};
