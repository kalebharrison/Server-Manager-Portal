 ...
 * - Normalize: migrate legacy id-username / Seerr / bare tags → one portal {user.id} tag
 *   (and n-{user.id} for notify), removing superseded ownership/notify tags from the item
...
/**
 * Scan Radarr/Sonarr: migrate legacy ownership/notify tags to stable portal id tags.
 * - Ensures `{user.id}` / `n-{user.id}` on the item
 * - Removes superseded Seerr / id-username / bare-username tags that map to that user
 * - Leaves unrelated Arr tags (anime, quality, etc.) alone
    ...
    for (const { mediaType, arrType } of scans) {
        ...
        for (const instance of instances) {
            ...
            for (const item of (Array.isArray(catalog) ? catalog : [])) {
                ...
                // Drop superseded ownership/notify labels once canonical forms are present.
                const keepIds = [];
                for (const tagId of nextTagIds) {
                    ...
                    const parsed = parseArrRequesterTagLabel(label);
                    const canonical = parsed?.kind === 'notify'
                        ? isExactCanonicalNotifyLabel(label, resolved)
                        : isExactCanonicalRequesterLabel(label, resolved);
                    if (canonical) {
                        keepIds.push(tagId);
                        continue;
                    }
                ...
                summary.tagsRemoved += removedOnItem;
