...
/** Seerr ownership tags use `{seerrUserId}-{username}` — never remove or rewrite these. */
const isSeerrRequesterTag = (label, user) => {
    const seerrId = sanitizeArrTagSegment(user?.seerrUserId);
    if (!seerrId) return false;
    const parsed = parseArrRequesterTagLabel(label);
    if (!parsed || parsed.kind === 'notify' || parsed.bare) return false;
    return sanitizeArrTagSegment(parsed.idPrefix) === seerrId;
};

/**
 * Tags we may strip after ensuring the canonical portal id form:
 * portal/media-server `{id}-{username}` and `n-{id}-{username}` leftovers.
 * Never Seerr, never bare usernames, never unrelated Arr tags.
 */
const isCorrectableLegacyUserTag = (label, user) => {
    if (!user?.id) return false;
    if (isSeerrRequesterTag(label, user)) return false;
    if (isExactCanonicalRequesterLabel(label, user) || isExactCanonicalNotifyLabel(label, user)) {
        return false;
    }
    const parsed = parseArrRequesterTagLabel(label);
    if (!parsed) return false;
    // Leave bare Seerr-era usernames alone.
 ...
 * - Never remove or rewrite Seerr `{seerrId}-{username}` tags
