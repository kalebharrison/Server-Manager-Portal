 ...
 * - Normalize legacy/bare requester tags → media-server {id}-{username} tags on Arr
...
/**
 * Scan Radarr/Sonarr: for each legacy/bare requester tag that maps to a portal user,
 * ensure the media-server `{plexOrJellyfinId}-{username}` tag exists on that item.
    ...
    for (const { mediaType, arrType } of scans) {
        ...
        for (const instance of instances) {
            ...
            for (const item of (Array.isArray(catalog) ? catalog : [])) {
                ...
                // Ignore notify-only labels when discovering owners to normalize.
                const requesterLabels = labels.filter((label) => parseArrRequesterTagLabel(label)?.kind !== 'notify');
                const owners = collectPortalUsersFromArrTagLabels(requesterLabels, portalUsers);
                if (!owners.size) {
                ...
                for (const { user } of owners.values()) {
                    // Dual-linked users get both plex and jellyfin requester tags.
                    const portalLabels = buildPortalRequesterTagsForUser(user);
                    for (const portalLabel of portalLabels) {
                        if (labels.some((label) => sanitizeArrTagSegment(label) === portalLabel)
                            || nextTagIds.some((id) => sanitizeArrTagSegment(tagById.get(id) || '') === portalLabel)) {
...
export const setArrNotifyTagForUser = async ({
    ...
    const notifyLabels = buildNotifyTagsForUser(user);
    if (!notifyLabels.length) return { ok: false, reason: 'Unable to build notify tag' };
    const primaryLabel = notifyLabels[0];
    ...
    if (subscribe) {
        let nextTagIds = [...currentTagIds];
        let added = false;
        for (const label of notifyLabels) {
        ...
        if (!added) return { ok: true, already: true, label: primaryLabel, labels: notifyLabels };
        const updated = await updateArrEntityTags(instance, item, nextTagIds, fetchOpts);
        return updated.ok
            ? { ok: true, label: primaryLabel, labels: [...labels, ...notifyLabels] }
            : { ok: false, reason: updated.reason || 'Tag update failed' };
    }

    const removeIds = new Set();
    const wanted = new Set(notifyLabels.map(sanitizeArrTagSegment));
    for (const [id, existingLabel] of tagById.entries()) {
        if (wanted.has(sanitizeArrTagSegment(existingLabel))) {
            removeIds.add(Number(id));
            continue;
        }
        // Also drop legacy / alternate-id notify tags that resolve to this user.
    ...
    if (!removeIds.size) return { ok: true, already: true, label: primaryLabel, labels: notifyLabels };
    const next = currentTagIds.filter((id) => !removeIds.has(id));
    const updated = await updateArrEntityTags(instance, item, next, fetchOpts);
    return updated.ok
        ? { ok: true, label: primaryLabel, labels: notifyLabels, removed: true }
        : { ok: false, reason: updated.reason || 'Tag update failed' };
};
