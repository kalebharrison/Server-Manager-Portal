...
const mediaServerTypeFromConfig = (config) => (
    String(config?.mediaServerType || 'plex').toLowerCase() === 'jellyfin' ? 'jellyfin' : 'plex'
);
...
export const normalizeArrRequesterTagsOnArr = async ({
    ...
    const mediaServerType = mediaServerTypeFromConfig(config);
    const fetchOpts = { resolveUrl, fetchImpl };
    const scans = [];
        ...
        for (const instance of instances) {
            ...
            for (const item of (Array.isArray(catalog) ? catalog : [])) {
                ...
                for (const { user } of owners.values()) {
                    const portalLabel = buildPortalRequesterTagForUser(user, mediaServerType);
                    if (!portalLabel) continue;

                    if (labels.some((label) => isPortalRequesterTagForUser(label, user, mediaServerType))) {
...
export const setArrNotifyTagForUser = async ({
    ...
    const mediaServerType = mediaServerTypeFromConfig(config);
    const label = buildNotifyTagForUser(user, mediaServerType);
    ...
    for (const [id, existingLabel] of tagById.entries()) {
        ...
        // Also drop legacy portal notifyUserIds-era duplicates that resolve to this user as notify.
