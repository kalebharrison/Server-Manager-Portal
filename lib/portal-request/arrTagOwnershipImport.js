 ...
 * - Normalize: ensure portal {user.id} / n-{user.id}; rewrite portal/media legacy
 *   id-username tags onto that form. Seerr `{seerrId}-{user}` tags are never touched.
...
import { loadArrInstanceCatalogList } from './libraryAvailability.js';
...
export const userOwnsArrRequesterLabel = (label, user, portalUsers = []) => {
    ...
    // (avoids `16-alice` matching a user who is merely named alice).
    if (parsed.idPrefix) {
    ...
    // Bare username tags: only if they resolve to this portal user (never match on
    // empty plexId/jellyfinId — that made every Plex-only admin "own" others' tags).
    ...
    if (String(resolved.id) === String(user.id)) return true;
    const resolvedPlex = String(resolved.plexId || '').trim();
    const userPlex = String(user.plexId || '').trim();
    if (resolvedPlex && userPlex && resolvedPlex === userPlex) return true;
    const resolvedJf = String(resolved.jellyfinId || '').trim();
    const userJf = String(user.jellyfinId || '').trim();
    if (resolvedJf && userJf && resolvedJf === userJf) return true;
    return false;
};
...
/**
 * Seerr requester tags are always `{1-2 digit id}-{username}` (e.g. `16-alice`).
 * Pattern match is the hard exclude — never remove/rewrite these, even if seerrUserId is missing.
 */
export const isSeerrStyleRequesterTag = (label) => {
    const parsed = parseArrRequesterTagLabel(label);
    if (!parsed || parsed.kind === 'notify' || parsed.bare || !parsed.idPrefix) return false;
    return /^\d{1,2}$/.test(String(parsed.idPrefix));
};

/**
 * Tags we may strip after ensuring the canonical portal id form:
 * portal/media-server `{id}-{username}` and `n-{id}-{username}` leftovers.
 * Never Seerr-style 1-2 digit tags, never bare usernames, never unrelated Arr tags.
 */
const isCorrectableLegacyUserTag = (label, user) => {
    if (!user?.id) return false;
    // Hard exclude: Seerr's short numeric id tags (1-2 digits).
    if (isSeerrStyleRequesterTag(label)) return false;
    if (isExactCanonicalRequesterLabel(label, user) || isExactCanonicalNotifyLabel(label, user)) {
        return false;
    }
    const parsed = parseArrRequesterTagLabel(label);
    if (!parsed) return false;
    // Leave bare usernames alone.
    if (parsed.bare) return false;

    const prefix = sanitizeArrTagSegment(parsed.idPrefix);
    if (!prefix) return false;
    // Extra safety: never correct any 1-2 digit prefixed tag, even if parse shape drifts.
    if (/^\d{1,2}$/.test(prefix)) return false;

    const portalId = sanitizeArrTagSegment(user.id);
    const plexId = sanitizeArrTagSegment(user.plexId);
    const jellyfinId = sanitizeArrTagSegment(user.jellyfinId);
    return prefix === portalId || prefix === plexId || prefix === jellyfinId;
};

/**
 * Scan Radarr/Sonarr:
 * - Ensure stable portal `{user.id}` / `n-{user.id}` tags
 * - Replace correctable portal/media legacy id-username tags with those
 * - Never remove or rewrite Seerr-style `{1-2 digit id}-{username}` tags
    ...
    for (const { mediaType, arrType } of scans) {
        ...
        for (const instance of instances) {
            ...
            for (const item of (Array.isArray(catalog) ? catalog : [])) {
                ...
                // Strip only correctable portal/media legacy labels — never Seerr.
                const keepIds = [];
                for (const tagId of nextTagIds) {
                    const label = tagById.get(tagId) || '';
                    const resolved = resolvePortalUserFromArrTag(label, portalUsers);
                    if (!resolved?.id || !isCorrectableLegacyUserTag(label, resolved)) {
                        keepIds.push(tagId);
                        continue;
                    }
                    correctedOnItem += 1;
                ...
                summary.tagsCorrected += correctedOnItem;
...
export const listArrOwnershipDtosForUser = async ({
    ...
    for (const { mediaType, arrType } of scans) {
        const instances = getArrInstances(config, { type: arrType, enabledOnly: true })
            .filter(isArrInstanceReady);
        for (const instance of instances) {
            const catalogOpts = { ...fetchOpts, timeoutMs: 8000 };
            const [tags, catalog] = await Promise.all([
                fetchArrTags(instance, catalogOpts).catch(() => []),
                // Production fetch shares Discover's warm catalog. Tests pass a custom
                // fetchImpl and must hit that mock, not the process-wide catalog cache.
                (fetchImpl === fetch
                    ? loadArrInstanceCatalogList(instance, catalogOpts)
                    : fetchArrInstanceCatalogItems(instance, catalogOpts).catch(() => [])),
            ]);
            const tagById = new Map(
                (Array.isArray(tags) ? tags : []).map((tag) => [Number(tag.id), String(tag.label || '').trim()]),
            );
