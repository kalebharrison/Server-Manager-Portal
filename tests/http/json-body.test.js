...
test('inbound webhook accepts large JSON while other API routes stay capped', async () => {
    ...
    app.post('/api/webhooks/inbound-email', (req, res) => res.json({ ok: true, bytes: JSON.stringify(req.body).length }));
    ...
    await withServer(app, async (port) => {
        const bulky = { 'Text-part': 'x'.repeat(80_000), Recipient: 't.smtp.test@reply.example.com' };
        const inbound = await fetch(`http://127.0.0.1:${port}/api/webhooks/inbound-email`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(bulky),
        });
        assert.equal(inbound.status, 200);
        const legacy = await fetch(`http://127.0.0.1:${port}/api/webhooks/mailjet-inbound`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(bulky),
        });
