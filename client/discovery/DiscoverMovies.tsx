    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const [home, popularRes, upcomingRes, dashboard, upgrades] = await Promise.all([
                ...
                browseMode === 'discover'
                    ? apiFetch('/api/discovery/recent-upgrades?mediaType=movie&take=40', { forceRefresh: true, cacheTtlMs: 0 }).catch(() => null)
                    : Promise.resolve(null),
            ]);
            ...
            if (browseMode === 'discover') {
                const mappedRecent = (Array.isArray(dashboard?.recentMovies) ? dashboard.recentMovies : [])
                    .map((item: any) => libraryRecentToDiscoveryItem(item, 'movie'));
                    ...
                    {browseMode === 'discover' && (
                        <>
                            <DiscoverMediaRail
                                ...
                                onBrowseRail={() => navigate(buildRailBrowsePath(browseBase, 'recently-added', 'movie'))}
                            />
                            <DiscoverMediaRail
                                ...
                                showPosterQualityBadges={showPosterQualityBadges}
                                viewAllLabel={viewAllLabel}
                                onBrowseRail={() => navigate(buildRailBrowsePath(browseBase, 'recently-upgraded', 'movie'))}
                            />
                        </>
                    )}
                    <DiscoverMediaRail
                        ...
                        showPosterQualityBadges={showPosterQualityBadges}
                        viewAllLabel={viewAllLabel}
                        onBrowseRail={() => navigate(buildRailBrowsePath(browseBase, 'trending', 'movie'))}
                    />
                    <DiscoverMediaRail
                        ...
                        showPosterQualityBadges={showPosterQualityBadges}
                        viewAllLabel={viewAllLabel}
                        onBrowseRail={() => navigate(buildRailBrowsePath(browseBase, 'upcoming', 'movie'))}
                    />
                    <DiscoverMediaRail
                        ...
                        showPosterQualityBadges={showPosterQualityBadges}
                        viewAllLabel={viewAllLabel}
                        onBrowseRail={() => navigate(buildRailBrowsePath(browseBase, 'popular', 'movie'))}
                    />
                </div>
            )}
        </div>
